...
#define LLDB_PREP_CMDS CFSTR("\
    ...
    command script add -s asynchronous -f {python_command}.prerun_command prerun\n\
...
#ifdef EMBARCADERO_Local
bool silent = false;
CFStringRef custom_lldb_shell = NULL;
#endif
...
#ifdef EMBARCADERO_Local
CFStringRef create_lldb_shell(char *path) {
    ...
    if( !path_exists(cf_path) ) {
        on_error(@"file does not exist: %@", cf_path);
    }

    *ptr = '\0';
    CFStringRef cf_dirname = CFStringCreateWithCString(NULL, path, kCFStringEncodingUTF8);
    *ptr = '/';

    CFStringRef cf_lldb_shell = CFStringCreateWithFormat(NULL, NULL, CFSTR("PATH=%@ %@ -s %s"), cf_dirname, cf_path, "%@");

    CFRelease(cf_dirname);
    CFRelease(cf_path);

    return cf_lldb_shell;
}

NSString *get_lldb_shell() {
    if (custom_lldb_shell != NULL)
        return (__bridge NSString *)custom_lldb_shell;
    return LLDB_SHELL;
}
#endif
...
#ifdef EMBARCADERO_Local
        lldb_shell = [NSString stringWithFormat:get_lldb_shell(), prep_cmds];
#else
...
void launch_debugger_and_exit(AMDeviceRef device, CFURLRef url) {
    ...
    if (pid == 0) {
        ...
        NSString* prep_cmds = [NSString stringWithFormat:PREP_CMDS_PATH, tmpUUID];
#ifdef EMBARCADERO_Local
        NSString* lldb_shell = [NSString stringWithFormat:get_lldb_shell(), prep_cmds];
#else
...
#ifdef EMBARCADERO_Local
void launch_debugger_nostart(AMDeviceRef device, CFURLRef url) {
    setup_lldb(device, url);

    NSString* prep_cmds = [NSString stringWithFormat:PREP_CMDS_PATH, tmpUUID];
    if(device_id != NULL) {
        prep_cmds = [prep_cmds stringByAppendingString: [[NSString stringWithUTF8String:device_id] stringByReplacingOccurrencesOfString:@"-" withString:@"_"]];
    }
    silent = false;
    NSLogOut(@"file=%@", prep_cmds);
}
#endif
...
void handle_device(AMDeviceRef device) {
    ...
    if (justlaunch) {
        ...
        launch_debugger_nostart(device, url);
#endif
    } else {
...
#ifdef EMBARCADERO_Local
        @"  -q, --quiet                  silent mode\n"
        @"  -z, --lldb <lldb pathname>   the path to the lldb\n"
#endif
...
int main(int argc, char *argv[]) {
...
#ifdef EMBARCADERO_Local
    while ((ch = getopt_long(argc, argv, "VmcdvunrILefFD:R:X:i:b:a:t:p:1:2:o:l:w:9BWjNs:OE:CA:k:S:Pqz:", longopts, NULL)) != -1)
#else
...
#endif
    {
        ...
        case 'q':
            ...
            break;
        case 'z':
            custom_lldb_shell = create_lldb_shell(optarg);
            break;
#endif
        case 'm':
            install = false;
