...
#else
			bool isSystemFramework = ( dylibFile->installPath() != NULL ) && ( dylibFile->installPath()[0] == PATH_SEP );
#endif
...
#else
						char* lastSlash = strrchr(tempPath, PATH_SEP);
#endif
						if ( lastSlash != NULL )
...
#else
		if ( (depInstallName != NULL) && (depInstallName[0] != PATH_SEP) ) {
#endif
...
#else
	const char* shortPath = strrchr(path, PATH_SEP);
#endif
	if ( shortPath == NULL )
		return path;
	else
...
#ifdef BORLAND_LD
						fprintf(stderr, "      %s in %s;", _options.demangleSymbol(atom->name()), pathLeafName(atom->safeFilePath()));
#else
	...
	if ( unresolvableCount != 0 ) {
		if ( doPrint ) {
			...
			for (const std::string_view& name : unresolvableUndefines) {
				unsigned int slot = _symbolTable.findSlotForName(name);
#ifdef BORLAND_LD
				fprintf(stderr, "  Error: \"%s\", referenced from:", _options.demangleSymbol(name.data()));
#else
...
#else
	for (auto index = 0; index < weakDefDylibs.size(); index++) {
		ld::dylib::File* dylib = weakDefDylibs[index];
