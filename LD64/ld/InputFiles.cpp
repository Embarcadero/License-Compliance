...
#else
                        static const int page_mask = 0x0000FFFF;
#endif
...
#ifdef BORLAND_LD
        objOpts.fBeQuiet = _options.mustBeQuiet();
#endif
...
#else
                                        char* lastSlash = (char*)findLastSlash(tmpPath);
#endif					
                                        if ( lastSlash != NULL )
...
#else
			throwf("building for %s-%s but attempting to link with file (\"%s\") built for %s-%s",
					_options.platforms().to_str().c_str(), _options.architectureName(),
				info.path,
					ld::nameFromPlatform(filePlatform), fileArchName);
#endif
...
#else
        const char* myLeaf = findLastSlash(_options.installPath());
#endif
	if ( myLeaf == nullptr )
		return;
...
#else
                const char* installNameLastSlash = findLastSlash(installName); 
#endif
		bool isParent = false;
