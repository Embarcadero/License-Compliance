...
const char* Parser<A>::getDwarfString(uint64_t form, const uint8_t*& di, bool dwarf64)
{
	...
	switch (form) {
		...
		case DW_FORM_strp:
			...
			offset = E::get32(*((uint32_t*)di));
			if (offset >= _file->_dwarfDebugStringSect->size())
				offset -= (uint32_t)(_file->_dwarfDebugStringSect->addr());
...
#ifdef BORLAND_LD
template<>
const char* Parser<arm64>::getDwarfString(uint64_t form, const uint8_t*& di, bool dwarf64)
{
	uint32_t offset;
	const char* dwarfStrings;
	const char* result = NULL;
  ...
  case DW_FORM_strx4:
    ...
    if (offset >= _file->_dwarfDebugStringSect->size())
      offset -= (uint32_t)(_file->_dwarfDebugStringSect->addr());
		...
		if (offset < _file->_dwarfDebugStringSect->size())
			result = &dwarfStrings[offset];
		if (strcmp(result, "_ZN6System8Generics11Collections14TDictionary__2IPvPNS_4Rtti11TRttiObjectEE4HashEPKv") == 0) {
			printf("ho!");
		}
		if (strcmp(result, "_ZN6System8Generics11Collections14TDictionary__2IPvPNS_4Rtti11TRttiObjectEE11TryGetValueEPKvRS6_") == 0) {
			printf("ho!");
		}
		//		else
//			if (!opts.fBeQuiet)
...
template<>
const char* Parser<x86_64>::getDwarfString(uint64_t form, const uint8_t*& di, bool dwarf64)
{
  uint32_t offset;
  const char* dwarfStrings;
  const char* result = NULL;
...
#else
	if ( (tuName != NULL) && (tuName[0] == PATH_SEP) ) {
#endif
