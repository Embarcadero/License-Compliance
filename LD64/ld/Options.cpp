...
#define SUPPRESS_STD
#include <cxxabi.h>			// gets the one from unix2win
#undef SUPPRESS_STD
#include <algorithm>
...
static size_t findLastSlash(const std::string &str, size_t pos = std::string::npos) {
  auto lastSlashIdx = str.find_last_of(PATH_SEP, pos);
  if (lastSlashIdx == std::string::npos)
    lastSlashIdx = str.find_last_of(OTHER_PATH_SEP, pos);
  else {
    auto lastSlashIdx2 = str.find_last_of(OTHER_PATH_SEP, pos);
    if (lastSlashIdx2 != std::string::npos) {
      if (lastSlashIdx2 > lastSlashIdx)
        return lastSlashIdx2;
    }
  }
  return lastSlashIdx;
}

static const char* findLastSlash(const char* file) {
  const char* s = strrchr(file, PATH_SEP);
  if (s == NULL)
    s = strrchr(file, OTHER_PATH_SEP);
  else {
    const char* s2 = strrchr(file, OTHER_PATH_SEP);
    if (s2) {
      if ((s2 - file) > (s - file))
        return s2;
    }
...
bool absPath(const std::string &path) {
#ifdef BORLAND_LD
	return (path[0] == PATH_SEP) 
			|| (path[0] == OTHER_PATH_SEP) 
...
fLinkSnapshot(this), 
#endif
fSnapshotRequested(false), fPipelineFifo(NULL),
...
#ifdef BORLAND_LD
static bool hasExtension(const std::string &path, const char* thisExt) {
  auto lastSlashIdx = findLastSlash(path);
  auto lastDotIdx = path.find_last_of('.');
  if ((lastDotIdx != std::string::npos)
    && ((lastSlashIdx == std::string::npos) || (lastDotIdx > lastSlashIdx))) {
    if (thisExt)
      return path.substr(lastDotIdx) == thisExt;
    return true;
  }
  return false;
}
...
#ifdef BORLAND_LD
std::string extractLnkFileContent(Options::FileInfo lnkFile) {
  int fd = ::open(lnkFile.path, O_RDONLY, 0);
  struct stat stat_buf;
  if ((fd != -1) && fstat(fd, &stat_buf) != -1) {
    char linkContent[PATH_MAX + 1];
    ssize_t readAmount = stat_buf.st_size;
    if (PATH_MAX < readAmount)
      readAmount = PATH_MAX;
    ssize_t amount = read(fd, linkContent, readAmount);
    ::close(fd);
    if (amount >= readAmount) {
      linkContent[amount] = '\0';
      std::string newPath = lnkFile.path;
      auto lastSlashIdx = findLastSlash(newPath);
      if (lastSlashIdx != std::string::npos)
        newPath = newPath.substr(0, lastSlashIdx + 1);
      else
        newPath.append("\\");
      if ((linkContent[0] == '\xEF') && (linkContent[1] == '\xBB') && (linkContent[2] == '\xBF'))
        newPath.append(linkContent + 3);
      else
        newPath.append(linkContent);
      return newPath;
    }
  }
  return std::string();
...
#else
  if (suffix != NULL) {
    // unless it's our magic lnk suffix, try without it
    if (strcmp(suffix, ".lnk") != 0)
      return findFramework(rootName, NULL);
  }
  else
    if (!hasExtension(rootName, 0)) {
      try {
        FileInfo result = findFramework(rootName, ".lnk");
        std::string newPath = extractLnkFileContent(result);
        if (newPath.length()) {
          FileInfo result(*this);
        ...
        throwf("file not found: %s", rootName);
      }
    }
  throwf("framework not found %s", rootName);
#endif
...
#else
        std::string::size_type lastSlashPos = findLastSlash(path);
#endif
...
#else
	if ( absPath(path) && !pathIsObjectFile ) {
#endif
...
#ifdef BORLAND_LD
  if (!hasExtension(path, 0)) {
    std::string newPath = path;
    newPath.append(".lnk");
    try {
      result = findFile(newPath, fromDylib);
      std::string newPath = extractLnkFileContent(result);
      if (newPath.length())
        return findFile(newPath, fromDylib);
    }
    catch (const char*) {
...
#else
        auto lastSlashPos = findLastSlash(installName);
#endif
...
#else
                auto beginPos = findLastSlash(installName, endPos);
#endif
...
#ifdef BORLAND_LD
ld::File::Ordinal Options::loadGRP_File(FileInfo info, bool fromFileList, ld::File::Ordinal startOrdinal) {
  char bomSearch[4];
  ...
  if (fstat(fd, &stat_buf) != -1) {
    ...
    if (4 < readAmount)
      readAmount = 4;
    ssize_t amount = read(fd, bomSearch, readAmount);
    ::close(fd);
    if (amount >= readAmount) {
      if ((bomSearch[0] == '\xEF') && (bomSearch[1] == '\xBB') && (bomSearch[2] == '\xBF')) {
        ...
        while (fgets(path, PATH_MAX, file) != NULL) {
          ...
          info = findFile(path);
          if (fromFileList) {
            info.ordinal = startOrdinal.nextFileListOrdinal();
            startOrdinal = info.ordinal;
            info.fromFileList = true;
            fInputFiles.push_back(info);
          }
          else {
            info.ordinal = startOrdinal;
            fInputFiles.push_back(info);
          }
        }
        return startOrdinal;
...
#ifdef BORLAND_LD
         previousOrdinal = loadGRP_File(info, true, previousOrdinal);
#else
...
#else					
                                        if ( _getcwd(expandedPath, MAXPATHLEN) == NULL )
#endif
...
#ifdef BORLAND_LD
      else if (strcmp(argv[i], "-bequiet") == 0) {
        fBeQuiet = true;
      }
#endif
			else {
			...
			if ( strcmp(&info.path[strlen(info.path)-2], ".a") == 0 )
				...
				fInputFiles.push_back(info);
#else
        loadGRP_File(info, false, ld::File::Ordinal::makeArgOrdinal((uint16_t)i));
#endif
		}
	}
...
#else
		// Support ';' separated list of paths
		while (const char* sc = strchr(path, ';')) {
			char subPath[PATH_MAX];
			strncpy(subPath, path, sc - path);
			subPath[sc - path] = '\0';
			fSDKPaths.push_back(strdup(subPath));
			path = sc + 1;
		}
		fSDKPaths.push_back(path);
#endif
...
#else
			// look through the entire list until we find something with a 'sdk' and a number
			for (int sdkIdx = fSDKPaths.size() - 1; sdkIdx >= 0; sdkIdx--) {
				const char* sdkPath = fSDKPaths[sdkIdx];
				if (strcasestr(sdkPath, "sdk")) {
					const char* end = &sdkPath[strlen(sdkPath) - 1];
					while ((end > sdkPath) && !isdigit(*end))
						--end;
					if (end > sdkPath) {
...
#else
   //fprintf(stderr, "Crash report not supported!!!\n");
#endif
}
