...
#else
		const char* lastSlash = strrchr(_options.outputFilePath(), PATH_SEP);
                if (lastSlash == NULL)
		  lastSlash = strrchr(_options.outputFilePath(), OTHER_PATH_SEP);
#endif
		if ( lastSlash !=  NULL ) {
...
void OutputFile::writeOutputFile(ld::Internal& state)
{
   ...
   #define kIsTranslated  0x4000000000000000ULL
...
#else
		char* end = strrchr(dirPath, PATH_SEP);
		if (end == NULL) 
			end = strrchr(dirPath, OTHER_PATH_SEP);
#endif
		if ( end != NULL )
			end[1] = '\0';
		else
	...
	if ( outputIsRegularFile && outputIsMappableFile ) {
#ifdef BORLAND_LD
		// EMBT_FIXME: Windows alternative?
#else
...
#else
			fd = open(_options.outputFilePath(),  O_WRONLY, 0);
#endif
		if ( fd == -1 ) 
...
#ifdef BORLAND_LD
		// EMBT_FIXME: SetFilePointer/SetFileEnd ?
#else
...
#else
		const char* lastNameSlash = strrchr(_options.installPath(), PATH_SEP);
#endif
...
const char* OutputFile::assureFullPath(const char* path)
{
	...
	if ( path[0] == '/' )
		return path;
	char cwdbuff[MAXPATHLEN];
	if ( _getcwd(cwdbuff, MAXPATHLEN) != NULL ) {
		char* result;
		asprintf(&result, "%s%c%s", cwdbuff, PATH_SEP, path);
		if ( result != NULL )
			return result;
	}
#endif
		...
		if ( const char* newTUPath = atom->translationUnitSource() ) {
			...
			if ( (curTUPath == nullptr) || (strcmp(curTUPath,newTUPath) != 0) ) {
				...
				while (true) {
					...
					if ( c == '/' )
#else
					if (( c == PATH_SEP ) || ( c == OTHER_PATH_SEP ))
#endif
						lastSlash = p;
					*p++ = c;
					if ( c == '\0' )
