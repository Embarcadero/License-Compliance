              ...
              const struct demangle_component *dc)
{
  ...
  switch (dc->type)
    ...
    case DEMANGLE_COMPONENT_LOCAL_NAME:
      ...
      if (((dpi->options & DMGL_JAVA) || (dpi->options & DMGL_DELPHI)) == 0)
	d_append_string (dpi, "::");
      else
	d_append_char (dpi, '.');
                     ...
                     demangle_callbackref callback, void *opaque)
{
  ...
  struct demangle_component *dc;
  int status;
  int options_save = options;

  if ((options & DMGL_DELPHI) != 0)
    {
      if (mangled[0] == '_' && mangled[1] == 'Z'
	  && mangled[2] == 'T' && mangled[3] == 'I')
	{
	  type = DCT_TYPE;
	  cplus_demangle_init_info (mangled+2, DMGL_DELPHI | DMGL_TYPES,
	      			    strlen (mangled+2), &di);
	}
      else if ((options & DMGL_TYPES) != 0)
	{
	  type = DCT_TYPE;
	  cplus_demangle_init_info (mangled, DMGL_DELPHI | DMGL_TYPES,
	      			    strlen (mangled), &di);	
	}
      else
	{
	  type = DCT_MANGLED;
	  cplus_demangle_init_info (mangled, DMGL_DELPHI | DMGL_PARAMS,
	      			    strlen (mangled), &di);	
	}
    }
  else if ((mangled[0] == '_' && mangled[1] == 'Z')
      	   || (strncmp(mangled, "__dyld_", 7) == 0) )
    {
      type = DCT_MANGLED;
      /* DYLD func ?  */	
      if (strncmp(mangled, "__dyld_", 7) == 0) 
	cplus_demangle_init_info (mangled+7, options, strlen (mangled+7), &di);		
      else
