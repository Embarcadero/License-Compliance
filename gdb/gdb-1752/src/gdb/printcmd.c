...
static void print_formatted (struct value *, int, 
  ...
  *string_ptr = p;
  ...
  if (val.format == '?')
    ...
    switch (val.format)
      {
      case 'a':
#if 0 // EMBARCADERO merge with 536fc59e543c616009a57fcd5a931f09d000f8d9
      case 's':
...
// EMBARCADERO FIXME: textual types
  if (size)
    {
			...
			int size, struct ui_file *stream)
{
  ...
  if (len > sizeof(LONGEST) &&
	...
	case 'x':
	  ...
	  return;
	case 'c':
	  print_char_chars (stream, type, valaddr, len);
	  return;
	default:
	  break;
	};
  ...
  if (options->format != 'f' && options->format != 'A')
			...
			char *leadin)
{
  ...
  /* EMBARCADERO Local - force demangling */
  /* was: if (build_address_symbolic (addr, do_demangle, &name, &offset,
                                      &filename, &line, &unmapped)) */
  if (build_address_symbolic (addr, 1, &name, &offset, &filename, &line, &unmapped))
    {
...
#if 0 /* APPLE LOCAL: permit 'i' to have custom size.  */
...
printf_command (char *arg, int from_tty)
{
  ...
  *f++ = '\0';
...

  {
    ...
    enum argclass
      {
	no_arg, int_arg, long_long_arg, pointer_arg,
