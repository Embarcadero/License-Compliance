...
value_coerce_string (struct value *arg)
{
  ...
  int c_delphi_str = c_delphi_string_type(tarray);
  if ((current_language->la_language == language_pascal
	...
	error (_("Can't dereference uninitialized string"));
	 ...
	 FIXME: On Windows, this would be wchar_t. */                                                                                                                            
      if (c_delphi_str && stringcharwidth == 2)                                                 
	    chartype = builtin_type(current_gdbarch)->builtin_char16; 
      if (stringcharwidth != TYPE_LENGTH (chartype))
	error (_("Unknown charcter type for string"));

      /* Turn the string into an array of chars starting at the dereferenced
	  address.  */
      lowerbound = current_language->string_lower_bound;
      ...
      VALUE_ADDRESS (retval) = stringaddr;
	  ...
	  if (j > 0 && args == 0)
		...
		if (!typecmp (TYPE_FN_FIELD_STATIC_P (f, j),
			      ...
			      TYPE_FN_FIELD_ARGS (f, j), args))
		  {
		    ...
		    if (TYPE_FN_FIELD_STATIC_P (f, j) && memfuncflagsp)
...
value_struct_elt (struct value **argp, struct value **args,
		  char *name, int *memfuncflagsp, char *err)
{
  ...
  *argp = coerce_array (*argp);
  ...
  if (!args)
	...
	error (_("Cannot get value of destructor"));
      ...
      v = search_struct_method (name, argp, args, 0, memfuncflagsp, t);
      if (v)
        {
          if (v == (struct value *) - 1)
...
find_overload_match (struct type **arg_types, int nargs, char *name, int method,
		     ...
		     struct value **valp, struct symbol **symp, int *memflagp)
{
  ...
  if (method)
    {
      if (memflagp != NULL)
	*memflagp = oload_method_flag (method, fns_ptr, oload_champ);
				 ...
				 int *oload_champ)
{
  ...
  *oload_champ_bv = NULL;
  ...
  if (new_oload_champ != -1
    ...
    {
      /* EMBARCADERO Local: Error if namespace not found.  */
      if (new_oload_champ == -1)
        error (_("overloaded function '%s': namespace not found"), func_name);
      *oload_syms = new_oload_syms;
      *oload_champ = new_oload_champ;
      *oload_champ_bv = new_oload_champ_bv;
      discard_cleanups (old_cleanups);
				...
				enum noside noside)
{
  ...
  for (i = TYPE_NPROP_FIELDS (t) - 1; i >= 0; i--)
      ...
      if (t_field_name && (strcmp_iw (t_field_name, name) == 0))
	{
	  ...
	  /* EMBARCADERO FIXME: writing not supported  */
	  int lval_needed = 0;

	  /* Are we assigning? E.g. "s.propname = val"  */
	  if (lval_needed)
	    {
	      char *setter_name = PROP_FIELD_WRITE_NAME (*f);
	      if (setter_name)
