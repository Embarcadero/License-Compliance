...
/* EMBARCADERO Local: fCall wrapper bkpts handling */
#include "infcall.h"
...
/* EMBARCADERO Local: Begin */
struct breakpoint *
find_breakpoint_by_location_addr (CORE_ADDR memaddr)
{ 

  struct bp_location *bloc;
  struct bp_location *temp_bloc;

  ALL_BP_LOCATIONS_SAFE (bloc, temp_bloc)
    {
      if (bloc->address == memaddr)
        return bloc->owner; 
    }
  return NULL;
}
...
void
find_and_delete_breakpoints_by_line_and_file_name (int line, char *file)
{ 
  struct breakpoint *b;
  struct breakpoint *b_temp;

  ALL_BREAKPOINTS_SAFE (b, b_temp)
    {
      if ((b->line_number == line)
          && (b->source_file)
          && (!strcmp(b->source_file, file)))
	delete_breakpoint (b);
    }
}
  ...
  ALL_BLOCK_SYMBOLS (block, iter, sym)
      ...
      if (print_me)
	{
	  ...
	  
	  {
		...
		/* KIRILL FIXME: anyway to clean this up?  */
		/* kind="802821656",name="nullpointerexception",object="0x112313",addr="0x1234",handle="0x2q132" */
//             NotificationKind="0x2fd00840",ExceptionObject="0x2fd0083c",ExceptionName="0x2fd00838",ExceptionLocation="0x2fd00834",HandlerAddr="0x2fd00830"
//		 	int errcode = target_read_memory (VALUE_ADDRESS(val), (char*)&argumentvalue, 4);

/*
0x000096a5:         TAG_formal_parameter [29]  
0x000096a6:          AT_name( .debug_str[0x0000be24] = "NotificationKind" )
0x000096aa:          AT_decl_file( 0x01 ( "C:/BDS/runtime/rtl/sys/System.pas" ) )
0x000096ab:          AT_decl_line( 0x435a ( 17242 ) )
0x000096ad:          AT_type( cu + 0x00000022 => {0x00000b7b} ( int32 ) )
0x000096b1:          AT_location( <0x2> 7d 10  ( breg13 +16 ) ) R0

0x000096b4:         TAG_formal_parameter [29]  
0x000096b5:          AT_name( .debug_str[0x0000be35] = "ExceptionObject" )
0x000096b9:          AT_decl_file( 0x01 ( "C:/BDS/runtime/rtl/sys/System.pas" ) )
0x000096ba:          AT_decl_line( 0x435b ( 17243 ) )
0x000096bc:          AT_type( cu + 0x00000047 => {0x00000ba0} ( uint8* ) )
0x000096c0:          AT_location( <0x2> 7d 0c  ( breg13 +12 ) ) R1

0x000096c3:         TAG_formal_parameter [29]  
0x000096c4:          AT_name( .debug_str[0x0000be45] = "ExceptionName" )
0x000096c8:          AT_decl_file( 0x01 ( "C:/BDS/runtime/rtl/sys/System.pas" ) )
0x000096c9:          AT_decl_line( 0x435c ( 17244 ) )
0x000096cb:          AT_type( cu + 0x000005c1 => {0x0000111a} ( N6System8AnsiCharE[]* ) )
0x000096cf:          AT_location( <0x2> 7d 08  ( breg13 +8 ) ) R2

0x000096d2:         TAG_formal_parameter [29]  
0x000096d3:          AT_name( .debug_str[0x0000be53] = "ExceptionLocation" )
0x000096d7:          AT_decl_file( 0x01 ( "C:/BDS/runtime/rtl/sys/System.pas" ) )
0x000096d8:          AT_decl_line( 0x435d ( 17245 ) )
0x000096da:          AT_type( cu + 0x00000047 => {0x00000ba0} ( uint8* ) )
0x000096de:          AT_location( <0x2> 7d 04  ( breg13 +4 ) ) R3

0x000096e1:         TAG_formal_parameter [29]  
0x000096e2:          AT_name( .debug_str[0x0000be65] = "HandlerAddr" )
0x000096e6:          AT_decl_file( 0x01 ( "C:/BDS/runtime/rtl/sys/System.pas" ) )
0x000096e7:          AT_decl_line( 0x435e ( 17246 ) )
0x000096e9:          AT_type( cu + 0x00000047 => {0x00000ba0} ( uint8* ) )
0x000096ed:          AT_location( <0x2> 7d 00  ( breg13 +0 ) ) R9
*/

		if (!strcmp (SYMBOL_PRINT_NAME (sym), "NotificationKind"))
		  {
		    unsigned argumentvalue = read_register (0); /* ARM_R0_REGNUM */
		    ui_out_field_int (uiout, "kind", argumentvalue);
		  }
		else if (!strcmp (SYMBOL_PRINT_NAME (sym), "ExceptionObject"))
		  {
		    CORE_ADDR argumentvalue = read_register (1);
		    ui_out_field_core_addr (uiout, "object", argumentvalue);
		  }
		else if (!strcmp (SYMBOL_PRINT_NAME (sym), "ExceptionName"))
		  {
		    CORE_ADDR argumentvalue;
		    unsigned char namelen = 0;
		    char buf[257];
		    int errcode = 0; 
		    argumentvalue = read_register (2);
		    memset (buf, 0, sizeof (buf));
		    /* CPP Exception name is just NULL terminated string 
		       but name length restricted 256 bytes max.  */
		    if (!isdelphi)
		      {
			errcode = target_read_memory (((char*)argumentvalue),
						      (char*)&buf[0], sizeof(buf)); 
		      } 
		    else
		      {
			/* Delphi Exception name has Pascal ShortStr type, 
			   first byte is length - 256 bytes max.  */
			errcode = target_read_memory (((char*)argumentvalue),
						      (char*)&namelen, 1);
			errcode = target_read_memory (((char*)argumentvalue + 1),
						      (char*)&buf[0], namelen);
		      }
		    if (!errcode)
		      ui_out_field_string (uiout, "name", &buf[0]);
		    else
		      ui_out_field_string (uiout, "name", "<error>");
		  }
		else if (!strcmp (SYMBOL_PRINT_NAME (sym), "ExceptionLocation"))
		  {
		    CORE_ADDR argumentvalue = read_register (3);
		    ui_out_field_core_addr (uiout, "addr", argumentvalue);
		  }
		else if (!strcmp (SYMBOL_PRINT_NAME (sym), "HandlerAddr"))
		  {
		    CORE_ADDR argumentvalue = read_register (9);
		    ui_out_field_core_addr (uiout, "handler", argumentvalue);
		  }
		else 
		  {
		    CORE_ADDR argumentvalue = 0;
		    int errcode = target_read_memory (VALUE_ADDRESS(val),
						      (char*)&argumentvalue, 4);
		    if (!errcode)
		      ui_out_field_core_addr (uiout, "unknown argument", argumentvalue);
		    else	
		      ui_out_field_string (uiout, "unknown argument", "<error>");
		  }
		break;
	      default:
...
print_it_typical (bpstat bs)
{
  ...
  switch (bs->breakpoint_at->type)
	...
	breakpoint_adjustment_warning (bs->breakpoint_at->loc->requested_address,
      ...
      if (strcmp (bs->breakpoint_at->addr_string, "System._DbgExcNotify") && 
          strcmp (bs->breakpoint_at->addr_string, "_CPPDbgExcNotify"))	  
	ui_out_print_annotation_string (uiout, 0, "reason",
	    				async_reason_lookup (EXEC_ASYNC_BREAKPOINT_HIT));
      else
        {
	  /* EMBARCADERO Local: Special handling for RTL exception
	     notifications.  */
	  ...
	  ui_out_print_annotation_string (uiout, 0, "reason",
		  			  async_reason_lookup (EXEC_ASYNC_EXCEPTION_RAISED));
...
//	  if (!block)
	    {
	      /* Syms not found, safe parameters print failed, 
		 just print parameters in fixed order */
		...
		/* CPP Exception name is just NULL terminated string 
		   but name length restricted 256 bytes max */
		if (!strcmp (bs->breakpoint_at->addr_string, "_CPPDbgExcNotify"))
		  {
		    errcode = target_read_memory (((char*)argumentvalue),
						  (char*)&buf[0], sizeof(buf)); 
		  } 
		...
		ui_out_field_core_addr (uiout, "addr", argumentvalue);
	      ...
	      {
		int errcode = 0; 
	        CORE_ADDR argumentvalue, stack_ptr;

	        /* HandlerAddr = *(dword *)(ARM_SP_REGNUM (13)) at *ExcNotify start */
		stack_ptr = read_register (13);
	        errcode = target_read_memory (((char*)stack_ptr),
						  (char*)&argumentvalue, sizeof(CORE_ADDR));
	        /* SP must have some valid address always, 
	           but, just in case, who knows for sure ? */
		if (!errcode)
		  ui_out_field_core_addr (uiout, "handler", argumentvalue);
		else
		  ui_out_field_core_addr (uiout, "handler", 0xDEADBEEF);
	      }         
	    }
	} /* exception-raised */
...
#undef shlr
  ...
  for (; bs != NULL; bs = bs->next)
	...
	case bp_finish:
	  if (file_log)
	    fprintf_unfiltered (file_log,
				"bpstat_what: stopped address: %s\n", 
				paddr (bs->breakpoint_at->loc->address));
	  if (bs->stop)
	    {
	      if (bs->print)
		bs_class = bp_noisy;
	...
	case bp_call_dummy:
	  ...
	  bs_class = bp_silent;
	  /* EMBARCADERO Local: begin fCall wrapper internal breakpoints */
	  if (fcw_breakpoint_hit (bs->breakpoint_at,
				  "fCallWrapperReadyToArgPush"))
	    {
	      current_action = BPSTAT_WHAT_SINGLE_FCALL;
	      /* Push user's function arguments, if FCW is supported by current
	         arch.  */
	      gdbarch_fcw_argument_push (current_gdbarch);
	    }
	  else if (fcw_breakpoint_hit (bs->breakpoint_at,
				       "fCallWrapperCatch"))
	    {
	      current_action = BPSTAT_WHAT_SINGLE_FCALL;
	      /* Record that the function call wrapper caught an
	         exception while evaluating the user's function call.  */
	      set_fcw_exception_raised ();
	    }
	  else if (fcw_breakpoint_hit (bs->breakpoint_at,
				       "fCallWrapperGetResult"))
	    {
	      current_action = BPSTAT_WHAT_SINGLE_FCALL;
	      /* Save results, if fCall succeeded.  */
	      if (!fcw_exception_raised ())
		{
		  ...
		  if (proceed_to_finish)
		    {
		      /* NB: The copy goes through to the target picking up
		         the value of all the registers.  */
		      /* KIRILL FIXME: what about struct return?  */
	  ...
	  /* EMBARCADERO Local: end fCall wrapper internal breakpoints */
	    {
	      retval.call_dummy = STOP_STACK_DUMMY;
	      if (file_log)
		fprintf_unfiltered (file_log,
				    "bpstat_what: stopped at call dummy: %s\n", 
				    paddr (bs->breakpoint_at->loc->address));
	    }
	  break;
...
/* EMBARCADERO Local: Set DbgExcNotify hook bkpt */
extern unsigned short DbgExcNotify_line;

int
set_dbgexcnotify (void)
{
  struct breakpoint *b;
  ...
  char *addr_start = "System._DbgExcNotify";
  char *addr_start_cpp = "_CPPDbgExcNotify";
  char *addr_end = addr_start;
  char *addr_end_cpp = addr_start_cpp;
  char buf[256];
  char *fakeline[2];
  struct symtab **file_symtab_arr = NULL;
  struct minimal_symbol *hook_sym;

  /* Delphi exception notification hook.  */
  hook_sym = lookup_minimal_symbol ("@DbgExcNotify", 0, 0);
  if (hook_sym == NULL)
    {
      if (file_log)
        fprintf_unfiltered (file_log,
			    "@DbgExcNotify minsym info not found, Delphi exception notifications disabled\n");
    }
  else
    {
      sprintf (&buf[0], "'@DbgExcNotify'"); /* Using export symbol */
      
      fakeline[0] = &buf[0];
      fakeline[1] = NULL;
      
      sals = decode_line_1 (&fakeline[0], 1, (struct symtab *) NULL, 
			    0, &canonical, NULL, 0);
	...
	discard_cleanups (canonical_strings_chain);
      ...
      sals.sals[0].pc = SYMBOL_VALUE_ADDRESS(hook_sym);
      sals.sals[0].symtab = NULL;
      
      b = set_raw_breakpoint (sals.sals[0], bp_breakpoint, 0);
      set_breakpoint_count (breakpoint_count + 1);
      b->number = breakpoint_count;
      b->cond = NULL;
      b->cond_string = NULL; 
      b->thread = thread;
      sprintf (&buf[0], "System._DbgExcNotify");
      addr_start = &buf[0];
      addr_end = (char*)((char*)buf + strlen(buf) + 1);
  ...
  /* CPP exception notification hook.  */
  hook_sym = lookup_minimal_symbol ("_CPPDbgExcNotify", 0, 0);
  if (hook_sym == NULL)
    {
      if (file_log)
        fprintf_unfiltered (file_log,
			    "_CPPDbgExcNotify minsym info not found, CPP exception notifications disabled\n");
      return 0;
    }
  else
    {
      sprintf (&buf[0], "'_CPPDbgExcNotify'"); /* Using export symbol */
	...
	discard_cleanups (canonical_strings_chain);
      ...
      b->cond_string = NULL; 
      b->thread = thread;
      sprintf (&buf[0], "_CPPDbgExcNotify");
      addr_start_cpp = &buf[0];
      addr_end_cpp = (char*)((char*)buf + strlen(buf) + 1);
      
      if (canonical != (char **) NULL && canonical[0] != NULL)
	b->addr_string = canonical[0];
      else if (addr_start_cpp)
	b->addr_string = savestring (addr_start_cpp, addr_end_cpp - addr_start_cpp);
...
disable_user_breakpoints_before_operation (void)
{
  ...
  ALL_BREAKPOINTS (b)
  {
    ...
    if (is_objc_exception_throw_breakpoint (b))
     ...
     /* EMBARCADERO Local: we should keep breakpoint on 
        exception hook enabled always */
    if ((b->type == bp_breakpoint
	 || b->type == bp_inlined_breakpoint)
	&& breakpoint_enabled (b))
	// && strcmp (b->addr_string, "System._DbgExcNotify")) 
      {
	b->enable_state = bp_hand_call_disabled;
	check_duplicates (b);
      }
  ...
  if ((*address) == NULL
	...
	restrict_cleanup = make_cleanup (null_cleanup, NULL);
	  ...
	  && (!cursal.symtab
 		  ...
 		  && ((*address)[1] != '['))))
	/* APPLE LOCAL begin return multiple symbols */
	/* EMBARCADERO Local: line spanning multiple basic blocks */
	/* Pass find_all_occurrences==1, so we'll set breakpoints on all
	   addresses which map to this line. */
