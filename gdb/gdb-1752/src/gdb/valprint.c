...
{
  /* APPLE LOCAL auto endianness */
  print_char_chars_with_byte_order (stream, type, valaddr, len,
				    BFD_ENDIAN_UNKNOWN);
}

void
print_char_chars_with_byte_order (struct ui_file *stream, struct type *type,
				  const gdb_byte *valaddr,
				  unsigned int len, int byte_order)
{
  const gdb_byte *p;
			  ...
			  unsigned int i)
{
  ...
  for (; i < len && things_printed < options->print_max; i++)
	...
	{
	  val_print (elttype, val_elt_addr (type, valaddr, i), 0, 0, stream,
  ...
  if (len > 0)
    ...
    if (fetchlimit > opts.print_max)
	  ...
	  /* Scan this chunk for the null character that terminates the string
	     ...
	     after the null character, or at the next character after the end of
	     the buffer. */
