...
demangled_name_to_comp (const char *demangled_name,
		        void **memory_p, const char **errmsg)
{
  if (current_language->la_language == language_pascal)
    return p_demangled_name_to_comp (demangled_name, memory_p, errmsg);
  else
    return cp_demangled_name_to_comp (demangled_name, memory_p, errmsg);
}

char *
comp_to_string (struct demangle_component *result,
		int estimated_len)
{
  if (current_language->la_language == language_pascal)
    return p_comp_to_string (result, estimated_len);
  else
    return cp_comp_to_string (result, estimated_len);
}
...
cp_canonicalize_string (const char *string)
{
  ...
  ret_comp = demangled_name_to_comp (string, &storage, NULL);
  if (ret_comp == NULL)
    return NULL;

  ret = comp_to_string (ret_comp, len);
		      ...
		      void **memory, char **demangled_p)
{
  ...
  ret = demangled_name_to_comp (demangled_name, memory, NULL);
  ...
  if (cur_comp != NULL && prev_comp != NULL)
    {
      ...
      *prev_comp = *d_left (prev_comp);
      ...
      ret = comp_to_string (ret_comp, 10);
    }
...
cp_func_name (const char *full_name)
{
  ...
  ret_comp = demangled_name_to_comp (full_name, &storage, NULL);
  if (!ret_comp)
    return NULL;
...
cp_remove_params (const char *demangled_name)
{
  ...
  ret_comp = demangled_name_to_comp (demangled_name, &storage, NULL);
  if (ret_comp == NULL)
    return NULL;
...
#if 0
      /* EMBARCADERO LOCAL begin Delphi */
      /* Delphi can have procedures with no params - they will come
	 through as DEMANGLE_COMPONENT_QUAL_NAME.
	 FIXME: fix p-name-parser.y to lookup if function.  */
      case DEMANGLE_COMPONENT_QUAL_NAME:
	if (current_language->la_language == language_pascal)
	  {
	    ret_comp = d_right (ret_comp);
  ...
  if (ret_comp->type == DEMANGLE_COMPONENT_TYPED_NAME)
    ret = comp_to_string (d_left (ret_comp), 10);
#if 0
  /* EMBARCADERO LOCAL begin Delphi */
  /* Delphi can have procedures with no params - they will come
      through as DEMANGLE_COMPONENT_QUAL_NAME
      with left=namespace and right=function.
      FIXME: fix p-name-parser.y to lookup if function.  */
  if (current_language->la_language == language_pascal
      && ret_comp->type == DEMANGLE_COMPONENT_QUAL_NAME)
    {
      ret_comp = d_right (ret_comp);
      if (ret_comp->type == DEMANGLE_COMPONENT_NAME)
	ret = comp_to_string (ret_comp, 10);
    }
  /* EMBARCADERO LOCAL end Delphi */
#endif
				 ...
				 const char *namespace)
{
  ...
  /* EMBARCADERO LOCAL begin Delphi */
	...
	= alloca (strlen (namespace) + 1 + strlen (func_name) + 1);
