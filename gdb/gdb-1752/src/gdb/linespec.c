...
struct symtab **symtab_from_filename (char **argptr,
        ...
        for (i = 0; file_symtab_arr[i] != NULL; i++)
          {
            ...
            this_result = decode_all_digits_exhaustive (&start_here, 
			      ...
			      not_found_ptr, find_all_occurrences);
...
/* EMBARCADERO Local: Lines in prolog */
/* This define is a workaround for the lack of CFI instructions in the
   .debug_frame section on ARM targets by not allowing breakpoints to be
   set in the function prolog.

   This supercedes Apple's fix for the same issue.  Apple's fix had the
   problem that if a user set a breakpoint at the function declaration,
   the actual breakpoint would be set at the next line following the prolog.
   When the BP was hit, the BP id returned to the IDE was for the requested
   line rather than the actual line, so the user would think they were at
   the start of the function instead of the first line.  The user would then
   try to step to get to the first line, but actually step over the first
   line.  */
#define WORKAROUND_FOR_PROLOG_STEPPING_BUG 1
...

{
  ...
  *argptr = q;
  ...
  do_with_best:
    ...
    if (lineno != 0)
      {
		...
		/* If we have a line which spans more than one code
		   range, then this will cause us to skip all but the first
		   code range.  If those ranges aren;t in the same basic block,
		   then we could miss a breakpoint.
		   FIXME: I'm guessing the call to blockvector_for_pc_sect()
		   above is supposed to return a different index in that case.
		   Changing blockvector_for_pc_sect() seems risky, so
		   we just add a test for find_all_occurrences here.  */
	...
	if (best > 0)
	  {
	    /* EMBARCADERO Local: Exact match only */
#if 0
	    lineno = best;
	    goto do_with_best;
#else
	...
	for (i = 0; i < values.nelts; i++)
	  {
	      ...
	      if (e.reason != NO_ERROR)
		{
		 ...
		 and then you don't want to move the pc.  */
		  ...
		  &&val->line <= sal.line)
#if defined(WORKAROUND_FOR_PROLOG_STEPPING_BUG)
		/* EMBARCADERO Local begin Lines in prolog */
		{
		  /* Don't set BPs in prolog.  */
		  if (val->line < sal.line)
		    error (_("Line %d in file \"%s\" is in function prolog."),
			   val->line, val->symtab->filename);
		  val->pc = sal.pc;
		}
		/* EMBARCADERO Local end lines in prolog */
#else
                val->pc = sal.pc;
#endif
            }
...

{
  ...
  *argptr = q;
  ...
  /* EMBARCADERO Local Lines in prolog */
  /* This code was disabled because it caused the debugger to lie about
     the address of a line in commands like "info line".  */
#if !defined(WORKAROUND_FOR_PROLOG_STEPPING_BUG)
  ...
  /* APPLE LOCAL end function first line */
#endif
