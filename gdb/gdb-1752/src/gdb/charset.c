...
/* EMBARCADERO LOCAL: Workaround for iconv infinite loop bug 5277:
   https://sourceware.org/bugzilla/show_bug.cgi?id=5277.  */
#define WORKAROUND_FOR_GLIBC_5277
#ifdef WORKAROUND_FOR_GLIBC_5277
/* Maximum chars for the encoding.  */
...
validate (void)
{
  ...
  /* EMBARCADERO LOCAL: begin UTF support */
  /* If "wchar_t" is used as the intermediate encoding, iconv will use the
     setting of LC_CTYPE for its string conversions.  If we want to use UTF-8,
     we need to set the locale so iconv will work correctly.  */
  if (strcmp (INTERMEDIATE_ENCODING, "wchar_t") == 0
      && strcmp (host_cset, "UTF-8") == 0)
    {
      setlocale (LC_CTYPE, "UTF-8");
    }
  /* EMBARCADERO LOCAL: end UTF support */
#endif
#endif
}
...
#ifdef WORKAROUND_FOR_GLIBC_5277
  /* The the encoding of the input.  */
  char encoding[MAX_ENCODING_SIZE];
#endif
};
...
#ifdef WORKAROUND_FOR_GLIBC_5277
  strncpy (result->encoding, charset, MAX_ENCODING_SIZE);
#endif
  return result;
}
...
/* EMBARCADERO LOCAL: begin UTF support */
/* If a UTF LE encoding is used, return the number of bytes belonging to
   the next character in PTR, else return 0.  */
size_t
get_nbytes_in_wchar (const char *encoding, const gdb_byte *ptr)
{
  size_t nbytes_in_char = 0;
  if (strcmp (encoding, "UTF-8") == 0)
    {
      gdb_byte byte = ptr[0];
      gdb_byte nibl = ptr[0] & 0xf0;

      if ((byte & 0x80) == 0)
	/* 7-bit ASCII.  */
	nbytes_in_char = 1;
      else if (nibl == 0xc0 || nibl == 0xd0)
	nbytes_in_char = 2;
      else if (nibl == 0xe0)
	nbytes_in_char = 3;
      else if (nibl == 0xF0)
...
#ifdef WORKAROUND_FOR_GLIBC_5277
      /* If input is Unicode encoding, make sure we always have enough bytes
         in the input buffer to hold the Unicode character to avoid infinite
         loops in OSX's old, buggy iconv.  */
      size_t nbytes_in_char = get_nbytes_in_wchar (iter->encoding, iter->input);
      if (nbytes_in_char > 0
	  && iter->bytes < nbytes_in_char)
	{
	  /* There aren't enough bytes in the input buffer for the full
	     Unicode character, so it must be invalid.  Just return error.  */
	  ...
	  *len = iter->width;
      ...
      if (nbytes_in_char > 0)
	{
	  /* We know how many bytes are in the Unicode char; pass only those
	     bytes to iconv.  */
	  char uchar[32];
	  const char *uchar_ptr = &uchar[0];
	  size_t nbytes_in_input, orig_nbytes_in_input;
	  size_t nbytes_converted;

	  /* Account for max bytes for a unicode char (= 4), and NULL.  */
	  /* EMBARCADERO LOCAL FIXME: Assumes byte order matches host.  */
	  orig_nbytes_in_input
	    = nbytes_in_input = nbytes_in_char + 4 + iter->width;
	  gdb_assert (nbytes_in_input < 32);
	  memcpy (&uchar[0], iter->input, nbytes_in_char);
	  memset (&uchar[nbytes_in_char], 0, 4 + iter->width);

	  r = iconv (iter->desc,
		     (ICONV_CONST char **) &uchar_ptr, &nbytes_in_input,
		     &outptr, &out_avail);
	  /* Advance our buffer as if iconv had parsed it.  */
	  nbytes_converted = orig_nbytes_in_input - nbytes_in_input;
	  iter->input += nbytes_converted;
	  iter->bytes -= nbytes_converted;
	}
      else
#endif
	{
...
find_charset_names (void)
{
  ...
  if (!pex_run (child, PEX_SEARCH | PEX_STDERR_TO_STDOUT | PEX_USE_PIPES, "iconv",
		args, NULL, NULL, &err))
    {
