...
/* DAWN FIXME: can clean this up so it's not iOS ARM specific.  */
/* -- CPSR register hack for iOS 7 --

   The global cpsr_reg is used in this CPSR register hack for iOS 7.

    * cpsr_reg - Holds the value of the CPSR register.
    Used in the following:
    - initialized in data to -1 before gdb starts
    - regcache.c (read_register):
	if regnum == 25 and cpsr_reg != -1,
	    return cpsr_reg
    - remote.c (remote_wait):
	if the remote packet reg says it's pnum == 16,
	    actually read reg 25 instead and init cpsr_reg = r25
    - infcall.c (hand_function_call):
	cpsr_reg's value is saved at the begining
	and restored after the function is called
	in case it is changed during the call.  */

static ULONGEST cpsr_reg = -1;

static void
init_register_CPSR_HACK ()
{
  cpsr_reg = -1;
}

ULONGEST
get_register_CPSR_HACK ()
{
  return cpsr_reg;
}

void
set_register_CPSR_HACK (LONGEST val)
{
  cpsr_reg = val;
}

/* A variant that can be registered with make_cleanup().
   FIXME assumes sizeof LONGEST <= sizeof void*.  */
static void
set_register_CPSR_HACK_cleanup (void *val)
{
  gdb_assert (sizeof (LONGEST) <= sizeof (void*));
  cpsr_reg = (LONGEST) val;
}

struct cleanup *
make_cleanup_set_restore_register_CPSR_HACK (void)
{
  struct cleanup *cleanup 
    = make_cleanup (set_register_CPSR_HACK_cleanup, (void *) cpsr_reg);
  return cleanup;
}
