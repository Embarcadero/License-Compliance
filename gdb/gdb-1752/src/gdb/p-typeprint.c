			...
			int level)
{
  ...
  if (show <= 0
      && TYPE_NAME (type) != NULL)
    {
      /* EMBARCADERO Local begin: Unmangle to check for Delphi types */
      ...
      if (tpname[0] == '_')
	{
      ...
      if (!type_printed)
      /* EMBARCADERO Local end: Unmangle to check for Delphi types */
	{
  ...
  switch (TYPE_CODE (type))
      ...
      if (TYPE_NAME (type) != NULL)
	{
	  ...
	  char *demangled = cplus_demangle (TYPE_NAME (type),
		  			    DMGL_DELPHI | DMGL_TYPES);
	  if (demangled != NULL)
	    {
	      fputs_filtered (demangled, stream);
	      xfree (demangled);
