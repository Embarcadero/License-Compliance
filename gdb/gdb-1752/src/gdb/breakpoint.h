...
enum bpstat_what_main_action
  {
    ...
    /* EMBARCADERO Local: fCall wrapper internal actions */
    BPSTAT_WHAT_SINGLE_FCALL,
...
enum stop_stack_kind
  {
    ...
    /* EMBARCADERO Local: Exception raised during fCall 
       left unhandled by debuggee. */
    /* Stopped at DbgExcNotify RTL exception hook function */
    STOP_EXCEPTION_HOOK
  };
...
struct breakpoint * find_breakpoint_by_location_addr (CORE_ADDR memaddr);
void find_and_delete_breakpoints_by_line_and_file_name (int line, char *file);
...
/* EMBARCADERO Local: DbgExcNotify bkpt installer function 
   Return 1, if hook bkpt was installed successfully.  */
extern int set_dbgexcnotify (void);

/* EMBARCADERO Local: Avaliable for modules other than breakpoint.c now */
extern struct breakpoint *set_raw_breakpoint (struct symtab_and_line,
					      enum bptype, int pending_p);
