	     ...
	     struct symtab_and_line sal)
{
  ...
  if (func)
      ...
      else
	{
	  ...
	  else if (funlang == language_pascal)
	    {
	      demangled = cplus_demangle (funname, DMGL_DELPHI);
	    }
	  else
	    demangled = NULL;
      ...
      if ((funlang == language_cplus) || (funlang == language_pascal))
        {
...
frame_info (char *addr_exp, int from_tty)
{
  ...
  if (func)
	...
	{
          /* EMBARCADERO FIXME: Should we copy what Apple does here? */
	  demangled = cplus_demangle (funname, DMGL_DELPHI);
	  if (demangled != NULL)
	    funname = demangled;
	}
