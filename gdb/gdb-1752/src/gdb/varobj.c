...
static struct language_specific
  ...
  ,
  /* C++ */
#if 0 /* EMBARCADERO LOCAL no fake children */
  {
   vlang_cplus,
	       ...
	       enum varobj_type type)
{
  ...
  if (expression != NULL)
      ...
      if (var->root->exp != NULL)
	{
	      ...
	      if (value_lazy (var->value))
               {
                 if (!gdb_value_fetch_lazy (var->value))
                   {
#if 1 /* EMBARCADERO LOCAL valobj errors */
		     /* EMBARCADERO LOCAL valobj errors */
		     warning ("Inaccessable value.");
		     /* Note: we have to unset the value because, if set,
		        do_free_variable_cleanup() will free it, but it has
		        already been added to the list of all_values which
			will be cleaned up at the start of the next command. */
                     var->value = NULL;
		     goto error_cleanup;
#else
                     var->value = NULL;
...
#if 1 /* EMBARCADERO LOCAL valobj errors */
	      /* EMBARCADERO LOCAL valobj errors */
	      warning ("Unable to evaluate expression.");
	      goto error_cleanup;
#else
	      int retval;
	  ...
	  /* Set language info */
	  /* TODO: EMBARCADERO Local: set var language depending on frame 
	     currently varobj lang set on expression lang.  */
...
varobj_update (struct varobj **varp, struct varobj_changelist **changelist)
{
  ...
  /* EMBARCADERO Local: Always re-evaluate array value or string contents 
     on update like new for current scope. No method for members values 
     changed detection if stack depth still same since last bkpt. */
  if ((TYPE_CODE ((*varp)->type) == TYPE_CODE_ARRAY)
      && ((*varp)->root->in_scope)
      && ((*varp)->join_in_expr) == VAROBJ_AS_ARRAY)
    {
      came_in_scope = 1;
    }
  else if ((TYPE_CODE ((*varp)->type) == TYPE_CODE_STRING)
      && ((*varp)->root->in_scope))
    {
      came_in_scope = 1;
    }
...
c_number_of_children (struct varobj *var)
{
  ...
  switch (TYPE_CODE (type))
      ...
      if (TYPE_DELPHI_DYNARRAY (type))
	{
	  ...
	  if (!var->value)
	    /* Is it possible to not have a value here? */
	    return -1;
	  addr = VALUE_ADDRESS (var->value);
	  ptrsize = TYPE_LENGTH (type);
	  if (!ptrsize)
	    ptrsize = 4;
	  addrmask = (ptrsize == 4) ? 0xffffffffUL : 0xffffffffffffffffULL;
	  /* Dereference to get the start of the array.  */
	  if (addr != 0)
	    {
	      errcode = target_read_memory (addr, (gdb_byte*)&dynarrstart, ptrsize);
	      dynarrstart &= addrmask;
	    }
	  /* The length of the dynamic array is sizeof(ptrdiff_t) bytes before
	     the start of the array.  */
	  if (errcode == 0 && dynarrstart != 0)
	    {
	      errcode = target_read_memory (dynarrstart - ptrsize, (gdb_byte*)&dynarrlen, ptrsize);
	      dynarrlen &= addrmask;
	    }
	  if (errcode != 0 || dynarrstart == 0)
	    {
	      if (errcode == EIO || dynarrstart == 0)
		/* error: Inaccesible value.  */
		children = -1;
	      else
		children = 0;
	    }
	    ...
	    {
	      /* EMBARCADERO LOCAL begin strings. Be sure that we always have 
	      limitation for strings equal to opts->print_max */
	      get_user_print_options (&opts);
	      children = min (opts.print_max, dynarrlen);
	      /* EMBARCADERO LOCAL end strings */
	    }
	  break;
	}
      /* else, fall thru */
...
c_make_name_of_child (struct varobj *parent, int index)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_ARRAY:
      {
        /*EMBARCADERO Local: Create name for array member 
        based on low bound attr set by compiler */
        struct type *index_type = TYPE_INDEX_TYPE (type);
        long low = 0;                                
        if ((index_type == NULL)
            || (TYPE_CODE (index_type) != TYPE_CODE_RANGE
                && TYPE_CODE (index_type) != TYPE_CODE_ENUM))
          {
             name = xstrprintf ("%d", index);
          } 
        else
          {
            low = TYPE_LOW_BOUND (index_type);
            if (low > TYPE_HIGH_BOUND (index_type))
              name = xstrprintf ("%d", index);
            else
              name = xstrprintf ("%d", low+index);
          }
      } 
      break;
    case TYPE_CODE_STRUCT:
    case TYPE_CODE_UNION:
      {
	if (TYPE_FIELD_NAME (type, index))
	  string = TYPE_FIELD_NAME (type, index);
        else
	  string = xstrdup ("unknown");
      }
	...
	case TYPE_CODE_UNION:
	  {
	    if (TYPE_FIELD_NAME (target, index))
...
c_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type)
{
  ...
  if (temp != NULL)
	...
	case TYPE_CODE_ARRAY:
	  {
	  /*EMBARCADERO Local: Adjust array member 
	    index to low bound attr set by compiler */
	    struct type *index_type = TYPE_INDEX_TYPE (type);
	    long low = 0;                                
	    int temp_index;
	    if ((index_type == NULL)
...
#else
	  indval = value_from_longest (builtin_type_int, (LONGEST) temp_index);
...
c_type_of_child (struct varobj *parent, int index)
{
  ...
  switch (TYPE_CODE (parent_type))
    ...
    case TYPE_CODE_PTR:
      /* EMBARCADERO LOCAL begin Delphi dynamic arrays.  */
      if (TYPE_DELPHI_DYNARRAY (check_typedef (parent_type)))
	{
	  type = check_typedef (get_target_type (parent_type));
	  break;
	}
      /* else, fall thru */
      /* EMBARCADERO LOCAL end Delphi dynamic arrays.  */
...
{
  /* The TYPE_CODE macro doesn't handle NULL types.  */
  if (get_type (var) == NULL)
    return 0;
...
/* EMBARCADERO LOCAL begin display contents */
/* Return 1 if we should display the contents of this varobj.  */
int should_display_contents (struct type *vartype)
{
  switch (TYPE_CODE (vartype))
	...
	/* Always display certain Delphi return types.  */
	if (TYPE_FLAGS (vartype) & TYPE_FLAG_DELPHI_RETURN)
	{
	  if (TYPE_NAME (vartype)
	      && (strcmp (TYPE_NAME (vartype), "System::Currency") == 0))
	  return 1;
	}
	/* Always display Delphi strings.  */
	if (TYPE_NAME (vartype)
	    && strcmp (TYPE_NAME (vartype), "System::UnicodeString") == 0)
	  return 1;
	/* EMBARCADERO LOCAL: begin expand aggregates in MI */
	/* Always display struct contents if so requested.  */
	get_user_print_options (&opts);
	if (opts.expand_aggregates)
	  return 1;
	/* EMBARCADERO LOCAL: end expand aggregates in MI */
	break;
      }
    /* EMBARCADERO LOCAL begin Delphi dynamic arrays.  */
    case TYPE_CODE_PTR:
      if (!TYPE_DELPHI_DYNARRAY (vartype))
	break;
      /* else fall thru */
    /* EMBARCADERO LOCAL end Delphi dynamic arrays.  */
    case TYPE_CODE_ARRAY:
	...
	/* Always display Delphi strings.  */
	struct type *elttype = get_target_type (vartype);
	if (elttype && TYPE_NAME (elttype)
	    && strcmp (TYPE_NAME (elttype), "N6System8WideCharE") == 0)
	  return 1;
	/* Always display C strings.  */
	get_user_print_options (&opts);
	if (elttype
	    && c_textual_element_type (elttype, opts.format))
	  return 1;
	/* EMBARCADERO LOCAL: begin expand aggregates in MI */
	/* Always display array contents if so requested.  */
	get_user_print_options (&opts);
	if (opts.expand_aggregates)
	  return 1;
	/* EMBARCADERO LOCAL: end expand aggregates in MI */
...
/* EMBARCADERO LOCAL end display contents */
...
c_value_of_variable (struct varobj *var)
{
  /* BOGUS: if val_print sees a struct/class, it will print out its
     children instead of "{...}" */
  struct type *vartype = get_type (var);

  /* EMBARCADERO LOCAL display contents */
  /* Strip top-level references.  */
  while (TYPE_CODE (vartype) == TYPE_CODE_REF)
    vartype = check_typedef (TYPE_TARGET_TYPE (vartype));

#if 0 /* EMBARCADERO FIXME: this causes us to lose information
	 such as the name of the original type, which may be needed
	 by should_display_contents().  */
  ...
  /* delphireturn structs with 1 field are empty wrappers around the
     ...
     these types.  */
  while (TYPE_DELPHI_RETURN (vartype)
	 && TYPE_CODE (vartype) == TYPE_CODE_STRUCT
	 && TYPE_NFIELDS (vartype) == 1)
    {
      if (TYPE_N_BASECLASSES (vartype) == 1)
	vartype = check_typedef (TYPE_BASECLASS (vartype, 0));
      else
	vartype = check_typedef (TYPE_FIELD_TYPE (vartype, 0));
    }
  ...
  switch (TYPE_CODE (vartype))
    ...
    case TYPE_CODE_UNION:
      {
	/* EMBARCADERO LOCAL begin display contents */
	/* See if we should display the contents of this varobj.  */
	if (should_display_contents (vartype))
	  goto do_eval;
	/* EMBARCADERO LOCAL end display contents */
	return xstrdup ("{...}");
	/* break; */
	...
	if (var->value == NULL)
	...
	else
	  {
	    ...
	    /* EMBARCADERO FIXME: not sure what change Apple meant here;
	       was it changing format to var->format?  */
	    get_formatted_print_options (&opts, format_code[(int) var->format]);
	    /* EMBARCADERO LOCAL: begin expand aggregates in MI */
	    /* If we're expanding aggregates, then we'll want to look
	       inside references.  */
	    if (opts.expand_aggregates)
	      opts.deref_ref = 1;
	    else
	    /* EMBARCADERO LOCAL: end expand aggregates in MI */
	      opts.deref_ref = 0;
	    common_val_print (var->value, stb, 0, &opts, current_language);
...
cplus_path_expr_of_child (struct varobj *parent, int index)
{
  ...
  switch (TYPE_CODE (type))
      ...
      if (CPLUS_FAKE_CHILD (parent))
	{
	  ...
	  if (TYPE_FIELD_NAME (type, index_in_type)) 
	    child_name = TYPE_FIELD_NAME (type, index_in_type);
	  else
	    child_name = xstrdup ("unknown");
...
#if 0	  /* KIRILL FIXME: removing this code this will break C++.  
	     I've Changed it to test for Pascal.  OK to enable?  */
	  /* was: if (is_ptr) */
	  if (is_ptr
	  ...
	  if (dynamic_expr_len > 0)
	    ...
	    {
#if 0	      /* KIRILL FIXME: do we want this for C++? */  
	      const char *format;
