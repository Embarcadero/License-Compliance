...
/* EMBARCADERO LOCAL Delphi strings
   If ARG is a string, convert it to an array.
...
extern struct value *coerce_string (struct value *value);
...
/* The flag passed to and/or returned from the following functions 
   can have the values defined below:
   value_struct_elt, value_struct_elt_as_lval, find_overload_match */
#define METHOD_FLAG_STATIC       0x1
#define METHOD_FLAG_CLASSMETHOD  0x2
#define METHOD_IS_STATIC(f)      ((f) & METHOD_FLAG_STATIC)
#define METHOD_IS_CLASSMETHOD(f) ((f) & METHOD_FLAG_CLASSMETHOD)
...
/* EMBARCADERO LOCAL Delphi char support */
extern LONGEST unpack_char (struct type *type, const gdb_byte *valaddr);
...
/* EMBARCADERO LOCAL Delphi strings */
extern struct value *value_coerce_string (struct value *value);
...
extern struct value *value_struct_elt (struct value **argp,
				       struct value **args,
				       char *name, int *memfuncflagsp,
				       char *err);

/* EMBARCADERO LOCAL support for properties.  */
extern struct value *value_struct_elt_as_lval (struct value **argp,
				               struct value **args,
				               char *name, int *memfuncflagsp,
				               char *err);
...
extern int using_pass_value_as_reference (struct value *function,
					  struct type *value_type);
...
extern struct value *value_of_implicit_frameptr_param (int complain);
extern struct value *value_of_implicit_frame_struct (int complain);
...
/* EMBARCADERO LOCAL: add declaration of defined in valarith.c */
extern struct value *value_subscripted_rvalue(struct value *value, struct value *idx);
