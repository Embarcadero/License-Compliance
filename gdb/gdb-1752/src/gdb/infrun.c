...
resume (int step, enum target_signal sig)
{
  ...
  if (should_resume)
      ...
      if (CANNOT_STEP_BREAKPOINT)
	{
      ...
      /* EMBARCADERO Local: Run all threads always, otherwise 
         GDB will not able to detect cases if stepped instruction 
         caused exception. No side effects noticed yet.
         KIRILL TODO: Probably RTL exception processing specific ?*/
#if 0
      target_resume (resume_ptid, step, sig);
#endif
      target_resume (RESUME_ALL, step, sig);
...
extern void macosx_print_extra_stop_info (int, CORE_ADDR);
  ...
  /* Handle cases caused by hitting a breakpoint.  */
  {
      ...
      case BPSTAT_WHAT_CHECK_SHLIBS_RESUME_FROM_HOOK:
	{
	  ...
	  /* APPLE LOCAL solib_changed */
	  if ((stop_on_solib_events && solib_changed) || (stop_stack_dummy != STOP_NONE))
	    {
