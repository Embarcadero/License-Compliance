...
struct type *builtin_type_Delphi_char;
...
static void dump_prop_fields (struct type *, int);
...
lookup_methodptr_type (struct type *to_type)
{
  ...
  TYPE_LENGTH_ASSIGN (mtype) = cplus_method_ptr_size ();
  ...
  if (name)
    {
      ...
      TYPE_FIELD_BITPOS_ASSIGN (type, bitpos) = bitpos;
    }
  else
    {
      /* Don't show this field to the user.  */
      TYPE_FIELD_BITPOS_ASSIGN (type, bitpos) = -1;
    }
}
...
/* Return the class which contains the member with the specified name,
   otherwise, return 0. */
struct type *
lookup_struct_elt_struct (struct type *type, char *name, int noerr)
{
  int i;
  char *type_for_printing;
  ...
  for (i = TYPE_N_BASECLASSES (type) - 1; i >= 0; i--)
    {
      struct type *t;
      t = lookup_struct_elt_struct (TYPE_BASECLASS (type, i), name, 1);
      if (t != NULL)
	{
	  return t;
	}
...
/* Find the method for the named function in class type T.
   Return the method if it was found, otherwise, return 0.  */

struct fn_field *
lookup_struct_elt_fn_field (struct type *t, char *name, char *physname)
{
  int i;
  struct fn_field *f = 0;
  char *type_for_printing;
  ...
  if (TYPE_CODE (t) != TYPE_CODE_STRUCT &&
      TYPE_CODE (t) != TYPE_CODE_UNION)
    {
      target_terminal_ours ();
      gdb_flush (gdb_stdout);
      type_for_printing = type_sprint (t, "", -1);
      make_cleanup (xfree, type_for_printing);
      error ("Type %s is not a structure or union.", type_for_printing);
    }

  for (i = 0; i < TYPE_NFN_FIELDS (t); i++)
    {
      int j;
      f = TYPE_FN_FIELDLIST1 (t, i);

      if (!physname)
        {
	  /* Search the normal name. */
        ...
        {
	  /* Search the physical name. */
          for (j = 0; j < TYPE_FN_FIELDLIST_LENGTH (t, i); j++)
	    {
              char *t_field_name = TYPE_FN_FIELD_PHYSNAME (f, j);
              if (t_field_name && (strcmp_iw (t_field_name, physname) == 0))
	        {
	          return f;
	        }
	    }
  ...
  for (i = TYPE_N_BASECLASSES (t) - 1; i >= 0; i--)
    {
      f = lookup_struct_elt_fn_field (TYPE_BASECLASS (t, i), name, physname);
      if (f != NULL)
	{
	  return f;
	}
...
/* Find the property for the specified named field in class type T.
   Return the property if it was found, otherwise, return 0.  */

struct prop_field *
lookup_struct_elt_prop_field (struct type *t, char *name)
{
  int i;
  struct prop_field *f;
  char *type_for_printing;
  ...
  if (TYPE_CODE (t) != TYPE_CODE_STRUCT &&
      TYPE_CODE (t) != TYPE_CODE_UNION)
    {
      ...
      type_for_printing = type_sprint (t, "", -1);
      make_cleanup (xfree, type_for_printing);
      error ("Type %s is not a structure or union t.", type_for_printing);
    }
  ...
  for (i = TYPE_N_BASECLASSES (t) - 1; i >= 0; i--)
    {
      f = lookup_struct_elt_prop_field (TYPE_BASECLASS (t, i), name);
      if (f != NULL)
	{
	  return f;
	}
...
/* Print an error for the unknown structure's element.  */

void
error_unknown_struct_elt (struct type *type, char *name)
{
  char *type_for_printing;
  ...
  type_for_printing = type_sprint (type, "", -1); 
  make_cleanup (xfree, type_for_printing);
  error ("Type %s has unknown component named %s.", type_for_printing, name);
}
...
check_stub_method (struct type *type, int method_id, int signature_id)
{
  ...
  /* EMBARCADERO LOCAL: FIXME: need a way to check for class method.  */ 
  ...
  /* EMBARCADERO LOCAL: FIXME: need a way to check for Delphi ABI.  */ 
...
/* EMBARCADERO LOCAL: begin Delphi strings in C++ */
/* Return true if T is any of Delphi three Delphi types.
   This happens when we are using C++ types based on Delphi ones. */

int c_delphi_string_type (struct type *type)
{
  CHECK_TYPEDEF (type);

  if (!TYPE_NAME (type) || current_language->la_language != language_cplus)
    return 0;

  if (strcmp (TYPE_NAME (type), "System::AnsiString") == 0)
    type = TYPE_TARGET_TYPE(type); 

  if (TYPE_CODE (type) == TYPE_CODE_STRUCT
      && TYPE_DELPHI_RETURN (type)
      && (strcmp (TYPE_NAME (type), "System::UnicodeString") == 0
	  || strcmp (TYPE_NAME (type), "System::AnsiStringT<0>") == 0	
	  || strcmp (TYPE_NAME (type), "System::RawByteString") == 0))
    return 1;
  else
    return 0;
}
/* EMBARCADERO LOCAL: end Delphi strings in C++ */
...
rank_one_type (struct type *parm, struct type *arg)
{
  ...
  */
  ...
  if (TYPE_NAME (parm) && TYPE_NAME (arg) &&
    ...
    return 0;
  /* EMBARCADERO LOCAL: begin check tag_names */
  /* Newer versions of gdb have the name = tag_name when the type is a
     struct, so let's also check the tag name if name is NULL.  */
  else if (!TYPE_NAME (parm) && !TYPE_NAME (arg) &&
	   TYPE_TAG_NAME (parm) && TYPE_TAG_NAME (arg) &&
	   !strcmp (TYPE_TAG_NAME (parm), TYPE_TAG_NAME (arg)))
    return 0;
  /* EMBARCADERO LOCAL: end check tag_names */
  ...
  switch (TYPE_CODE (parm))
	...
	case TYPE_CODE_RANGE:
	case TYPE_CODE_BOOL:
	  /* EMBARCADERO LOCAL don't allow passing integrals as pointers */
	  /* was: return POINTER_CONVERSION_BADNESS; */
...
/* EMBARCADERO LOCAL begin support for properties.  */
static void
dump_prop_fields (struct type *type, int spaces)
{
  int prop_idx;
  struct prop_field *f;

  printfi_filtered (spaces, "prop_fields ");
  gdb_print_host_address (TYPE_PROP_FIELDS (type), gdb_stdout);
  printf_filtered ("\n");
  for (prop_idx = 0; prop_idx < TYPE_NPROP_FIELDS (type); prop_idx++)
    {
      f = &TYPE_PROP_FIELD (type, prop_idx);
      printfi_filtered (spaces + 2, "[%d] name '%s' (",
			prop_idx, PROP_FIELD_NAME (*f));
      gdb_print_host_address (PROP_FIELD_NAME (*f), gdb_stdout);
      printf_filtered (")\n");

      printfi_filtered (spaces + 8, "type ");
      gdb_print_host_address (PROP_FIELD_TYPE (*f), gdb_stdout);
      printf_filtered ("\n");
      recursive_dump_type (PROP_FIELD_TYPE (*f), spaces + 8 + 2);

      /* Print read fields. */
      if (PROP_FIELD_READ_NAME (*f))
	{
          printfi_filtered (spaces + 8, "read.name '%s' (",
  		            PROP_FIELD_READ_NAME (*f));
          gdb_print_host_address (PROP_FIELD_READ_NAME (*f), gdb_stdout);
	}
      if (PROP_FIELD_READ_PHYSNAME (*f))
	{
          printfi_filtered (spaces + 8, "read.physname '%s' (",
  		            PROP_FIELD_READ_PHYSNAME (*f));
          gdb_print_host_address (PROP_FIELD_READ_NAME (*f), gdb_stdout);
	}
  
      /* Print write fields. */
      if (PROP_FIELD_WRITE_NAME (*f))
	{
          printfi_filtered (spaces + 8, "write.name '%s' (",
  		            PROP_FIELD_WRITE_NAME (*f));
          gdb_print_host_address (PROP_FIELD_WRITE_NAME (*f), gdb_stdout);
	}
      if (PROP_FIELD_WRITE_PHYSNAME (*f))
	{
          printfi_filtered (spaces + 8, "write.physname '%s' (",
  		            PROP_FIELD_WRITE_PHYSNAME (*f));
          gdb_print_host_address (PROP_FIELD_WRITE_NAME (*f), gdb_stdout);
	}
  
      /* Print other property fields. */
      printfi_filtered (spaces + 8, "has_getter %d\n",
  		        PROP_FIELD_GETTER (*f));
      printfi_filtered (spaces + 8, "has_setter %d\n",
  		        PROP_FIELD_SETTER (*f));
      printfi_filtered (spaces + 8, "is_static %d\n",
  		        PROP_FIELD_STATIC (*f));
      printfi_filtered (spaces + 8, "is_private %d\n",
  		        PROP_FIELD_PRIVATE (*f));
      printfi_filtered (spaces + 8, "is_protected %d\n",
  		        PROP_FIELD_PROTECTED (*f));
      printfi_filtered (spaces + 8, "is_public %d\n",
  		        PROP_FIELD_PUBLIC (*f));
      printfi_filtered (spaces + 8, "index %d\n",
  		        PROP_FIELD_INDEX (*f));
    }
}
/* EMBARCADERO LOCAL end support for properties.  */
...
print_cplus_stuff (struct type *type, int spaces)
{
  ...
  /* EMBARCADERO LOCAL: Support for properties.  */
  printfi_filtered (spaces, "nprop_fields %d\n",
		    TYPE_NPROP_FIELDS (type));
  ...
  /* EMBARCADERO LOCAL: Support for properties.  */
  if (TYPE_NPROP_FIELDS (type) > 0)
    {
      dump_prop_fields (type, spaces);
    }
}
...
const char *type_code_name (int code)
{
  switch (code)
    ...
    case TYPE_CODE_PROPERTY:
      return "TYPE_CODE_PROPERTY";
    case TYPE_CODE_REF:
...
recursive_dump_type (struct type *type, int spaces)
{
  ...
  if (TYPE_DELPHI_ABI (type))
    {
      puts_filtered (" TYPE_FLAG_DELPHI_ABI");
    }
...
/* vinschen@redhat.com 2002-02-08:
  ...
  builtin_type_Delphi_char =
    init_type (TYPE_CODE_CHAR, 16 / TARGET_CHAR_BIT,
	       0,
	       "N6System8WideCharE", (struct objfile *) NULL);
  /* EMBARCADERO LOCAL begin string literals */
...
gdbtypes_post_init (struct gdbarch *gdbarch)
{
  ...
  builtin_type->builtin_int8 =
    init_type (TYPE_CODE_INT, 8 / TARGET_CHAR_BIT,
	       0,
  ...
  builtin_type->builtin_char16 =
    init_type (TYPE_CODE_INT, 16 / TARGET_CHAR_BIT,
	       0,
	       "char16_t", (struct objfile *) NULL);
  builtin_type->builtin_char32 =
    init_type (TYPE_CODE_INT, 32 / TARGET_CHAR_BIT,
	       0,
	       "char32_t", (struct objfile *) NULL);
