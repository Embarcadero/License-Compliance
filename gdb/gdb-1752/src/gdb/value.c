...
/* EMBARCADERO Local begin parsing support */
...
/* EMBARCADERO Local end parsing support */
...
show_convenience (char *ignore, int from_tty)
{
  ...
  for (var = internalvars; var; var = var->next)
      ...
      if (!varseen)
	{
      ...
      value_print (var->value, gdb_stdout, &opts);
      printf_filtered (("\n"));
    }
  if (!varseen)
    printf_unfiltered (_("\
...
retry:
  ...
  switch (code)
	...
	lowbound = current_language->string_lower_bound;
...
int
using_pass_value_as_reference (struct value *function,
			       struct type *arg_type)
{
  ...
  return (gdbarch_pass_value_as_reference (current_gdbarch, function,
					   arg_type));
}
