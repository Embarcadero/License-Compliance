...
{
  /* EMBARCADERO Local don't put quotes around types */
  //was: int has_quotable = (strstr (TYPE_NAME (type), "::") != NULL);
  int has_quotable = 0;
		   ...
		   int level)
{
  ...
  switch (TYPE_CODE (type))
    ...
    struct_union:
      ...
      if (TYPE_FLAGS (type) & TYPE_FLAG_DELPHI_CLASS)
	fprintf_filtered (stream, "__declspec(delphiclass) ");
      if (TYPE_FLAGS (type) & TYPE_FLAG_DELPHI_RECORD)
	fprintf_filtered (stream, "__declspec(delphirecord) ");
      if (TYPE_FLAGS (type) & TYPE_FLAG_DELPHI_RETURN)
	fprintf_filtered (stream, "__declspec(delphireturn) ");
      ...
      else if (show > 0 || TYPE_TAG_NAME (type) == NULL)
	{
	  ...
	  for (i = 0; i < len; i++)
	      ...
	      for (j = 0; j < len2; j++)
		{
		  ...
		  if (TYPE_FN_FIELD_METACLASS (f, j))
		    fprintf_filtered (stream, "__classmethod ");
	  ...
	  for (i = 0; i < len; i++)
	      ...
	      else
		{
	      ...
	      /* Eg: __property int pValue = { read=p_value, write=p_value }; */
	      print_spaces_filtered (level + 4, stream);
	      if (TYPE_PROP_FIELD_STATIC (type, i))
	        fprintf_filtered (stream, "static ");
	      fprintf_filtered (stream, "__property ");
