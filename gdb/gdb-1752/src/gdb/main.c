...
/* EMBARCADERO Local: StopSocket control variables - see remote.c */
char *stopsocket_path = NULL;
int stopsocket_enable = 0;
...
captured_main (void *data)
{
  ...
  /* Parse arguments and options.  */
  {
    ...
    enum {
      ...
      OPT_STOPSOCKET /* EMBARCADERO LOCAL */
    };
...
/* EMBARCADERO LOCAL: StopSocket option */
      {"socketstop", required_argument, 0, OPT_STOPSOCKET},
      {0, no_argument, 0, 0}
    };

    /* EMBARCADERO LOCAL: begin File logger */
    /* See if the user has requested a log file to be generated.
       If so, dump command line options.  */
    if (logging_enabled ())
      {
	int i;
	fprintf_unfiltered (file_log, "options passed to gdb:\n");
	for (i = 0; i < argc; i++)
	  fprintf_unfiltered (file_log, "    argv[%d]: %s\n", i, argv[i]);
      }
    /* EMBARCADERO LOCAL: end File logger */

    while (1)
      {
	int option_index;

	if (file_log && optind < argc)
	  fprintf_unfiltered (file_log, "next option argv[%d]: %s\n",
			      optind, argv[optind]);
	...
	{
	  if (file_log)
	    fprintf_unfiltered (file_log, "-- end of options --\n");
	  break;
	}
	if (file_log)
	  {
	    fprintf_unfiltered (file_log, "    option_index=%d\n", option_index);
	    fprintf_unfiltered (file_log, "    optind=%d\n", optind);
	    fprintf_unfiltered (file_log, "    optarg=%s\n", optarg ? optarg : "(nil)");
	  }
	...
	switch (c)
	  ...
	  case OPT_STOPSOCKET:
	    { 
	      ...
	      stopsocket_path = xstrdup (optarg);
	      /* Check passed stopsocket path length */
	      if (strlen (stopsocket_path) > (sizeof (ss_address.sun_path) - 1))
		{
		  if (logging_enabled ())
		    fprintf_unfiltered (file_log, 
					"error: StopSocket path exceeds allowed maximum length, GDB exiting.\n");
		  fprintf_unfiltered (gdb_stderr,
				      _("error: StopSocket path \"%s\" exceeds allowed maximum length of %d\n"),
				      optarg, sizeof (ss_address.sun_path) - 1);
		  exit (1);
		}
	    }
	    stopsocket_enable = 1;
...
{
  fputs_unfiltered (_("\
This is the Locally modified GNU debugger.  Usage:\n\n\
...
/* EMBARCADERO LOCAL: StopSocket option */
  fputs_unfiltered (_("\
  --socketstop=NAME  Use socket NAME for receiving stop requests on a separate thread.\n\
"), stream);
  fputs_unfiltered (_("\n\
