...
struct field_info
  {
    ...
    /* EMBARCADERO LOCAL begin dwarf extensions */
    /* Member properties fields array, entries are allocated in the order they
       are encountered in the object file.  */
    struct nextpropfield
      {
	struct nextpropfield *next;
	int accessibility;
	int virtuality;
	struct prop_field propfield;
      }
     *propfields;

    /* Number of entries in the propfields array.  */
    int npropfields;
    /* EMBARCADERO LOCAL end dwarf extensions */
  };
...
/* EMBARCADERO LOCAL begin dwarf extensions */
static void dwarf2_add_prop_field (struct field_info *fip,
	                           struct die_info *die,
				   struct dwarf2_cu *cu);

static void dwarf2_attach_prop_fields_to_type (struct field_info *fip,
					       struct type *type,
					       struct dwarf2_cu *cu);
/* EMBARCADERO LOCAL end dwarf extensions */
  ...
  for (i = 0; i < oso_nlists_count; i++)
    {
      ...
      if (pst->language == language_pascal)
        /* EMBARCADERO Local: Delphi support */
        demangled  = cplus_demangle (nlists[i].name, DMGL_PARAMS | DMGL_DELPHI);
...
dwarf2_kext_psymtab_to_symtab (struct partial_symtab *pst)
{
  ...
  load_full_comp_unit (this_cu, pst->objfile, addr_map);
...
queue_comp_unit (struct dwarf2_per_cu_data *per_cu, struct objfile *objfile)
{
  ...
  /* EMBARCADERO FIXME: how to get addr_map?  */
  load_full_comp_unit (item->per_cu, objfile, NULL);
...
process_die (struct die_info *die, struct dwarf2_cu *cu)
{
  switch (die->tag)
      ...
      if (dwarf2_allow_inlined_stepping)
	{
      ...
      /* APPLE LOCAL end subroutine inlining  */
      break;
      /* EMBARCADERO LOCAL fall through to func scope */
    case DW_TAG_subprogram:
    ...
    /* EMBARCADERO LOCAL Delphi sets */
    case DW_TAG_BORLAND_Delphi_set:
    case DW_TAG_array_type:
    /* EMBARCADERO LOCAL Delphi dynamic arrays */
    ...
    case DW_TAG_subrange_type:
      ...
      break;
    /* EMBARCADERO LOCAL merged die type handling */
    case DW_TAG_typedef:
      /* Add a typedef symbol for the type definition, if it has a
         DW_AT_name.  */
      /* EMBARCADERO LOCAL begin Set typedef names */
      {
	struct type *type = read_type_die (die, cu);
	struct symbol *sym = new_symbol (die, type, cu);
	char *name = SYMBOL_LINKAGE_NAME (sym);
	/* If we didn't give the typedef a name, set it to that of its
	   corresponding symbol.  FIXME: This shouldn't be needed, as its not
	   needed in newer GDBs.  See RS-53508 for reference. */
	if (name && !TYPE_NAME (type))
	  TYPE_NAME (type) = name;
      }
      /* EMBARCADERO LOCAL end Set typedef names */
      break;
	...
	complaint (&symfile_complaints, _("Tag '%s' has unexpected children"),
      ...
      break;
#if 0 /* EMBARCADERO LOCAL merged die type handling conflict */
    /* APPLE LOCAL: Handle DW_TAG_const_type.  */
    case DW_TAG_const_type:
      read_type_die (die, cu);
      break;
#endif /* 0 */
#if 0 /* EMBARCADERO LOCAL DW_TAG_typedef now handled above */
...
read_import_statement (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  imported_cu = cu;
  imported_die = follow_die_ref (die, import_attr, &imported_cu);
  ...
  if (strlen (imported_name_prefix) > 0)
    {
      ...
      const char *nsop = (cu->language == language_pascal) ? "." : "::";
      canonical_name = alloca (strlen (imported_name_prefix)
                               + strlen (nsop) + strlen (imported_name) + 1);
      strcpy (canonical_name, imported_name_prefix);
      strcat (canonical_name, nsop);
...
read_file_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  /* EMBARCADERO LOCAL Delphi */
  /* Remember the current language so we can restore it later.  */
  back_to = make_cleanup (restore_language, (void *) current_language->la_language);
  ...
  /* EMBARCADERO LOCAL Delphi */
  /* Set the current language to match that of the CU.  */
  set_language (cu->language);
  ...
  if (abs_orig_attr)
    {
      ...
      abstract_origin = follow_die_ref (die, abs_orig_attr, &arg_cu);
      decl_file = dwarf2_attr (abstract_origin, DW_AT_decl_file, arg_cu);
      decl_line = dwarf2_attr (abstract_origin, DW_AT_decl_line, arg_cu);
    }
...
/* EMBARCADERO Local: DbgExcNotify line number */
unsigned short DbgExcNotify_line = 0;
...
/* APPLE LOCAL end address ranges  */
{
  ...
  *ranges = NULL;
  ...
  if (attr)
	...
	return 0;
      /* EMBARCADERO Local: dSYM workaround 0xdeadbeef check */
      if ((low == (CORE_ADDR)0xdeadbeef) || (high == (CORE_ADDR)0xdeadbeef))
	{
	  return 0;
	}
		      ...
		      struct type *type, struct dwarf2_cu *cu)
{
  ...
  if (this_type && TYPE_CODE (this_type) == TYPE_CODE_FUNC)
    {
      ...
      if (TYPE_DELPHI_ABI (this_type))
	TYPE_FLAGS (fnp->type) |= TYPE_FLAG_DELPHI_ABI;

      /* EMBARCADERO Local: begin calling conventions */
      if (TYPE_CALLING_CONVENTION (this_type) != 0) 
	TYPE_CALLING_CONVENTION (fnp->type) = TYPE_CALLING_CONVENTION (this_type);
      else
	TYPE_CALLING_CONVENTION (fnp->type) = DW_CC_normal;
	...
	fnp->voffset = VOFFSET_STATIC;
      ...
      else if (TYPE_FIELD_METACLASS (this_type, 0) == 1)
	fnp->is_metaclass = 1;
      /* EMBARCADERO Local: Delphi nested functions */
      else if (TYPE_FIELD_FRAMEPTR (this_type, 0) == 1)
	fnp->is_frameptr = 1;
    }
  else
  ...
  /* EMBARCADERO Local: Delphi class methods */
  /* Check for Delphi class methods.  */
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_metaclass, cu);
  if (attr && DW_UNSND (attr) != 0)
    fnp->is_metaclass = 1;

  /* EMBARCADERO Local: Delphi nested functions */
  /* Check for Delphi nested function frame pointer.  */
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_frameptr, cu);
  if (attr && DW_UNSND (attr) != 0)
    fnp->is_frameptr = 1;
...
/* EMBARCADERO LOCAL begin dwarf extensions */
/* Add a property to the proper fieldlist.  */

static void
dwarf2_add_prop_field (struct field_info *fip, struct die_info *die,
		       struct dwarf2_cu *cu)
{
  struct attribute *attr;
  struct prop_field *fpp;
  char *fieldname;
  struct nextpropfield *new_propfield;
  int pass;
  ...
  /* Allocate a new property list entry and link it in.  */
  new_propfield = (struct nextpropfield *) xmalloc (sizeof (struct nextpropfield));
  make_cleanup (xfree, new_propfield);
  memset (new_propfield, 0, sizeof (struct nextpropfield));
  new_propfield->next = fip->propfields;
  fip->propfields = new_propfield;
  fip->npropfields++;

  fpp = &new_propfield->propfield;

  PROP_FIELD_TYPE (*fpp) = die_type (die, cu);
  PROP_FIELD_NAME (*fpp) = fieldname;

  /* Get index of property (if any).  */
  attr = dwarf2_attr (die, DW_AT_BORLAND_property_index, cu);
  if (attr && DW_SND (attr))
    {
      fpp->has_index = 1;
      fpp->index = DW_SND (attr);
    }
  ...
  /* Get name of property read/write.  */
  for (pass = 1; pass <= 2; ++pass) 
    {
      struct attribute *ref_attr;
      struct die_info *ref_die;
      struct type *class_type;
      struct dwarf2_cu *arg_cu = cu;
      char *ref_name = 0;
      enum dwarf_attribute attr_kind;
      int pass_read = (pass == 1);
  
      attr_kind = (pass_read)
  	? DW_AT_BORLAND_property_read
  	: DW_AT_BORLAND_property_write;
  
      ref_attr = dwarf2_attr (die, attr_kind, cu); 
      if (!ref_attr)
        continue;
      ref_die = follow_die_ref (die, ref_attr, &arg_cu);
      if (!ref_die)
        {
          continue;
        }
      if (ref_die->parent->tag != DW_TAG_structure_type &&
          ref_die->parent->tag != DW_TAG_class_type)
        {
          continue;
        }
      class_type = die_type (ref_die->parent, cu);
      if (!class_type)
        {
          continue;
        }
      attr = dwarf2_attr (ref_die, DW_AT_name, cu);
      if (attr && DW_STRING (attr))
	ref_name = DW_STRING (attr);
      if (!ref_name)
        {
          continue;
        }
      if (pass_read)
        fpp->read.name = ref_name;
      else
        fpp->write.name = ref_name;
      if (ref_die->tag == DW_TAG_subprogram)
        {
	  char *ref_physname;
	  ref_physname = dwarf2_linkage_name (ref_die, cu);
	  if (ref_physname)
	    {
              if (pass_read)
                fpp->read.physname = ref_physname;
              else
                fpp->write.physname = ref_physname;
	    }
	  else
	    {
	      complaint (&symfile_complaints, _("property '%s' references %s subroutine with no linkage name"),
					        fieldname,
					        (pass_read ? "read" : "write"));
	    }
  	  if (pass_read)
  	    fpp->has_getter = 1;
  	  else
        ...
        {
	  complaint (&symfile_complaints, _("property '%s' references unexpected %s tag '%s'"),
					    fieldname,
					    (pass_read ? "read" : "write"),
					    dwarf_tag_name(ref_die->tag));
  	  return;
        }
      /* Note: to set field/fnfield, we'd need to loop
         through fields/fnfields looking for name.
         For now we leave them empty. */
    }
}

/* Create the vector of property fields, and attach it to the type.  */

static void
dwarf2_attach_prop_fields_to_type (struct field_info *fip, struct type *type,
				   struct dwarf2_cu *cu)
{
  struct nextpropfield *flp;
  int i;

  ALLOCATE_CPLUS_STRUCT_TYPE (type);
  TYPE_PROP_FIELDS (type) = (struct prop_field *)
    TYPE_ALLOC (type, sizeof (struct prop_field) * fip->npropfields);

  for (i = 0, flp = fip->propfields; i < fip->npropfields; i++, flp = flp->next)
    {
      TYPE_PROP_FIELD (type, i) = flp->propfield;
    }
  TYPE_NPROP_FIELDS (type) = fip->npropfields;
}
/* EMBARCADERO LOCAL begin dwarf extensions */
...
read_structure_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (die->child != NULL && ! die_is_declaration (die, cu))
      ...
      while (child_die && child_die->tag)
	{
	  ...
	  /* EMBARCADERO LOCAL begin dwarf extensions */
	  else if (child_die->tag == DW_TAG_BORLAND_property)
	    {
	      /* C++ property.  */
	      dwarf2_add_prop_field (&fi, child_die, cu);
	    }
	  /* EMBARCADERO LOCAL end dwarf extensions */
	  child_die = sibling_die (child_die);
	}
      ...
      if (fi.nfnfields)
	{
      ...
      /* EMBARCADERO LOCAL begin dwarf extensions */
      if (fi.npropfields)
	dwarf2_attach_prop_fields_to_type (&fi, type, cu);
      /* EMBARCADERO LOCAL end dwarf extensions */
    }
  ...
  /* EMBARCADERO LOCAL begin dwarf extensions */
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_class, cu);
  if (attr && DW_UNSND (attr))
    TYPE_FLAGS (type) |= TYPE_FLAG_DELPHI_CLASS;
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_record, cu);
  if (attr && DW_UNSND (attr))
    TYPE_FLAGS (type) |= TYPE_FLAG_DELPHI_RECORD;
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_return, cu);
  if (attr && DW_UNSND (attr))
    TYPE_FLAGS (type) |= TYPE_FLAG_DELPHI_RETURN;
  /* EMBARCADERO LOCAL end dwarf extensions */
	...
	if (attr && DW_STRING (attr))
	  {
	    ...
	    TYPE_TARGET_TYPE (this_type) = id_type;
...
/* Extract all information from a DW_TAG_BORLAND_Delphi_dynamic_array DIE and add to
   the user defined type vector.  */

static struct type *
read_tag_Delphi_dynamic_array_type (struct die_info *die, struct dwarf2_cu *cu)
{
  struct comp_unit_head *cu_header = &cu->header;
  struct type *type;
  struct attribute *attr_byte_size;
  ...
  /* We represent Delphi dynamic arays as special pointers, but we need to create a
     new instance of the pointer so that we don't end up marking a normal pointer that
     is already in use by another symbol as a dynamic array.
     FIXME: We should probably be using instance flags for this.  */

  type = alloc_type (TYPE_OBJFILE (target_type));
  TYPE_TARGET_TYPE (type) = target_type;

  /* FIXME!  Assume the machine has only one representation for pointers!  */

  TYPE_LENGTH_ASSIGN (type) = TARGET_PTR_BIT / TARGET_CHAR_BIT;
  ...
  /* Flag the pointer as being a Delphi dynamic array. */
  TYPE_FLAGS (type) |= TYPE_FLAG_DELPHI_DYNARRAY;

  /* Now do the normal pointer stuff (as in read_tag_pointer_type). */
  attr_byte_size = dwarf2_attr (die, DW_AT_byte_size, cu);
  if (attr_byte_size)
    byte_size = DW_UNSND (attr_byte_size);
  else
...
read_tag_const_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  cv_type = make_cvr_type (1, TYPE_VOLATILE (base_type),
...
read_tag_volatile_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  base_type = die_type (die, cu);
  cv_type = make_cvr_type (TYPE_CONST (base_type), 1,
			   TYPE_RESTRICT (base_type), base_type, 0);
  return set_die_type (die, cv_type, cu);
}
...
read_tag_restrict_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  base_type = die_type (die, cu);
  cv_type = make_cvr_type (TYPE_CONST (base_type), TYPE_VOLATILE (base_type),
			   1, base_type, 0);
  return set_die_type (die, cv_type, cu);
}
...
read_tag_string_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (cu->language == language_pascal)
    {
      type = dwarf2_fundamental_type (objfile, FT_STRING, cu);
    }
  else
...
read_subroutine_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  /* EMBARCADERO LOCAL: Delphi ABI */
  /* Check for Delphi ABI.  */
  attr = dwarf2_attr (die, DW_AT_BORLAND_Delphi_ABI, cu);
  if (attr && (DW_UNSND (attr) != 0))
    TYPE_FLAGS (ftype) |= TYPE_FLAG_DELPHI_ABI;
      ...
      while (child_die && child_die->tag)
	{
	  if (child_die->tag == DW_TAG_formal_parameter)
		...
		TYPE_FIELD_ARTIFICIAL (ftype, iparams) = 0;
	      ...
	      attr = dwarf2_attr (child_die, DW_AT_BORLAND_Delphi_metaclass, cu);
	      if (attr)
		TYPE_FIELD_METACLASS (ftype, iparams) = DW_UNSND (attr);
	      else
		TYPE_FIELD_METACLASS (ftype, iparams) = 0;
	      /* EMBARCADERO Local: end Delphi class methods */
	      /* EMBARCADERO Local: begin Delphi nested functions */
	      attr = dwarf2_attr (child_die, DW_AT_BORLAND_Delphi_frameptr, cu);
	      if (attr)
		TYPE_FIELD_FRAMEPTR (ftype, iparams) = DW_UNSND (attr);
	      else
		TYPE_FIELD_FRAMEPTR (ftype, iparams) = 0;
	      /* EMBARCADERO Local: end Delphi nested functions */
...
read_base_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  struct type *type;
  struct attribute *attr;
  int encoding = 0, size = 0, decimal_scale = 0;
  char *name;
  ...
  if (0 != decimal_scale &&
      (encoding == DW_ATE_signed || encoding == DW_ATE_unsigned))
    TYPE_DECIMAL_SCALE (type) = decimal_scale;
  TYPE_NAME (type) = name;
  TYPE_TARGET_TYPE (type) = target_type;

  /* EMBARCADERO LOCAL Delphi types */
  if (name && (strcmp_iw (name, "Currency") == 0))
    TYPE_DECIMAL_SCALE (type) = -4;
	...
	case DW_AT_MIPS_linkage_name:
		...
		part_die->name = DW_STRING (&attr);
		 ...
		 the '$' must be uppercase, a digit or a '$'.  For
	      ...
	      if (part_die->name != NULL && strlen (part_die->name) >=  5
		  && part_die->name[0] == '*'
		  && part_die->name[1] == '_')
		{
		  char *short_name;
...
set_cu_language (unsigned int lang, struct dwarf2_cu *cu)
{
  switch (lang)
    ...
    case DW_LANG_Ada95:
      ...
      break;
    /* EMBARCADERO Local: Delphi treated as Pascal.  */
...
dwarf2_attr (struct die_info *die, unsigned int name, struct dwarf2_cu *cu)
{
  ...
  if (repository_spec)
    {
      die = follow_die_ref (die, repository_spec, &cu);
      return dwarf2_attr (die, name, cu);
    }
  /* APPLE LOCAL end dwarf repository  */
      ...
      if (current && (call_site_subfile == NULL || decl_site_subfile == NULL))
	{
	  ...
	  if (call_site_subfile == NULL)
	    {
	      ...
	      /* EMBARCADERO Local: Check file entry against possible buggy 
		 line table and filenames */   
	      if (fe && fe->dir_index && 
		  lh && lh->include_dirs && 
		  ((int)(fe->dir_index - 1) >= 0) && 
		  (fe->dir_index <= lh->num_include_dirs))
		dir = lh->include_dirs[fe->dir_index - 1];
	      else
		dir = comp_dir;
      
	      /* Create the new subfile record.  */
	      if (fe && (current->file_index <= lh->num_file_names))
	  ...
	  if (decl_site_subfile == NULL)
		...
		dir = comp_dir;
	      ...
	      /* Create the new subfile record.  */
	      if (fe && (current->decl_file_index <= lh->num_file_names))    
	        dwarf2_start_subfile (fe->name, dir, cu->comp_dir);
	    }
	    ...
	    case DW_LNS_set_file:
              {
                ...
                if (fe && fe->dir_index && 
                    lh && lh->include_dirs && 
                    ((signed)(fe->dir_index - 1) >= 0) && 
                    (fe->dir_index <= lh->num_include_dirs))
                  dir = lh->include_dirs[fe->dir_index - 1];
                else
                  dir = comp_dir;
                /* APPLE LOCAL: Pass in the compilation dir of this CU.  */
                if (!decode_for_pst_p && fe && (file <= lh->num_file_names))
                  dwarf2_start_subfile (fe->name, dir, cu->comp_dir);
              }
	      break;
  ...
  if (decode_for_pst_p)
    {
      ...
      for (file_index = 0; file_index < lh->num_file_names; file_index++)
	    ...
	    /* EMBARCADERO LOCAL DOS style filenames */
            if (FILENAME_CMP (include_name, pst_filename) != 0)
...
dwarf2_start_subfile (char *filename, char *dirname, char *comp_dir)
{
  ...
  /* EMBARCADERO Local: Checks against possible buggy line 
			table and filenames */
  if (!filename || ((unsigned)filename >= UINT_MAX))
    return;
  ...
  if (!IS_ABSOLUTE_PATH (filename) && dirname != NULL)
	...
	{
	  /* EMBARCADERO LOCAL DOS style filenames */
	  /* If subfile->name has no path, don't compare with the fullname.  */
	  if ((HAS_DIR_SEPARATOR (subfile->name)
	       && FILENAME_CMP (subfile->name, fullname) == 0)
	      || FILENAME_CMP (subfile->name, filename) == 0)
	    {
		     ...
		     struct dwarf2_cu *cu)
{
...
#if 0 /* EMBARCADERO Local: Workaround no longer needed */
  /* EMBARCADERO Local: Workaround against issue with GDB crash 
     on evaluation variables resolved via location lists.  
     DAWN KIRILL FIXME: dsymlink tool should generate debug_loc 
     section to enable correct evaluation of variables described
     with location lists */
  ...
  if (attr_form_is_block (attr)
      ...
      && DW_BLOCK (attr)->data[0] == DW_OP_addr)
    {
      ...
      /* EMBARCADERO Local: dSYM workaround 0xdeadbeef check */
      /* A value of 0xdeadbeef set in AT_location attr means the variable was
         optimized out by the compiler.  */
      if (symaddr == 0xdeadbeef)
	{
	  SYMBOL_CLASS (sym) = LOC_OPTIMIZED_OUT;
	  return;
	}
  ...
  if (name)
	...
	case DW_TAG_formal_parameter:
	  ...
	  if (attr)
		...
		{
		  struct dwarf2_cu *arg_cu = cu;
		  abs_orig = follow_die_ref (die, abs_orig_attr, &arg_cu);
		  attr = dwarf2_attr (abs_orig, DW_AT_location, arg_cu);
		  if (attr)
		    {
		      var_decode_location (attr, sym, arg_cu);
		      if (SYMBOL_CLASS (sym) == LOC_COMPUTED)
			SYMBOL_CLASS (sym) = LOC_COMPUTED_ARG;
		    }
		}
...
read_type_die (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  switch (die->tag)
    ...
    /* EMBARCADERO LOCAL Delphi dynamic arrays */
    case DW_TAG_BORLAND_Delphi_dynamic_array:
      this_type = read_tag_Delphi_dynamic_array_type (die, cu);
      gdb_assert (TYPE_DELPHI_DYNARRAY (this_type));
      break;
    /* EMBARCADERO LOCAL Delphi sets */
    case DW_TAG_BORLAND_Delphi_set:
      // FIXME: treat Delphi sets as pointers as a workaround for crashes.
		 ...
		 struct dwarf2_cu *cu)
{
  ...
  if (suffix == NULL || suffix[0] == '\0' || prefix == NULL || prefix[0] == '\0')
    ...
    sep = ".";
  /* EMBARCADERO Local: Delphi using . separator */
  else if (cu->language == language_pascal)
    sep = ".";
  else
    sep = "::";
			...
			int return_die)
{
  ...
  if (temp_type && !die_type (type_die, cu))
    set_die_type (type_die, temp_type, cu);
