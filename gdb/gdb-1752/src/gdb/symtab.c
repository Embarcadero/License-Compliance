			    ...
			    const char *mangled)
{
  ...
  if (gsymbol->language == language_pascal
    ...
    {
      demangled =
        cplus_demangle (mangled, DMGL_DELPHI);
      if (demangled != NULL)
	{
	  gsymbol->language = language_pascal;
	  return demangled;
		  ...
		  const char *linkage_name, int len, struct objfile *objfile)
{
  ...
  if ((*slot)[lookup_len + 1] != '\0')
   	gsymbol->language_specific.cplus_specific.demangled_name = &(*slot)[lookup_len + 1];
  else
   	gsymbol->language_specific.cplus_specific.demangled_name = NULL;
		   ...
		   struct symtab **symtab, struct symbol_search **sym_list)
{
  ...
  else if (current_language->la_language == language_pascal)
    {
      /* EMBARCADERO LOCAL Delphi */
      demangled_name = cplus_demangle (name, DMGL_DELPHI);
      if (demangled_name)
	{
	  mangled_name = name;
	  modified_name = demangled_name;
		   ...
		   int *is_a_field_of_this, struct symtab **in_symtab_ptr)
{
  ...
  if (current_language->la_value_of_this != NULL
      ...
      if (v && check_field (v, name))
	{
	  ...
	  *is_a_field_of_this = ifk_this;
      ...
      /* We didn't find a 'this' pointer.  Do we have a 'frame'
         structure?  */
      v = value_of_implicit_frame_struct (0);
      if (v && check_field (v, name))
	{
	  struct type *val_type;
	  ...
	  *is_a_field_of_this = ifk_frame;
      ...
      v = value_of_implicit_frameptr_param (0);
      if (v && check_field (v, name))
	{
	  struct type *val_type;
...
#if 0 /* EMBARCADERO LOCAL merge conflict reverted due to bad type state */
  /* EMBARCADERO LOCAL begin merge conflict */
  /* EMBARCADERO FIXME: not sure how to merge the code above with the
     newer trunk's lookup code below, so adding it here as a second
     lookup instead.  */
  if (current_language->la_value_of_this != NULL
      && is_a_field_of_this != NULL
      && block != NULL)
    {
      ...
      /* EMBARCADERO LOCAL note: language passed in as a parameter in trunk.  */
      const char *name_of_this
	= (current_language->la_language == language_pascal)
	? "self" : "this";
	...
	sym = lookup_block_symbol (function_block, name_of_this,
				   NULL, VAR_DOMAIN);
      if (sym)
	{
  ...
  /* EMBARCADERO LOCAL end merge conflict */
#endif
...
/* EMBARCADERO LOCAL: begin case insensitive */
...
/* EMBARCADERO LOCAL: end case insensitive */
		       ...
		       domain_enum domain)
{
  ...
  /* EMBARCADERO LOCAL: case insensitive */
  char *search_name;
  ...
  if (global)			/* This means we can use a binary search. */
      ...
      while (top > bottom)
	{
	  ...
	  if (strcmp_iw_ordered (SYMBOL_SEARCH_NAME (*center), search_name) >= 0)
	    {
	      top = center;
	    }
	  else
