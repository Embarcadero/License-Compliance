...
struct gdbarch startup_gdbarch =
{
  ...
  /* EMBARCADERO Local: fCall Wrapper function arguments pusher */
  default_fcw_argument_push, /* fcw_argument_push */
  /* EMBARCADERO Local: Delphi ABI */
               ...
               struct gdbarch_tdep *tdep)
{
  ...
  /* EMBARCADERO Local: fCall Wrapper function arguments pusher */
  current_gdbarch->fcw_argument_push = default_fcw_argument_push;
  /* EMBARCADERO Local: Delphi ABI */
  current_gdbarch->pass_value_as_reference = default_pass_value_as_reference;
  /* EMBARCADERO Local: begin calling conventions */
  current_gdbarch->calling_convention_supported = default_calling_convention_supported;
  current_gdbarch->calling_convention_string = default_calling_convention_string;
  /* EMBARCADERO Local: end calling conventions */
  /* gdbarch_alloc() */
...
gdbarch_dump (struct gdbarch *current_gdbarch, struct ui_file *file)
{
  ...
  fprintf_unfiltered (file,
                      ...
                      (long) current_gdbarch->fcw_argument_push);
  ...
  fprintf_unfiltered (file,
                      ...
                      gdbarch_pass_value_as_reference_p (current_gdbarch));
  fprintf_unfiltered (file,
                      "gdbarch_dump: pass_value_as_reference = <0x%lx>\n",
                      (long) current_gdbarch->pass_value_as_reference);
  /* EMBARCADERO Local: end Delphi ABI */
  /* EMBARCADERO Local: begin calling conventions */
  fprintf_unfiltered (file,
                      "gdbarch_dump: calling_convention_supported = <0x%lx>\n",
                      (long) current_gdbarch->calling_convention_supported);
  fprintf_unfiltered (file,
                      "gdbarch_dump: calling_convention_string = <0x%lx>\n",
                      (long) current_gdbarch->calling_convention_string);
  /* EMBARCADERO Local: end calling conventions */
...
CORE_ADDR
gdbarch_push_dummy_code (struct gdbarch *gdbarch, CORE_ADDR sp, CORE_ADDR funaddr, struct value **args, int nargs, struct type *value_type, CORE_ADDR *real_pc, CORE_ADDR *bp_addr)
{
  gdb_assert (gdbarch != NULL);
  gdb_assert (gdbarch->push_dummy_code != NULL);
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_push_dummy_code called\n");
  return gdbarch->push_dummy_code (gdbarch, sp, funaddr, args, nargs, value_type, real_pc, bp_addr);
}
...
{
  gdb_assert (gdbarch != NULL);
  /* FCW supported only for ARM currently.  */
  if (gdbarch->fcw_argument_push == NULL)
    return 0;
  if (gdbarch_debug >= 2)
...
void 
set_gdbarch_fcw_argument_push (struct gdbarch *gdbarch,
			       gdbarch_fcw_argument_push_ftype *fcw_argument_push)
{
