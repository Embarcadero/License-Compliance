...
#include <uuid/uuid.h>
#include <regex.h>
/* EMBARCADERO LOCAL trunk merge for wide char support */
...
/* Additional file logging used for debugging and MI client-GDB interaction.
   Needed because MI log already redirected to console.  */
/* Non-NULL if we are generating a log file.  */
		...
		struct cleanup *old_chain)
{
  ...
  while ((*pmy_chain != NULL) && ((ptr = *pmy_chain) != old_chain))
    {
...
static struct internal_problem internal_error_problem = {
  "internal-error", AUTO_BOOLEAN_TRUE, AUTO_BOOLEAN_FALSE
};
  ...
  while (*str)
    {
      ...
      /* If this is a multi-byte Unicode char, print all bytes without
         interpretation.  */
      size_t nbytes_in_char;
      nbytes_in_char = get_nbytes_in_wchar ("UTF-8", str);
      if (nbytes_in_char > 1)
	while (nbytes_in_char-- && *str)
	  fprintf_unfiltered (stream, "%c", *str++);
      else
...
/* EMBARCADERO LOCAL: begin File logger */
/* If a debug log was requested by the user, open the log file and return 1,
   else return 0.  */
int
  ...
  if (getenv("MIGDB_DEBUG") != NULL)
    {
      ...
      if (file_log_fs == NULL)
	error ("Unable to open log file %s", logname);
      else
	{
	  file_log = stdio_fileopen (file_log_fs);
	  if (file_log == NULL)
	    error ("Unable to redirect output to log file %s", logname);
	  else
	    return 1;
	}
    }
