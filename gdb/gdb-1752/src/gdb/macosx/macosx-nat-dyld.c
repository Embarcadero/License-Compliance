...
// EMBARCADERO LOCAL: add debugging output
//#define DEBUG_MACOSX
#ifdef DEBUG_MACOSX
#define DBGPRINT(p) printf p
#else
#define DBGPRINT(p)
#endif
...
macosx_init_addresses (macosx_dyld_thread_status *s)
{
  ...
  DBGPRINT(("#25 find_objfile_by_name (\"dyld\",0) returns dyld_objfile=%p\n", dyld_objfile));
  ...
  if (dyld_objfile && dyld_objfile->obfd)
	  ...
	  osabi_seen_in_attached_dyld)
	{
	  ...
	  DBGPRINT(("#24 after reread_symbols_for_objfile, osabi_name=%s\n", osabi_name));
	}
    }
  ...
  if (s->dyld_image_infos == INVALID_ADDRESS)
    {
      ...
      DBGPRINT(("#23 after lookup_dyld_address, s->dyld_image_infos=%s\n", paddr(s->dyld_image_infos)));

      if (s->dyld_image_infos == INVALID_ADDRESS)
	{
	  DBGPRINT(("#22 s->dyld_image_infos == INVALID_ADDRESS, returning\n"));
	  return;
	}
    }
  ...
  DBGPRINT(("#20 s->dyld_notify=%s\n", paddr(s->dyld_notify)));
  DBGPRINT(("#21 s->dyld_slide = infos.dyld_actual_load_address - infos.dyld_intended_load_address=%s\n", paddr(s->dyld_slide)));
}
...
{
  DBGPRINT(("#26 macosx_locate_dyld_via_taskinfo(s=%p)\n", s));
  if (s == NULL)
    return 0;

  DBGPRINT(("#27 s->dyld_image_infos=%s\n", paddr(s->dyld_image_infos)));
  if (s->dyld_image_infos == INVALID_ADDRESS)
    {
     ...
     if (target_is_remote ())
	  ...
	  DBGPRINT(("#28 all_image_info_addr=%s\n", paddr(all_image_info_addr)));
       }
     else
       {
#if defined (NM_NEXTSTEP)
      ...
      DBGPRINT(("#31 TARGET_ARM : all_image_info_addr=%s\n", paddr(all_image_info_addr)));
      if (target_is_remote () && all_image_info_addr == INVALID_ADDRESS
          && dyld_starts_here_p (0x2fe00000))
        {
          s->dyld_addr = 0x2fe00000;
          s->dyld_slide = 0;
	  DBGPRINT(("#32 TARGET_ARM : s->dyld_addr=%s\n", paddr(s->dyld_addr)));
	  DBGPRINT(("#33 TARGET_ARM : s->dyld_slide=0\n"));
          return 1;
        }
#endif
	...
	{
	  DBGPRINT(("#34 all_image_info_addr == INVALID_ADDRESS => return 0\n"));
	  return 0;
	}
      s->dyld_image_infos = all_image_info_addr;
      DBGPRINT(("#35 TARGET_ARM : s->dyld_image_infos=%s\n", paddr(s->dyld_image_infos)));
    }
  ...
  if (s->dyld_image_infos != INVALID_ADDRESS)
    {
	  ...
	  DBGPRINT(("#37 after dyld_read_raw_infos\n"));
        }
      if (e.reason != NO_ERROR)
	{
	  DBGPRINT(("#36 TRY_CATCH failed => return 0\n"));
	  return 0;
	}
      ...
      if (raw_infos.dyld_actual_load_address != INVALID_ADDRESS
          ...
          && dyld_starts_here_p (raw_infos.dyld_actual_load_address))
        {
	      ...
	      DBGPRINT(("#38 : s->dyld_addr=%s\n", paddr(s->dyld_addr)));
	      DBGPRINT(("#39 : s->dyld_slide=%s\n", paddr(s->dyld_slide)));
	      DBGPRINT(("#43 return 1\n"));
              return 1;
            }
        }
	  ...
	  DBGPRINT(("#40 : s->dyld_addr=%s\n", paddr(s->dyld_addr)));
	  DBGPRINT(("#41 : s->dyld_slide=%s\n", paddr(s->dyld_slide)));
	  DBGPRINT(("#42 return 1\n"));
          return 1;
        }
    }
  DBGPRINT(("#44 FAILED return 0\n"));
  return 0;
}
  ...
  if (target_is_remote ())
    {
      if (hint != INVALID_ADDRESS && dyld_starts_here_p (hint))
        {
          *value = hint;
	  DBGPRINT(("#45 macosx_locate_dyld : hint=%s\n", paddr(hint)));
          return 1;
        }
      DBGPRINT(("#46 macosx_locate_dyld : FAILED return 0\n"));
      return 0;
    }
...
#endif /* NM_NEXTSTEP */
  DBGPRINT(("#47 macosx_locate_dyld : FAILED return 0\n"));
  return 0;
}
...
macosx_locate_dyld_static (macosx_dyld_thread_status *s, CORE_ADDR *value)
{
  ...
  DBGPRINT(("#48 macosx_locate_dyld_static : dyld_name=%s\n", dyld_name));
  ...
  ALL_OBJFILES (objfile)
  {
	...
	DBGPRINT(("#51 macosx_locate_dyld_static : bfd_filename=%s\n", bfd_filename));
	if (strstr (bfd_filename, "[memory object \"/usr/lib/dyld\"") == bfd_filename)
	  {
	    ...
	    *value = bfd_section_vma (objfile->obfd, text_section);
	    DBGPRINT(("#49 macosx_locate_dyld_static : value from bfd_section_vma (objfile->obfd, text_section) for LC_SEGMENT.__TEXT=%s\n", paddr(*value)));
	    return 1;
	  }
      }
    if (strcmp (dyld_name, bfd_get_filename (objfile->obfd)) == 0)
      {
        text_section = bfd_get_section_by_name (objfile->obfd, 
						"LC_SEGMENT.__TEXT");
        *value = bfd_section_vma (objfile->obfd, text_section);
	DBGPRINT(("#50 macosx_locate_dyld_static : value from bfd_section_vma (objfile->obfd, text_section) for LC_SEGMENT.__TEXT=%s\n", paddr(*value)));
        return 1;
      }
  }
  DBGPRINT(("#52 macosx_locate_dyld_static : FAILED => return 0\n"));
  return 0;
}
...
static struct ui_file *dump_prompt = NULL;
                           ...
                           int num)
{
  ...
  if (NULL == dump_prompt)
    dump_prompt = stdio_fileopen (stdout);

  fputs_unfiltered ("(gdb) \n", dump_prompt);
  gdb_flush (dump_prompt);
...
dyld_read_raw_infos (CORE_ADDR addr, struct dyld_raw_infos *info)
{
  ...
  if (info->version >= 9)
    {
      ...
      DBGPRINT(("#1 addr=%s\n", paddr(addr)));
      DBGPRINT(("#2 dyld_image_infos_addr=%s\n", paddr(dyld_image_infos_addr)));
      if (dyld_image_infos_addr != addr)
        {
	  // EMBARCADERO LOCAL begin: fix for iOS10
	  if (info->version >= 15)
	    {
	      uint64_t image_infos_offset = dyld_image_infos_addr - info->dyld_actual_load_address;
	      uint64_t notification_offset = info->dyld_notify - info->dyld_actual_load_address;
	      info->dyld_actual_load_address = addr - image_infos_offset;
	      info->dyld_notify = info->dyld_actual_load_address + notification_offset;
	      DBGPRINT(("#3 image_infos_offset=%s\n", paddr(image_infos_offset)));
	      DBGPRINT(("#4 notification_offset=%s\n", paddr(notification_offset)));
	      DBGPRINT(("#5 info->dyld_actual_load_address=%s\n", paddr(info->dyld_actual_load_address)));
	      DBGPRINT(("#5 info->dyld_notify=%s\n", paddr(info->dyld_notify)));
	      DBGPRINT(("#7 (not adjusted) info->info_array=%s\n", paddr(info->info_array)));
	    }
	  else
	    // EMBARCADERO LOCAL end: fix for iOS10
	    {
	      adjustment = addr - dyld_image_infos_addr;
	      DBGPRINT(("#6 adjustment=%s\n", paddr(adjustment)));
	    }
        }
    }
  ...
  if (adjustment != INVALID_ADDRESS
      && dyld_starts_here_p (info->dyld_actual_load_address + adjustment))
    {
      ...
      DBGPRINT(("#7 info->info_array=%s\n", paddr(info->info_array)));
      DBGPRINT(("#8 info->dyld_actual_load_address=%s\n", paddr(info->dyld_actual_load_address)));
      DBGPRINT(("#9 info->dyld_notify=%s\n", paddr(info->dyld_notify)));
    }

  if (dyld_starts_here_p (info->dyld_actual_load_address))
    {
      dyld_debug ("dyld infos: dyld was found at calculated load address = %s\n", 
		  paddr (info->dyld_actual_load_address));
      info->dyld_intended_load_address = find_text_segment_load_address 
					   (info->dyld_actual_load_address);
      DBGPRINT(("#10 info->dyld_intended_load_address=%s\n", paddr(info->dyld_intended_load_address)));
      // EMBARCADERO LOCAL begin: fix for iOS10
      if (info->version >= 15 &&
	  info->dyld_intended_load_address == 0)
	{
	  // Set the intended load address to INVALID_ADDRESS
	  // so that the actual address will be used as the slide
	  // amount.
      ...
      if (ui_out_is_mi_like_p (uiout))
	{
	  struct cleanup *notify_cleanup;
	  notify_cleanup =
	    make_cleanup_ui_out_notify_begin_end (uiout, "dyld-load-address-was-resolved");
	  do_cleanups (notify_cleanup);
	}
    }
  else
    {
      dyld_debug ("dyld infos error: dyld was not found at calculated load address = %s\n", 
		  paddr (info->dyld_actual_load_address));
      DBGPRINT(("#11 dyld not found at info->dyld_actual_load_address=%s\n", paddr(info->dyld_actual_load_address)));
      if (ui_out_is_mi_like_p (uiout))
	{
	  struct cleanup *notify_cleanup;
	  notify_cleanup =
	    make_cleanup_ui_out_notify_begin_end (uiout, "dyld-load-address-was-not-resolved");
	  do_cleanups (notify_cleanup);
	}
      else
	ui_out_text (uiout,
		     "error: dyld load address not found. Restart debug session.\n");
      info->dyld_intended_load_address = INVALID_ADDRESS;
      info->dyld_actual_load_address = INVALID_ADDRESS;
      DBGPRINT(("#12 reset to -1 info->dyld_actual_load_address=%s\n", paddr(info->dyld_actual_load_address)));
    }
}
                ...
                struct dyld_objfile_info *info, int dyldonly)
{
  ...
  if (status->dyld_addr != INVALID_ADDRESS)
    {
      ...
      entry->dyld_valid = 1;
      entry->reason = dyld_reason_dyld;
      DBGPRINT(("#53 dyld_info_read : entry->dyld_slide=%s\n", paddr(entry->dyld_slide)));
    }

  if (dyldonly)
    {
      DBGPRINT(("#54 dyld_info_read : dyldonly==1 => leaving\n"));
      return;
    }
  ...
  DBGPRINT(("#55 dyld_info_read : read %d dyld image infos\n", nrinfo));
  for (i = 0; i < nrinfo; i++)
    {
      ...
      DBGPRINT(("#56 dyld_info_read : rinfo[%d].name=%lu\n", i, rinfo[i].name));
...
macosx_dyld_update (int dyldonly)
{
  ...
  DBGPRINT(("#60 macosx_dyld_update() : libraries_changed=%d\n", libraries_changed));
  ...
  if (e->reason == dyld_reason_init)
    {
      ...
      DBGPRINT(("#61 set_load_state_1() : old_image_addr=%s\n", paddr(old_image_addr)));
      DBGPRINT(("#62 set_load_state_1() : old_pre_run_slide_addr=%s\n", paddr(old_pre_run_slide_addr)));
      DBGPRINT(("#63 set_load_state_1() : old_image_addr_valid=%d\n", old_image_addr_valid));
    }
  ...
  if (e->reason == dyld_reason_init)
    {
	  ...
	  DBGPRINT(("#64 set_load_state_1() : e->image_addr=%s\n", paddr(e->image_addr)));
          e->image_addr_valid = 1;
        }

    }

  DBGPRINT(("#65 set_load_state_1() : e->image_addr_valid=%d\n", e->image_addr_valid));
  DBGPRINT(("#65b set_load_state_1() : calling dyld_load_symfile_preserving_objfile\n"));
  ...
  if (e->reason == dyld_reason_init)
    {
      ...
      DBGPRINT(("#66 set_load_state_1() : reset e->image_addr=%s\n", paddr(e->image_addr)));
      DBGPRINT(("#67 set_load_state_1() : reset e->image_addr_valid=%d\n", e->image_addr_valid));
    }

#ifdef DEBUG_MACOSX
  DBGPRINT(("#68 dump of shlib:\n")); dyld_print_entry_info (e, index, 0);
#endif
  if (ui_out_is_mi_like_p (uiout))
    {
      struct cleanup *notify_cleanup;
...
dyld_set_load_state_command (char *args, int from_tty)
{
  ...
  /* Since we've change the load state of some libraries, we should
     see if any of our pending breakpoints will now take.  */
  DBGPRINT(("#69 dyld_set_load_state_command : calling re_enable_breakpoints_in_shlibs:\n"));
  re_enable_breakpoints_in_shlibs (0);
