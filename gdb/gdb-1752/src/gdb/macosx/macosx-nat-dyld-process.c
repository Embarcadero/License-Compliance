			     ...
			     int print_errors)
{
  ...
  e->loaded_error = 0;
  DBGPRINT(("#100 dyld_load_library_from_file: bfd_name=%s\n", e->loaded_name));
}
                            ...
                            int preserving_objfile_p)
{
  ...
  DBGPRINT(("#77 dyld_load_symfile_internal: bfd_filename=%s\n",
	    (e->objfile && e->objfile->obfd) ? bfd_get_filename(e->objfile->obfd) : "(null)"));

  if (e->loaded_error)
    {
      DBGPRINT(("#71 ERROR  dyld_load_symfile_internal: e->loaded_error=%d\n", e->loaded_error));
      return;
    }
  ...
  DBGPRINT(("#70 dyld_load_symfile_internal: e->dyld_valid=%s\n", paddr(e->dyld_valid)));
  if (e->dyld_valid)
    {
      e->loaded_addr = e->dyld_addr;
      e->loaded_addrisoffset = 0;
      DBGPRINT(("#72 dyld_load_symfile_internal: e->loaded_addr=%s (abs)\n", paddr(e->loaded_addr)));
    }
  else if (e->image_addr_valid)
    {
      e->loaded_addr = e->image_addr;
      e->loaded_addrisoffset = 0;
      DBGPRINT(("#73 dyld_load_symfile_internal: e->loaded_addr=%s (abs)\n", paddr(e->loaded_addr)));
    }
  else
    {
      e->loaded_addr = e->dyld_slide;
      e->loaded_addrisoffset = 1;
      DBGPRINT(("#74 dyld_load_symfile_internal: e->loaded_addr=%s (OFFSET)\n", paddr(e->loaded_addr)));
    }

  if (e->loaded_from_memory == 1)
    {
      e->loaded_memaddr = e->loaded_addr;
      DBGPRINT(("#75 dyld_load_symfile_internal: e->loaded_memaddr=%s\n", paddr(e->loaded_memaddr)));
    }
  ...
  if (e->objfile != NULL && !preserving_objfile_p)
    {
      DBGPRINT(("#76 calling slide_objfile(): e->dyld_slide=%s, e->dyld_section_offsets=%p\n", paddr(e->dyld_slide), e->dyld_section_offsets));
      ...
      if (e->dyld_section_offsets == NULL)
	{
	  ...
	  for (i = 0; i < addrs->num_sections; i++)
              ...
              if (e->dyld_valid == 0 && e->pre_run_slide_addr_valid == 1)
		{
		  ...
		  DBGPRINT(("#78 dyld_load_symfile_internal: bfd_filename=%s, addrs->other[i].addr = e->pre_run_slide_addr=%s\n",
			    (e->objfile && e->objfile->obfd) ? bfd_get_filename(e->objfile->obfd) : "(null)",
			    paddr(addrs->other[i].addr)));
		}
              else
		{
		  addrs->other[i].addr = e->dyld_slide;
		  DBGPRINT(("#79 dyld_load_symfile_internal: bfd_filename=%s, addrs->other[i].addr = e->dyld_slide=%s\n",
			    (e->objfile && e->objfile->obfd) ? bfd_get_filename(e->objfile->obfd) : "(null)",
			    paddr(addrs->other[i].addr)));
		}
	...
	addrs = NULL;
      ...
      if (e->objfile == NULL)
	{
	  DBGPRINT(("#80 calling symbol_file_add_bfd_safe \n"));
      ...
      else
	{
	  ...
	  TRY_CATCH (exc, RETURN_MASK_ALL)
	    {
	      DBGPRINT(("#81 calling symbol_file_add_with_addrs_or_offsets_using_objfile \n"));
	      e->objfile =
	  ...
	  {
	    DBGPRINT(("#82 dyld_load_symfile_internal : RETURN_ERROR\n"));
	    e->objfile = NULL;
	  }
	}
      e->abfd = NULL;
      ...
      if (e->objfile == NULL)
        {
          e->loaded_error = 1;
	  DBGPRINT(("#83 ERROR dyld_load_symfile_internal : loaded_error \n"));
          return;
        }

      e->loaded_name = e->objfile->name;
      /* CHECK_FATAL (e->objfile->obfd == e->abfd); */
      DBGPRINT(("#84 dyld_load_symfile_internal : e->loaded_name=%s\n", e->loaded_name?e->loaded_name:"(null)"));
  ...
  DYLD_ALL_OBJFILE_INFO_ENTRIES (result, e, i)
    {
      ...
      if (e->objfile != NULL)
        {
	  DBGPRINT(("#85 dyld_load_symfiles: bfd_name=%s\n",
		    (e->objfile && e->objfile->obfd) ? bfd_get_filename(e->objfile->obfd) : "(null)"));
          if (e->dyld_valid && e->loaded_addr == e->dyld_addr
              && !e->loaded_addrisoffset)
	    {
	      DBGPRINT(("#89 skipping,  e->loaded_addr == e->dyld_addr && !e->loaded_addrisoffset\n"));
	      continue;
	    }
          if (e->dyld_valid && e->loaded_addr == e->dyld_slide
              && e->loaded_addrisoffset)
	    {
	      DBGPRINT(("#90 skipping,  e->loaded_addr == e->dyld_slide && e->loaded_addrisoffset\n"));
	      continue;
	    }
          if (e->image_addr_valid && e->loaded_addr == e->image_addr
              && !e->loaded_addrisoffset)
	    {
	      DBGPRINT(("#91 skipping,  e->loaded_addr == e->image_addr && !e->loaded_addrisoffset\n"));
	      continue;
	    }
          if (!e->dyld_valid && !e->image_addr_valid)
	    {
	      DBGPRINT(("#92 skipping,  !e->dyld_valid && !e->image_addr_valid\n"));
	      continue;
	    }
        }
