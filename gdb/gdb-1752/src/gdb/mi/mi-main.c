...
mi_cmd_gdb_exit (char *command, char **argv, int argc)
{
  ...
  if (file_log)
    {
      if (current_command_token)
	fputs_unfiltered (current_command_token, file_log);
      fputs_unfiltered ("^exit\n", file_log);
    }
...
mi_cmd_exec_interrupt (char *args, int from_tty)
{
  ...
  if (file_log)
      ...
      if (current_command_token)
	{
      ...
      fprintf_unfiltered (file_log, "^done");
    }
...
mi_cmd_exec_status (char *command, char **argv, int argc)
{
  ...
  if (!target_has_execution)
	...
	/* File log.  */
	if (file_log)
	  fputs_unfiltered ("^done", file_log);
  ...
  if (file_log)
    {
      if (current_command_token)
	fputs_unfiltered (current_command_token, file_log);
      fputs_unfiltered ("^connected", file_log);
    }
...
/* EMBARCADERO Local: DbgExcNotify enable/disable command */
extern int debughook_enable;

enum mi_cmd_result
mi_cmd_debughook_control  (char *command, char **argv, int argc)
{
  if (argc == 0)
    debughook_enable = 1;
  else if (argc == 1)
 ...
 usg_error:
  error ("mi_cmd_debughook_control: Usage: %s {yes|no}", command);
  return MI_CMD_ERROR;
  
}

enum mi_cmd_result
mi_cmd_target_info_async (char *command, char **argv, int argc)
{
  if (argc > 1)
    {
      error (_("mi_cmd_target_info_async: No parameters required."));
      return MI_CMD_ERROR;
    }

  ui_out_text (uiout, "Can async mode: ");
  ui_out_field_string (uiout, "can_async",
		       (target_can_async_p ()) ? "true" : "false");
  ui_out_text (uiout, "\nIn async mode: ");
  ui_out_field_string (uiout, "in_async",
		       (target_is_async_p()) ? "true" : "false");
  ui_out_text (uiout, "\n");
...
captured_mi_execute_command (struct ui_out *uiout, void *data)
{
  ...
  switch (context->op)
	...
	/* File log. Drop incoming MI command.  */
	if (file_log)
	  fprintf_unfiltered (file_log, " token=`%s' command=`%s' args=`%s'\n",
			      context->token, context->command, context->args);
	  ...
	  if (args->rc == MI_CMD_DONE)
	    {
	      ...
	      if (file_log)
		{
		  fputs_unfiltered (context->token, file_log);
		  fputs_unfiltered ("^done", file_log);
		}
	      mi_out_put (saved_uiout, raw_stdout);
	      mi_out_rewind (saved_uiout);
	  ...
	  else if (args->rc == MI_CMD_QUIET)
	    {
	      ...
	      if (do_timings && context->cmd_start)
		print_diff (context->cmd_start, &cmd_finished);
	      /* EMBARCADERO LOCAL: We added extra prompt here for stable detection 
		 result of async -exec-* commands execution on remote target */
	      fputs_unfiltered ("\n(gdb) \n", raw_stdout);
	      /* File log.  */
	      if (file_log)
		fputs_unfiltered ("\n(gdb) \n", file_log);
		...
		{
		  fputs_unfiltered (context->token, file_log);
		  fputs_unfiltered ("^error", file_log);
		}
	      if (mi_error_message)
		{
		  ...
		  if (file_log)
		    {
		      fputs_unfiltered (",msg=\"", file_log);
		      fputstr_unfiltered (mi_error_message, '"', file_log);
		    }
...
mi_interpreter_exec_continuation (struct continuation_arg *in_arg)
{
  ...
  if (!target_executing) 
	...
	{
	  if (arg && arg->token)
	    fputs_unfiltered (arg->token, file_log);
	  fputs_unfiltered ("*stopped", file_log);
	}
	  ...
	  if (file_log)
	    {
	      if (arg && arg->token)
		fputs_unfiltered (arg->token, file_log);
	      fputs_unfiltered ("*started", file_log);
	    }
	  if (do_timings && arg && arg->timestamp)
            {
              end_remote_counts (arg->timestamp);
...
mi_execute_command (char *cmd, int from_tty)
{
  ...
  if (command != NULL)
      ...
      if (result.reason < 0)
	{
	  ...
	  if (file_log)
	    {
	      fputs_unfiltered (command->token, file_log);
	      fputs_unfiltered ("^error,msg=\"", file_log);
	    }
	  ...
	  if (file_log)
	    {
	      fputstr_unfiltered (result.message, '"', file_log);
	      fputs_unfiltered ("\"", file_log);
	    }
          mi_out_put (saved_uiout, raw_stdout);
          mi_out_rewind (saved_uiout);
	  fputs_unfiltered ("\n", raw_stdout);
...
mi_cmd_execute (struct mi_parse *parse)
{
  if (parse->cmd->argv_func != NULL
	  ...
	  if (!mi_command_completes_while_target_executing(parse->command))
	    {
	      ...
	      if (file_log)
		{
		  fputs_unfiltered (parse->token, file_log);
		  fputs_unfiltered ("^error,msg=\"", file_log);
		  fputs_unfiltered ("Cannot execute command ", file_log);
		  fputstr_unfiltered (parse->command, '"', file_log);
		  fputs_unfiltered (" while target running", file_log);
		  fputs_unfiltered ("\"\n", raw_stdout);
		}
	      return MI_CMD_ERROR;
	    }
    ...
    {
      /* EMBARCADERO Local: insert ?MI prefix for getting CLII "info line" true MI output */
      if (!strcmp (parse->cmd->cli.cmd, "info line"))
	{
	  void *buffer = NULL;
	  buffer = xmalloc (strlen(parse->args) + sizeof("?MI") + 1);
	  sprintf ((char*)buffer, "%s%s", "?MI", parse->args);
      ...
      if (file_log)
	{
	  fputs_unfiltered (parse->token, file_log);
	  fputs_unfiltered ("^error,msg=\"", file_log);
	  fputs_unfiltered ("Undefined mi command: ", file_log);
	  fputstr_unfiltered (parse->command, '"', file_log);
	  fputs_unfiltered (" (missing implementation)", file_log);
	  fputs_unfiltered ("\"\n", file_log);
	  gdb_flush (file_log);
	}
	...
	run = xstrprintf ("%s %s", cmd, args);
      ...
      if (mi_debug_p)
        ...
        fprintf_unfiltered (gdb_stdout, "cli=%s run=%s\n",
                            cmd, run);
      /* File log. Drop incoming CLI command.  */
      if (file_log)
	fprintf_unfiltered (file_log, "cli=%s run=%s\n",
                            cmd, run);
