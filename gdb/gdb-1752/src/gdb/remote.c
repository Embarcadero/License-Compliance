...
/* EMBARCADERO Local: begin Stop Socket and service thread */
#include <unistd.h>
#include <dirent.h>
#include <sys/types.h>
...
/* StopSocket control variables - main.c */
extern char *stopsocket_path;
extern int stopsocket_enable;

int fdstopsocket;
int fdstopsocket_accepted;

pthread_t tSS_ServiceThreadID;
int IsSSClientConnected = 0;
int FirstTReceived = 0;
/* EMBARCADERO Local: end Stop Socket */

/* EMBARCADERO Local: begin debugserver version */
unsigned int debugserver_version = 0;
/* EMBARCADERO Local: end debugserver version */
 ...
 * arguments.
 */
#define DEFAULT_MAX_REMOTE_PACKET_SIZE 4096
...
/* EMBARCADERO Local: begin Service thread for Stop Socket */
void *
SS_ServiceThread(void *pThreadParam)
{
  int fd_ss, rc;
  char buf[100];

  fd_ss = *(int *)pThreadParam;

  while ((rc = read (fd_ss, buf, sizeof (buf))) > 0)
    {
      /* Stop request ? */
      if (buf[0] == 0x03)
	{
...
/* EMBARCADERO Local: end Service thread for Stop Socket */
  ...
  while (1)
    {
      ...
      /* If socket setup done fine AND stopsocket enabled by client,
         create service thread.  */
      if (IsSSClientConnected 
	  && stopsocket_enable
	  && FirstTReceived)
	{
	  int PTError;
	  PTError = pthread_create (&tSS_ServiceThreadID,
				    NULL,
				    &SS_ServiceThread,
				    &fdstopsocket_accepted);
	}
      getpkt (buf, (rs->remote_packet_size), 1);
      if (IsSSClientConnected
	  && stopsocket_enable
	  && FirstTReceived)
	pthread_cancel (tSS_ServiceThreadID);
      signal (SIGINT, ofunc);
...
Packet: '%s'\n"),
		    ...
		    else if (strncmp (p, "qaddr", p1 - p) == 0)
		      {
			p_temp = unpack_varlen_hex (++p1, &qaddr);
				    p = p_temp;
		      }
...
Packet: '%s'\n"),
		    ...
		    /* EMBARCADERO Local: begin CPSR register read hack for iOS 7 */
#if defined (TARGET_ARM)
		    if (pnum == 16)
		      {
			/* ARM_PS_REGNUM = 25, handled separately */
			fieldsize = hex2bin (p, regs,
					     register_size (current_gdbarch,
							    25));
			p += 2 * fieldsize;
			set_register_CPSR_HACK (*(unsigned*)regs);
		      }
		    else
#endif /* TARGET_ARM */
		      /* EMBARCADERO Local: end CPSR register read hack for iOS 7 */
		      {
			fieldsize = hex2bin (p, regs,
					     register_size (current_gdbarch,
							    reg->regnum));
...
fetch_register_using_p (int regnum)
{
  ...
  p = buf;
  *p++ = 'p';
  /* EMBARCADERO Local: begin VFP register number hack for debugserver. 
     Apple debugserver using different S0 start number for VFP registers */
#if defined (TARGET_ARM)
  if ((regnum >= 26) /* KIRILL FIXME: replace with ARM_VFP_REGNUM_S0, see arm-tdep.h */
     && (debugserver_version >= 310))
     p += hexnumstr (p, regnum - 9);
  else
#endif
  /* EMBARCADERO Local: end VFP register number hack for debugserver */
...
store_register_using_P (int regnum)
{
  ...
  /* EMBARCADERO Local: begin VFP register number hack for debugserver. 
     Apple debugserver using different S0 start number for VFP registers */
#if defined (TARGET_ARM)
  if ((reg->pnum >= 26) /* KIRILL FIXME: replace with ARM_VFP_REGNUM_S0, see arm-tdep.h */
     && (debugserver_version >= 310))
     xsnprintf (buf, rs->remote_packet_size, "P%s=", phex_nz (reg->pnum - 9, 0));
  else
#endif
  /* EMBARCADERO Local: end VFP register number hack for debugserver */
...
remote_write_bytes (CORE_ADDR memaddr, const gdb_byte *myaddr, int len)
{
  ...
  /* EMBARCADERO Local: begin X packet may be serviced wrong */
  /* KIRILL iOS 8.x WORKAROUND: X pkt serviced wrong in latest Beta
     debugserver. We will use M packet instead, for a while. */
#if 0
  check_binary_download (memaddr);
#else
  remote_protocol_binary_download.support = PACKET_DISABLE;
#endif
  /* EMBARCADERO Local: end X packet may be serviced wrong */
...
remote_macosx_get_all_image_infos_addr ()
{
  ...
  /* EMBARCADERO LOCAL: Getting remote dyld hook address is critical op,
     so we need to set timeout 2 secs here against target not respond.  */
  {
    int old_remote_timeout = remote_timeout;
    /* Give up in 6 secs - including 3 retries.  */
    remote_timeout = 2;
    timed_out = getpkt_sane (buf, sizeof (buf) - 1, 0);
  ...
  /* EMBARCADERO Local: BEGIN Get debugserver version */
  putpkt ("qGDBServerVersion");
  {
    int old_remote_timeout = remote_timeout;
    /* Give up in 6 secs - including 3 retries.  */
    remote_timeout = 2;
  ...
  if ((timed_out)
     || (buf[0] == '\0'))
    {
      /* Unknown debugserver version or qGDBServerVersion not supported */
      debugserver_version = 0;
    }
  else if (!strcmp(buf,"name:debugserver;version:@("))
    {
      /* 8.0 Beta debugserver - version string set as "com.apple.debugserver-@(#)"*/
      debugserver_version = 310;
    }
  else
    {
      /* name:debugserver;version:xxx.xxx.xxx; */
      sscanf (buf,"name:debugserver;version:%d.%*s", &debugserver_version);  
    }

  if (remote_debug)
    {
      fprintf_unfiltered (gdb_stdlog, "debugserver major version: %d\n", debugserver_version);
    }
  /* EMBARCADERO Local: END Get debugserver version */
...
remote_macosx_create_inferior (char *exec_file, char *allargs, char **env, int from_tty)
{
  ...
  char *exe_and_args_buffer = (char *) xmalloc (exe_and_args_buffer_length*2);
  ...
  *exe_and_args_buffer_end = '\0';
  ...
  if (timed_out)
    {
      pop_target ();
//      error ("Error app and args string send timed out.");
      internal_error (__FILE__, __LINE__, "Error app and args string send timed out.");
    }
  else if (buf[0] == 'E')
    {
      pop_target ();
...
//      error ("Error launching timed out.");
      internal_error (__FILE__, __LINE__, "Error launching timed out.");
    }
  else if (buf[0] == 'E')
    {
      pop_target ();
...
#if 0 /* EMBARCADERO Local: use software stepping always due to atomics */
...
push_remote_macosx_target (char *name, int from_tty)
{
  ...
  /* EMBARCADERO Local: begin Service thread for Stop Socket */
  /* UNIX socket setup for receiving target stop requests
     to GDB.  Socket serviced by seperate thread created here.  */
  if (stopsocket_enable && (stopsocket_path != NULL))
    {
      mode_t oldmask;
      struct sockaddr_un ss_address;

      oldmask = umask (0000); /* Set socket permisions to *777 */

      if ((fdstopsocket = socket (AF_UNIX, SOCK_STREAM, 0)) == -1)
	{
	  if (file_log)
	    fprintf_unfiltered (file_log, "Stop socket create error, external target stop control disabled\n");
	  umask (oldmask);
	  return;
	}

      memset (&ss_address, 0, sizeof(ss_address));
      ss_address.sun_family = AF_UNIX;
      strcpy (ss_address.sun_path, stopsocket_path);
      ss_address.sun_len = SUN_LEN (&ss_address);
      unlink (stopsocket_path);

      if (bind (fdstopsocket, (struct sockaddr*)&ss_address,
		sizeof(ss_address)) == -1)
	{
	  if (file_log)
	    fprintf_unfiltered (file_log, "Stop socket bind error: %d, external target stop control disabled\n", errno);
	  umask (oldmask);
	}

      /* Only one client allowed.  */
      if (listen (fdstopsocket, 1) == -1)
	{
	  if (file_log)
	    fprintf_unfiltered (file_log, "Stop socket listen error: %d, external target stop control disabled\n", errno);
	  umask (oldmask);
	  return;
	}

      /* Block here until client connected or error.  */
      if ((fdstopsocket_accepted = accept (fdstopsocket, NULL, NULL)) == -1)
	{
	  if (file_log)
	    fprintf_unfiltered (file_log, "Stop socket accept error: %d, external target stop control disabled\n", errno);
	  umask (oldmask);
	  return;
	}

      umask (oldmask);
      IsSSClientConnected = 1;
    }
  /* EMBARCADERO Local: end Service thread for Stop Socket */
}
