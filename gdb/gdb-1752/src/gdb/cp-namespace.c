				  ...
				  struct obstack *obstack)
{
  if (SYMBOL_DEMANGLED_NAME (symbol) != NULL)
	...
	return;
      ...
      /* EMBARCADERO FIXME: can Delphi have anonymous namespaces?  */
...
cp_lookup_symbol_nonlocal (const char *name,
			   ...
			   const struct block *block,
			   const domain_enum domain,
			   /* EMBARCADERO LOCAL merge conflict */
			   struct symtab **symtab)
{
  struct symbol *sym;
			       ...
			       struct symtab **symtab)
{
  if (namespace[0] == '\0')
  ...
  else
    {
      ...
      /* EMBARCADERO LOCAL Delphi namespaces */
      /* EMBARCADERO FIXME: get language from block */
      if ((symtab && (*symtab) && (*symtab)->language == language_pascal)
	  || current_language->la_language == language_pascal)
	strcat (concatenated_name, ".");
      else
	strcat (concatenated_name, "::");
      strcat (concatenated_name, name);
      return lookup_symbol_file (concatenated_name, linkage_name, block,
				 domain, symtab, cp_is_anonymous (namespace));
    }
}
...
cp_lookup_symbol_namespace (const char *scope,
                            ...
                            const domain_enum domain,
			    const int search_parents,
			    /* EMBARCADERO FIXME: merge conflict */
			    struct symtab **symtab)
{
  struct symbol *sym;
  ...
  while (block != NULL)
    {
      sym = cp_lookup_symbol_imports (scope,name, linkage_name, block,
				      domain, search_parents, symtab);
  ...
  if (scope[scope_len] != '\0')
    {
      ...
      /* If the current scope is followed by "::" (or if Pascal * "."),
         skip past that.  */
      if (new_scope_len != 0)
	{
	  /* EMBARCADERO LOCAL: Delphi scopes */
