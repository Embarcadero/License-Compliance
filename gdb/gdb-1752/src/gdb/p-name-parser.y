...
#define	yymaxdepth pnamee_maxdepth
#define	yyparse	pnamee_parse
#define	yylex	pnamee_lex
#define	yyerror	pnamee_error
#define	yylval	pnamee_lval
#define	yychar	pnamee_char
#define	yydebug	pnamee_debug
#define	yypact	pnamee_pact	
#define	yyr1	pnamee_r1			
#define	yyr2	pnamee_r2			
#define	yydef	pnamee_def		
#define	yychk	pnamee_chk		
#define	yypgo	pnamee_pgo		
#define	yyact	pnamee_act		
#define	yyexca	pnamee_exca
#define yyerrflag pnamee_errflag
#define yynerrs	pnamee_nerrs
#define	yyps	pnamee_ps
#define	yypv	pnamee_pv
#define	yys	pnamee_s
#define	yy_yys	pnamee_yys
#define	yystate	pnamee_state
#define	yytmp	pnamee_tmp
#define	yyv	pnamee_v
#define	yy_yyv	pnamee_yyv
#define	yyval	pnamee_val
#define	yylloc	pnamee_lloc
#define yyreds	pnamee_reds		/* With YYDEBUG defined */
#define yytoks	pnamee_toks		/* With YYDEBUG defined */
#define yyname	pnamee_name		/* With YYDEBUG defined */
#define yyrule	pnamee_rule		/* With YYDEBUG defined */
#define yylhs	pnamee_yylhs
#define yylen	pnamee_yylen
#define yydefred pnamee_yydefred
#define yydgoto	pnamee_yydgoto
#define yysindex pnamee_yysindex
#define yyrindex pnamee_yyrindex
#define yygindex pnamee_yygindex
#define yytable	 pnamee_yytable
#define yycheck	 pnamee_yycheck
...
%token STRUCT CLASS UNION ENUM SIZEOF UNSIGNED COLONCOLON NSQUAL_DOT
%token TEMPLATE
%token ERROR
...
nested_name	:	NAME NSQUAL_DOT
		...
		|	nested_name template NSQUAL_DOT
			{ $$.comp = $1.comp;
			  d_right ($1.last) = make_empty (DEMANGLE_COMPONENT_QUAL_NAME);
			  $$.last = d_right ($1.last);
			  d_left ($$.last) = $2;
 ...
 retry:
  ...
  switch (c = *tokstart)
      ...
      if (lexptr[1] == '.' && lexptr[2] == '.')
	{
      ...
      {
#if 1 // HACK
	/* FIXME: we should do a lookup here to see if this is a namespace
	   qualifier or a field specifier.  But since the mangling is the
	   same for both, we can get away with pretending this is a namespace
	   specifier for now.  */
	lexptr++;
	return NSQUAL_DOT;
#else
...
char *
p_comp_to_string (struct demangle_component *result, int estimated_len)
{
  ...
  str = cplus_demangle_print (DMGL_DELPHI | DMGL_PARAMS | DMGL_ANSI, result, estimated_len, &err);
  if (str == NULL)
    return NULL;
...
struct demangle_component *
p_demangled_name_to_comp (const char *demangled_name, void **memory,
			   const char **errmsg)
{
...
static void
p_print (struct demangle_component *result)
{
  char *str;
  size_t err = 0;
  ...
  str = cplus_demangle_print (DMGL_DELPHI | DMGL_PARAMS | DMGL_ANSI, result, 64, &err);
  if (str == NULL)
    return;
...
main (int argc, char **argv)
{
  ...
  if (argv[arg] == NULL)
	...
	result = p_demangled_name_to_comp (str2, &memory, &errmsg);
	if (result == NULL)
	  {
	    fputs (errmsg, stderr);
	    fputc ('\n', stderr);
    ...
    {
      result = p_demangled_name_to_comp (argv[arg], &memory, &errmsg);
      if (result == NULL)
	{
	  fputs (errmsg, stderr);
	  fputc ('\n', stderr);
