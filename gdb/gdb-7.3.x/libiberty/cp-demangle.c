              ...
              const struct demangle_component *dc)
{
  ...
  switch (dc->type)
    ...
    case DEMANGLE_COMPONENT_LOCAL_NAME:
      ...
      if (((options & DMGL_JAVA) || (options & DMGL_DELPHI)) == 0)
	d_append_string (dpi, "::");
      else
	d_append_char (dpi, '.');
                     ...
                     demangle_callbackref callback, void *opaque)
{
  ...
  if (strncmp (mangled, "_GLOBAL_", 8) == 0
  ...
  else if ((options & DMGL_DELPHI) != 0)
    {
      if (mangled[0] == '_' && mangled[1] == 'Z'
	  && mangled[2] == 'T' && mangled[3] == 'I')
	{
	  type = DCT_TYPE;
	  cplus_demangle_init_info (mangled+2, options | DMGL_TYPES,
	      			    strlen (mangled+2), &di);
	}
