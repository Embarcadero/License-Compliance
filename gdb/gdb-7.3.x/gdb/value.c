...
#include <ctype.h>
/* EMBARCADERO LOCAL: fix warnings - math.h needed for pow */
#include <math.h>
...
{
  /* EMBARCADERO LOCAL begin Pascal lower bound of array should be 1 */
  struct gdbarch *gdbarch;
  gdbarch = get_type_arch(type);
  int low_bound = (current_language->la_language == language_pascal) ?
  ...
  *invp = 0;			/* Assume valid.  */
  ...
  if (code == TYPE_CODE_FLT)
  ...
  else if (nosign)
    {
      ...
      result = (ULONGEST) unpack_long (type, valaddr);
    }
  else
    {
      /* Signed -- we are OK with unpack_long.  */
      result = unpack_long (type, valaddr);
    }

  if (TYPE_DECIMAL_SCALE(type))
    result *= pow ((double)10.0, TYPE_DECIMAL_SCALE(type));
  return result;
}
		       ...
		       int fieldno, struct type *arg_type)
{
  ...
  /* EMBARCADERO LOCAL begin Delphi indexing string members */
  /* Due to a Dcc bug, string members are coming through with
     DW_AT_bit_size/DW_AT_bit_offset.  */
  if (TYPE_FIELD_BITSIZE (arg_type, fieldno)
      && TYPE_CODE (type) == TYPE_CODE_STRING)
    {
      /* value_from_longest() will turn the string into an array of chars
         and set the VALUE_ADDRESS to the actual start of the character array.
         We do this here so that subindexing into the string
         will evaluate to the correct character, however it will cause 
         an evaluation of the member's address to be that of the string,
         not the member.
	 FIXME: how to fix this so that both of these work? : 
         (gdb) p @MyTestObject.MyString
         (gdb) p MyTestObject.MyString[1]

	 Note: We can't let the value be lazy because we need to dereference
         now in order to get the real start of the string.  */
  ...
  /* EMBARCADERO LOCAL end Delphi indexing string members */
...
value_from_longest (struct type *type, LONGEST num)
{
  ...
  /* EMBARCADERO LOCAL begin Delphi string members */
  if (TYPE_CODE (check_typedef (type)) == TYPE_CODE_STRING)
    {
      /* We need to read the string from memory.
	 NUM is the address of the start of the Delphi Unicode string
	 structure, with a 4-byte length at NUM-4.

	 Note: we can't use value_coerce_string() to handle the indirection
	 because num is already dereferenced; num is the start of the string,
	 not the pointer to the struct containing the string.

	 Note: we can't free the newly allocated value because it's already 
	 been added to the list.  Since we think strings are rather
	 rare, we'll just leave that value unused in the list rather
	 than special casing the allocation above.  */

      struct value *stringval;
      ...
      gdbarch = get_type_arch (type);
      if (num == 0)
	error ("Can't access uninitialized string");

      /* Read string length, 4 bytes at address NUM-4.  */
      buf = (gdb_byte*)&stringlen;
      read_memory (num - 4, buf, 4);
      len = stringlen;
      //KIRILL: TODO: Temporary fix against crash on badly formed string info
      if ((len <= 0) || (len > 65536))
	error ("Can't access string with invalid length");

      /* Create a new value from the array of characters 
	 of the string at address NUM.  */
	  /* EMBARCADERO LOCAL for Pascal lower bound of array should be 1 */
	  lowbound = (current_language->la_language == language_pascal) ?
	               pascal_string_lower_bound(gdbarch) :  
	               current_language->string_lower_bound;
      ...
      stringtype = create_array_type ((struct type *) NULL,
				      chartype,
				      rangetype);
      stringval = allocate_value (stringtype);
      set_value_address (stringval, num);
      len = TYPE_LENGTH (value_enclosing_type (stringval));
      if (len)
	read_memory (num, value_contents_all_raw (stringval), len);
      set_value_lazy (stringval, 0);
      return stringval;
    }
  /* EMBARCADERO LOCAL end Delphi string members */
...
coerce_array (struct value *arg)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_STRING:
      arg = value_coerce_string (arg);
      if (TYPE_CODE (value_type (arg)) == TYPE_CODE_ARRAY)
...
struct value *
coerce_string (struct value *arg)
{
  arg = coerce_ref (arg);
  if (TYPE_CODE (value_type (arg)) == TYPE_CODE_STRING)
    arg = value_coerce_string (arg);
  return arg;
}
...
/* Return true if the function passed the specified type is using
   the convention of passing structures in memory (passing in the
   address as a reference parameter).  GCC_P is nonzero if compiled
   with GCC.  */

int
using_pass_value_as_reference (struct gdbarch *gdbarch,
			       ...
			       struct type *arg_type)
{
  ...
  if (arg_type == NULL)
    error (_("Argument type unknown."));
  arg_type = check_typedef (arg_type);
  if (arg_type == NULL)
    error (_("Argument type unknown."));
  code = TYPE_CODE (arg_type);
  if (code == TYPE_CODE_ERROR || code == TYPE_CODE_VOID)
    error (_("Argument type unknown."));

  /* Probe the architecture for the passing-by-value convention.  */
  return (gdbarch_pass_value_as_reference (gdbarch, function, arg_type));
}
