...
remote_close (int quitting)
{
  ...
  /* EMBARCADERO LOCAL closing remote debug session fixes */
...
#ifdef USE_WIN32API
/* EMBARCADERO Local StopPipe control variable - see main.c */
extern int stoppipe_enable;
/* EMBARCADERO Local: Sync target stop via pipe */
#define  STOP_PIPE "\\\\.\\pipe\\androidgdb"
BOOL   IsConnected = FALSE; 
HANDLE hPipe = INVALID_HANDLE_VALUE;
int pipebyteswasread = 0;
extern struct ui_file *file_log;
char pipebuf[256];
int pipe_verbose_flag = 0;
int first_time = 1;

DWORD dwTargetStopThreadId;
HANDLE hTargetStopThread = INVALID_HANDLE_VALUE;
int TargetStopThreadParam = 0;

DWORD TargetStopThreadProc ( LPVOID lpdwThreadParam )
{
  DWORD cbBytesRead = 0; 
  BOOL fSuccess = FALSE;
  HANDLE hThreadPipe  = NULL;

  if (lpdwThreadParam == NULL)
   return 0; 

  hThreadPipe = (HANDLE) lpdwThreadParam; 
  pipebyteswasread = 0;

  while (1) 
    { 
      fSuccess = ReadFile( hThreadPipe, &pipebuf[0], 1, &cbBytesRead, NULL);
	...
	{
	  pipebyteswasread = (int)cbBytesRead;
	  if (pipebyteswasread && (pipebuf[0] == 0x03))
	    {
//            if (!first_time)
		send_interrupt_sequence(); 
//            else
//              first_time = 0;
	  ...
	  if (pipebyteswasread && (pipebuf[0] == 0x04))
	    pipe_verbose_flag = 1;
	}
      SuspendThread(hTargetStopThread);
    }
  return 0; 
}
#endif
	  ...
	  do
	    {
	      ...
	      if (IsConnected)
		{
		  /* Log bytes received through stop pipe */ 
		  if (file_log && pipe_verbose_flag)
		    {
		      if (pipebyteswasread > 0)
			{
			  if (pipe_verbose_flag == 1)
			    {
			      pipe_verbose_flag = 2;
			      fprintf_unfiltered (file_log, "stop pipe verbose enabled\n");
			    } 
			  fprintf_unfiltered (file_log, "stop pipe received: #%d\n",
			      		      pipebuf[0]);
			  pipebyteswasread = 0;
			  gdb_flush(file_log); 
			}
		    }
		  /* Report pipe errors regardless of verbosity enabled */
		  if (file_log && (pipebyteswasread == -1))
		    {
		      fprintf_unfiltered (file_log, "stop pipe error: %d\n",
			  		  pipebyteswasread);
		      pipebyteswasread = 0;
		      gdb_flush(file_log); 
		      IsConnected = FALSE;
		      TerminateThread(hTargetStopThread, 0);
		      DisconnectNamedPipe(hPipe); 
		      CloseHandle(hPipe);
		    }
		  else
		    ResumeThread(hTargetStopThread);
		}
#endif
	      c = readchar (timeout);
#ifdef USE_WIN32API
...
_initialize_remote (void)
{
  ...
  char *cmd_name;
#ifdef USE_WIN32API
  /* EMBARCADERO LOCAL: using pipe (UNIX socket analogue on Win) as cmd channel 
                        for sending IDE target stop requests to GDB.  */
  DWORD	dwBytesRead;
  char	buf[100];

  if ( stoppipe_enable )
#if 0
       getenv ("DONTUSEANDROIDGDBSTOPPIPE") == NULL)
#endif
    {
      sprintf (buf, "%s%d", STOP_PIPE, getpid());
	...
	{
	  IsConnected = FALSE; /* Pipe creation failed */
	}
      else
        IsConnected = ConnectNamedPipe(hPipe, NULL); /* Will hang here until client connected */
      if (GetLastError() == ERROR_PIPE_CONNECTED)
	{
           IsConnected = TRUE;
           if (file_log)
             fprintf_unfiltered (file_log, "stop pipe client connected, target pausing available\n");
	}
      else
	if (file_log)
	  fprintf_unfiltered (file_log, "stop pipe client connection error, target pausing will be not available\n");
    }
  else
    IsConnected = FALSE; /* Disable stop pipe */
#endif
  ...
  if (IsConnected) 
    { 
      if (hTargetStopThread == INVALID_HANDLE_VALUE)
	hTargetStopThread = CreateThread (NULL,
					  512,
					  (LPTHREAD_START_ROUTINE)&TargetStopThreadProc,
					  (LPVOID) hPipe,
					  /* EMBARCADERO LOCAL create thread
					     in suspended mode.  */
					  CREATE_SUSPENDED,
					  &dwTargetStopThreadId);
	if (hTargetStopThread == NULL)
	  {
	  QUIT;
	    pop_target ();
	    error (_("Error creating target stop thread.  Target detached."));
	  }
	else
	  SetThreadPriority(hTargetStopThread, THREAD_PRIORITY_ABOVE_NORMAL);
    }
  else
    CloseHandle(hPipe);
#endif
