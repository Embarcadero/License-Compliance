...
run_command_1 (char *args, int from_tty, int tbreak_at_main)
{
  ...
  if (tbreak_at_main)
    {
      /* EMBARCADERO Local: Begin Use entry point address if present in image 
         instead of hardcoded main function sym */
      CORE_ADDR entry_address;

      /* Get entry point address of main module */
      entry_address = entry_point_address ();

      if (entry_address != NULL)
        {
          char buf[128];
          sprintf(buf, "*0x%x", entry_address);
          tbreak_command (buf, 0);
        }
      else
      /* EMBARCADERO Local: End Use entry point address if present in image */
...
/* EMBARCADERO Local: DbgExcNotify functionality control */
int set_dbgexchook = 0;
int debughook_enable = 0;
/* End of EMBARCADERO Local: DbgExcNotify functionality control */
...
continue_1 (int all_threads)
{
  ...
  if (non_stop && all_threads)
  ...
  else
    {
      ...
      if ((set_dbgexchook == 1) && debughook_enable)
	{
	  set_dbgexcnotify (); 
	  set_dbgexchook = 2;
	}
...

/* EMBARCADERO LOCAL: A command to get the inferior's process ID. 
Uses Apple patch for MI API compatibility */
void
...
/* EMBARCADERO Local: Stop pipe and service thread cleanup on target kill*/
#ifdef USE_WIN32API
extern BOOL   IsConnected; 
extern HANDLE hPipe;
extern HANDLE hTargetStopThread;
#endif
...
Register name as argument means describe only that register."));
  ...
  /* EMBARCADERO LOCAL info pid - uses Apple patch */
  /* "info pid" & MI's "-pid-info" are Apple local cmds for use by Xcode.  */
  add_info ("pid", pid_info,
	    _("Process ID of the program."));
