...
new_tty (void)
{
...
#if 0 /* EMBARCADERO LOCAL: fix "Failed to set controlling terminal" */

  /* This code causes the user to get the warning:
	"warning: GDB: Failed to set controlling terminal: Operation not permitted"

     This issue was originally reported to gdb and closed as "not a bug" in
     https://bugzilla.redhat.com/show_bug.cgi?id=593483.
     The following comment was added to the bug by Jan Kratochvil in 2010-09-06:
       "It is not fixable in GDB.  I can only disable a warning message to strictly
       match the previous GDB behavior.  For some forced takover of a console please
       consult kernel tty people."
 
     The patch which introduced this call to ioctl is
     http://sourceware.org/ml/gdb-patches/2008-03/msg00103.html.
     Accoring to the patch's comments, the patch was commited to fix the following case:
       "if the user specifies a different terminal for the inferior to run on,
       GDB doesn't set it as the inferior's controlling terminal."
 
     With paserver, there is no way for the user to specify a different terminal for
     the inferior to run on, so we don't need this fix, and hve therefore disabled
     it in our sources.
 
     Other fixes that we have also tried include:
     1. Redirect input/output to new tty terminal:
 	if (ioctl (tty, TIOCCONS) == -1)
     2. Passing 1 instead of 0 in case of PTY (XTerm):
 	if (ioctl (tty, TIOCSCTTY, 0) == -1 && ioctl (tty, TIOCSCTTY, 1) == -1)
     Both of which require that the following command be run:
 	sudo setcap cap_sys_admin+ep linuxgdb */
  ...
  if (ioctl (tty, TIOCSCTTY, 0) == -1)
    ...
    warning (_("GDB: Failed to set controlling terminal: %s"),
	     safe_strerror (errno));
#endif
#endif /* EMBARCADERO LOCAL: fix "Failed to set controlling terminal" */
