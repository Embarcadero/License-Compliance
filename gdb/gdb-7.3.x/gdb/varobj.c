...
char *varobj_language_string[] = { "unknown", "C", "C++", "Java", "Pascal" };
...
VEC(varobj_update_result) *varobj_update (struct varobj **varp, int explicit)
{
  ...
  if ((*varp)->root->rootvar == *varp)
	...
	r.changed = 1;
      ...
      /* EMBARCADERO Local: Always re-evaluate array value on update 
         like new for current scope. No method for members changes
         detection by value, not scope or type. */
      if ((*varp)->type && TYPE_CODE ((*varp)->type) == TYPE_CODE_ARRAY)
	r.changed = 1;
			 ...
			 struct value *value)
{
  ...
  /* EMBARCADERO Local: If the name is empty construct fake name
     in combination with index, as unique part of faked name.  */
  if (*name != '\0')
    childs_name = xstrprintf ("%s.%s", parent->obj_name, name);
  else
    childs_name = xstrprintf ("%s.#anonymous#%d", parent->obj_name, index);
		       ...
		       struct varobj *var)
{
  ...
  else if ((TYPE_CODE (value_type (value)) == TYPE_CODE_STRING)
	   || ((TYPE_CODE (value_type (value)) == TYPE_CODE_STRUCT)
		&& (TYPE_NAME (value_type (value)))
		&& !strcmp (TYPE_NAME (value_type (value)), "System::UnicodeString")))
      val_print (value_type (value),
		 value_contents_for_printing (value),
		 value_embedded_offset (value), value_address(value),
		 stb, 0, value, &opts, current_language);
  ...
  switch (TYPE_CODE (type))
      ...
      if (TYPE_DELPHI_DYNARRAY (type))
	{
	  ...
	  if (!var->value)
	    return 0;
	  if (var->not_fetched && value_lazy (var->value))
	    /* Frozen variable and no value yet.  */
	    return 0;
	  ...
	  if (errcode != 0 || dynarrstart == 0 || dynarrlen == 0)
	      /* Array inaccesible or empty.  */
	      children = 0;
	  else 
	    {
	      /* EMBARCADERO LOCAL begin strings. Be sure that we always have limitation for
	      strings equal to opts->print_max */
		  ...
		  char **cfull_expression)
{
  ...
  int was_ptr;
  /* EMBARCADERO Local: field name may be NULL */
  char *tname;
  ...
  switch (TYPE_CODE (type))
	...
	*cfull_expression = 
    ...
    case TYPE_CODE_UNION:
      /* EMBARCADERO Local: field name may be NULL */
      tname = TYPE_FIELD_NAME (type, index);
      if (!tname)
	tname = "unknown";
	...
	*ctype = TYPE_FIELD_TYPE (type, index);
	...
	{
	  char *join;
	  enum language lang = parent->root->exp->language_defn->la_language;

	  /* EMBARCADERO Local: use "." in Pascal */
	  if (lang == language_java || lang == language_pascal)
	    join = ".";
	  else
	    join = was_ptr ? "->" : ".";

	  /* EMBARCADERO Local: don't use parens in Pascal */
	  /* KIRILL FIXME: why were parens removed for Pascal?  */
	  if (lang == language_pascal)
	...
	{
	  if (cname)
	    *cname = xstrdup (int_string (index, 10, 1, 0, 0));

	  if (cvalue && value)
	    gdb_value_subscript (value, index, cvalue);
...
c_value_of_variable (struct varobj *var, enum varobj_display_formats format)
{
  ...
  /* delphireturn structs with 1 field are empty wrappers around the
     ...
     these types.  */
  while (TYPE_DELPHI_RETURN (type)
	 && TYPE_CODE (type) == TYPE_CODE_STRUCT
	 && TYPE_NFIELDS (type) == 1)
    {
      if (TYPE_N_BASECLASSES (type) == 1)
	type = check_typedef (TYPE_BASECLASS (type, 0));
      else
  ...
  switch (TYPE_CODE (type))
	...
	if (should_display_contents (type))
	  goto do_eval;
	/* EMBARCADERO LOCAL end display contents */
	return xstrdup ("{...}");
	/* break; */
	...
	if (var->value == NULL)
	...
	else
	  {
		...
		/* EMBARCADERO LOCAL: Only reuse print_value
		   if print_value != NULL.  */
		if ((format == var->format) && (var->print_value))
		  return xstrdup (var->print_value);
		else
