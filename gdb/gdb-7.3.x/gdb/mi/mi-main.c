...
mi_cmd_gdb_exit (char *command, char **argv, int argc)
{
  ...
  if (file_log)
    {
      if (current_token)
	fputs_unfiltered (current_token, file_log);
      fputs_unfiltered ("^exit\n", file_log);
    }
...
mi_cmd_data_evaluate_expression (char *command, char **argv, int argc)
{
  ...
  /* EMBARCADERO LOCAL Disable breakpoints during varobj evaluation. */
...
/* End of EMBARCADERO Local: DbgExcNotify enable/disable command */
...
captured_mi_execute_command (struct ui_out *uiout, void *data)
{
  ...
  switch (context->op)
	...
	fprintf_unfiltered (file_log, " token=`%s' command=`%s' args=`%s'\n",
			    context->token, context->command,
			    context->args ? context->args : "<NULL>");
...
mi_print_exception (const char *token, struct gdb_exception exception)
{
  ...
  if (file_log)
    {
      fputs_unfiltered (token, file_log);
  ...
  if (exception.message == NULL)
    {
      ...
      /* File log.  */
      if (file_log)
        fputs_unfiltered ("unknown error", file_log);
    }
  else
    {
      ...
      /* File log.  */
      if (file_log)
        fputstr_unfiltered (exception.message, '"', file_log);
    }
  fputs_unfiltered ("\"\n", raw_stdout);
  /* File log.  */
  if (file_log)
...
mi_cmd_execute (struct mi_parse *parse)
{
  ...
  if (parse->cmd->argv_func != NULL)
    ...
    {
      /* EMBARCADERO Local: insert ?MI prefix for getting CLI "info line" true MI output */
		  ...
		  unsigned long grand_total)
{
  ...
  if (new_section)
      ...
      if (current_token)
	{
      ...
      /* File log.  */
      if (file_log)
        fputs_unfiltered ("+download", file_log);
      cleanup_tuple = make_cleanup_ui_out_tuple_begin_end (uiout, NULL);
