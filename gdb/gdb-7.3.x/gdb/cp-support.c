		        ...
		        const char **errmsg)
{
  if (current_language->la_language == language_pascal)
    return p_demangled_name_to_comp (demangled_name, errmsg);
  else
    return cp_demangled_name_to_comp (demangled_name, errmsg);
}
...
cp_canonicalize_string (const char *string)
{
  ...
  ret_comp = demangled_name_to_comp (string, NULL);
  if (ret_comp == NULL)
    return NULL;

  estimated_len = strlen (string) * 2;
  ret = comp_to_string (ret_comp, estimated_len);
		      ...
		      void **memory, char **demangled_p)
{
  ...
  /* If we could demangle the name, parse it to build the component
     tree.  */
  ret = demangled_name_to_comp (demangled_name, NULL);
...
cp_func_name (const char *full_name)
{
  ...
  /* EMBARCADERO LOCAL: test for NULL full_name to not crash */
  if (!full_name)
    return NULL;

  ret_comp = demangled_name_to_comp (full_name, NULL);
  if (!ret_comp)
    return NULL;
...
cp_remove_params (const char *demangled_name)
{
  ...
  ret_comp = demangled_name_to_comp (demangled_name, NULL);
  if (ret_comp == NULL)
    return NULL;
