...
/* Embarcadero fCall wrapper
   ...
   The RTL code looks something like:
	...
	__try {
	  ...
	  // BP2 fcw_ResultFetchPoint (= fcw_ReturnLRPoint)
	}
	__except(1) {
   ...
   * fcw_ResultFetchPoint -
   ...
   Note: fcw_ReturnLRPoint is Just another name for fcw_ResultFetchPoint.
   KIRILL FIXME: why do we have both fcw_ResultFetchPoint and 
   fcw_ReturnLRPoint?  Can we remove one of them?
   ...
   * args_to_delete -
     If set, malloced pointer to fCall arguments to xfree on cleanup.
   ...
   - infcall.c (hand_function_call):
	  ...
	  Scan the fCall wrapper at fcallwrapper_addr...
	      ...
	      Set fcw_ResultFetchPoint, fcw_ReturnLRPoint, and user_lr
...
extern void push_current_fcw (CORE_ADDR fcallwrapper_addr);
