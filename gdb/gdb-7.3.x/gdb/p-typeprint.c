...
void
pascal_type_print_method_args (struct type *mtype, char *physname, char *methodname,
			       int staticp, struct ui_file *stream)
{
  ...
  /* EMBARCADERO Local: args from type, not mangled name. */
  if (mtype)
    {
      args = TYPE_FIELDS (mtype);
      nargs = TYPE_NFIELDS (mtype);
    }

  /* EMBARCADERO FIXME: check for Delphi ctor/dtor. */
  ...
  /* EMBARCADERO Local: begin args from type, not mangled name. */
  /* If we have the type, use that to print the args. */
  if (mtype)
    {
      /* Skip the self variable.  */
      i = staticp ? 0 : 1;
      if (nargs > i)
	{
	  fputs_filtered (" (", stream);
  ...
  /* EMBARCADERO Local: end args from type, not mangled name. */
  /* Else, determine the args by demangling the physname. */
  else if (physname && (*physname != 0))
    {
      fputs_filtered (" (", stream);
				  ...
				  int demangled_args)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_METHOD:
      if (passed_a_ptr)
	fprintf_filtered (stream, ")");
      pascal_type_print_method_args (type, "", "", 0, stream);
      if (TYPE_CODE (TYPE_TARGET_TYPE (type)) != TYPE_CODE_VOID)
	{
	  fprintf_filtered (stream, " : ");
			...
			int level)
{
  ...
  if (show <= 0
      && TYPE_NAME (type) != NULL)
    {
      /* EMBARCADERO Local begin: System.WideChar and possible others types */
      char *tpname = TYPE_NAME(type);
      int type_printed = 0;
      /* Is the name demangled?  Note: Delphi uses Itanium mangling.  */
      if (tpname[0] == '_')
	{
	  char *demangled = cplus_demangle (tpname, DMGL_DELPHI | DMGL_TYPES);
	  if (demangled != NULL)
	    {
      ...
      if (!type_printed)
      /* EMBARCADERO Local end: System.WideChar and possible others types */
        {
  ...
  switch (TYPE_CODE (type))
      ...
      else if (show > 0 || TYPE_TAG_NAME (type) == NULL)
	{
	  ...
	  for (i = 0; i < len; i++)
	      ...
	      for (j = 0; j < len2; j++)
		{
		  ...
		  pascal_type_print_method_args (TYPE_FN_FIELD_TYPE (f, j),
						 physname,
						 method_name,
	  ...
	  /* EMBARCADERO Local: begin properties support. */
	  /* If there are both fields/methods and properties, put a space between. */
	  len = TYPE_NPROP_FIELDS (type);
	  if (len && section_type != s_none)
	    fprintf_filtered (stream, "\n");

	  /* Delphi: print out the properties */
	  for (i = 0; i < len; i++)
	    {
	      struct prop_field *f = &TYPE_PROP_FIELD (type, i);
	      char *prop_name = TYPE_PROP_FIELD_NAME (type, i);

	      QUIT;
	      if (TYPE_PROP_FIELD_PROTECTED (type, i))
		{
		  if (section_type != s_protected)
		    {
			section_type = s_protected;
	      ...
	      else if (TYPE_PROP_FIELD_PRIVATE (type, i))
		{
		  if (section_type != s_private)
		    {
			section_type = s_private;
	      ...
	      else
		{
	      ...
	      /* Eg: property pValue : Integer read p_value write p_value; */
	      print_spaces_filtered (level + 4, stream);
	      if (TYPE_PROP_FIELD_STATIC (type, i))
	        fprintf_filtered (stream, "class ");
	      fprintf_filtered (stream, "property ");
	      if (TYPE_PROP_FIELD_TYPE (type, i) == 0)
		{
		  /* Keep GDB from crashing here.  */
		  fprintf_filtered (stream, "%s : <undefined type>;\n",
				    TYPE_PROP_FIELD_NAME (type, i));
		  break;
		}
	      fprintf_filtered (stream, "%s : ", TYPE_PROP_FIELD_NAME (type, i));
	      type_print (TYPE_PROP_FIELD_TYPE (type, i), "", stream, -1);
	      if (PROP_FIELD_READ_NAME (*f))
		{
		  fprintf_filtered (stream, " read %s",
				    PROP_FIELD_READ_NAME (*f));
		}
	      if (PROP_FIELD_WRITE_NAME (*f))
		{
		  fprintf_filtered (stream, " write %s",
				    PROP_FIELD_WRITE_NAME (*f));
		}
	      if (PROP_FIELD_INDEX (*f))
		{
	          fprintf_filtered (stream, " index %d",
				    PROP_FIELD_INDEX (*f));
		}
	      fprintf_filtered (stream, ";\n");
	    }
	  /* EMBARCADERO Local: end properties support. */
	...
	{
	  /* EMBARCADERO Local: System.WideChar and possible others types */
	  char *demangled = cplus_demangle (TYPE_NAME(type), DMGL_DELPHI | DMGL_TYPES);
	  if (demangled != NULL)
	    {
	      fputs_filtered (demangled, stream);
	      xfree (demangled);
