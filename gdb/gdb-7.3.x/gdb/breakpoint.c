...
should_be_inserted (struct bp_location *bl)
{
  ...
  TRY_CATCH (ex, RETURN_MASK_ERROR)
    {
      int state = read_memory_integer (bl->address, 4, gdbarch_bits_big_endian(target_gdbarch));
      rv = 1;
    }
...
/* EMBARCADERO Local: DbgExcNotify bkpt installer function */
int
set_dbgexcnotify (void)
{
  ...
  hook_sym = lookup_minimal_symbol ("_DbgExcNotify", NULL, NULL);
  if (hook_sym == NULL)
    {
      if (file_log)
        fprintf_unfiltered (file_log,
			    "_DbgExcNotify minsym not found, Delphi exception notifications disabled\n");
    }
  else
    {
      memset(buf,0,sizeof(buf));
      dbghook_addr = SYMBOL_VALUE_ADDRESS (hook_sym);
      if (file_log)
	fprintf_unfiltered (file_log,
			    "_DbgExcNotify() function minsym info found, address valid: %s\n", 
			    paddress (get_current_arch (), dbghook_addr));
      
      sprintf (&buf[0], "_DbgExcNotify"); /* Using global symbol */
      ...
      /* Set internal breakpoint on each sal was found */
      for (i=0; i < sals.nelts; i++)
	{
	  resolve_sal_pc (&sals.sals[i]);
      
	  b = set_raw_breakpoint (gdbarch, sals.sals[i], bp_breakpoint);
	  set_breakpoint_count (breakpoint_count + 1);
	  b->number = breakpoint_count;
	  b->cond_string = NULL; 
	  b->thread = thread;
      ...
      /* Update set bkpt(s) to actual locations */
      breakpoint_re_set ();
   }

  /* CPP exception notification hook */
  hook_sym = lookup_minimal_symbol ("_CPPDbgExcNotify", NULL, NULL);
  if (hook_sym == NULL)
    {
      if (file_log)
        fprintf_unfiltered (file_log,
			    "_CPPDbgExcNotify minsym not found, CPP exception notifications disabled\n");
      return 0;
    }
  else
    {
      ...
      if (file_log)
	fprintf_unfiltered (file_log,
			    "_CPPDbgExcNotify() function minsym info found, address valid: %s\n", 
			    paddress (get_current_arch (), dbghook_addr));
      
      sprintf (&buf[0], "_CPPDbgExcNotify"); /* Using global symbol */
  ...
  /* Update set bkpt(s) to actual locations.  */
  breakpoint_re_set ();
  /* Both exception hooks were set succesfully.  */
  return 1;
}

/* EMBARCADERO Local: _NativeMain bkpt installer - 
   called only on main module loaded event.  */
CORE_ADDR
get_nativemain_entry_point (void)
{
  struct breakpoint *b;
  struct bp_location *loc;
  struct symtabs_and_lines sals;
  int thread = -1;		/* All threads. */
  char *addr_start = "_NativeMain";
  char *addr_end = addr_start;
  char buf[256];
  char *fakeline[2];
  int i; /* nelts counter */
  CORE_ADDR result_entry_addr = NULL;
  struct symtab **file_symtab_arr = NULL;
  struct gdbarch *gdbarch = get_current_arch ();

  sprintf (&buf[0], "_NativeMain"); /* Using global symbol */
  ...
  for (i=0; i < sals.nelts; i++)
    {
      ...
      b->enable_state = bp_enabled;
      b->disposition = disp_donttouch;
      /* For debugging only - keep silence here */
      /* mention (b); */
    }
  ...
  /* Finding first valid location */
  for (loc = b->loc; loc; loc = loc->next)
    if (should_be_inserted (loc))
      {
	result_entry_addr = loc->address;
	delete_breakpoint (b);
	update_global_location_list (1);
	break;
      }

  return result_entry_addr;
}
	  ...
	  sym2 = lookup_symbol (SYMBOL_NATURAL_NAME (sym),
		...
		case PRINT_ALL_VALUES:
		  ...
		  if (!strcmp (SYMBOL_PRINT_NAME (sym), "NotificationKind"))
		    {
		      ULONGEST argumentvalue; 
		      regcache_raw_read_unsigned (get_current_regcache(), 0,
			  			  &argumentvalue); /* ARM_R0_REGNUM */
		      ui_out_field_int (uiout, "kind", argumentvalue);
		    }
		  else if (!strcmp (SYMBOL_PRINT_NAME (sym), "ExceptionObject"))
		    {
		      ULONGEST argumentvalue;
		      regcache_raw_read_unsigned (get_current_regcache(), 1,
			  			  &argumentvalue);
		      ui_out_field_core_addr (uiout, "object", gdbarch, argumentvalue);
		    }
		  else if (!strcmp(SYMBOL_PRINT_NAME (sym), "ExceptionName"))
		    {
		      char buf[256];
		      ULONGEST argumentvalue=0;
		      unsigned char namelen=0;
		      int errcode = 0;
		      memset (buf,0,sizeof(buf));
		      regcache_raw_read_unsigned (get_current_regcache(), 2,
						  &argumentvalue);
		      /* CPP Exception name is just NULL terminated string 
		         but name length restricted 255 bytes max */
		      if (!isdelphi) 
		        {
		      ...
		      else 
		        {
		          /* Delphi Exception name has Pascal ShortStr type, 
		             first byte is length - 255 bytes max */
		          errcode = target_read_memory (((char*)argumentvalue),
		                                        (char*)&namelen, 1);
			  ...
			  char *demangled = NULL;
			  if (!isdelphi)
			    demangled = cplus_demangle (&buf[0], DMGL_TYPES | DMGL_ANSI);
			  else
			    demangled = cplus_demangle (&buf[0], DMGL_TYPES | DMGL_DELPHI);
			  if (demangled != NULL)
			    {
			      ui_out_field_string (uiout, "name", demangled);
			      xfree(demangled);
			    }
			  else
		    ...
		    {
		      ULONGEST argumentvalue; 
		      regcache_raw_read_unsigned (get_current_regcache(), 3,
			  			  &argumentvalue); 
		      ui_out_field_core_addr (uiout, "addr", gdbarch, argumentvalue);
		    }
		  else if (!strcmp(SYMBOL_PRINT_NAME (sym), "HandlerAddr"))
		    {
		      ULONGEST argumentvalue; 
		      regcache_raw_read_unsigned (get_current_regcache(), 4,
			  			  &argumentvalue); 
		      ui_out_field_core_addr (uiout, "handler", gdbarch,
			  		      argumentvalue);
		    }
		  else
		    {
		      ...
		      if (!errcode)
			ui_out_field_core_addr (uiout, "unknown argument",
			    			gdbarch, argumentvalue);
		      else	
...
print_it_typical (bpstat bs)
{
  ...
  switch (b->type)
	...
	{
	  if (strcmp (b->addr_string, "System._DbgExcNotify")
	      && strcmp (b->addr_string, "_CPPDbgExcNotify"))	  
	    ui_out_field_string (uiout, "reason",
				 async_reason_lookup (EXEC_ASYNC_BREAKPOINT_HIT));
	  else
	    {
	      /* EMBARCADERO Local: Special handling for RTL exception notifications */
	      struct frame_info *fi;
...
//	      if (!block || (dict_empty (BLOCK_DICT (block))))
		{
		  ...
		  /* ExceptionObject */
		  {
		    ...
		    regcache_raw_read_unsigned (get_current_regcache(), 1,
						&argumentvalue);
		    ui_out_field_core_addr (uiout, "object",
					    get_current_arch(), argumentvalue);
		  }
		  /* ExceptionName */
		  {
		    char buf[256];
		    ...
		    /* CPP Exception name is just NULL terminated string 
		       but name length restricted 255 bytes max */
		    if (!strcmp(b->addr_string, "_CPPDbgExcNotify")) 
		      {
		        errcode = target_read_memory (((char*)argumentvalue),
						      (char*)&buf[0],
						      sizeof(buf)); 
		      ...
		      {
		        /* Delphi Exception name has Pascal ShortStr type, 
		           first byte is length - 127 bytes max */
		        errcode = target_read_memory (((char*)argumentvalue),
		                                      (char*)&namelen, 1);
		        errcode = target_read_memory (((char*)argumentvalue+1),
		                                      (char*)&buf[0], namelen);
		  ...
		  {
		    int errcode = 0; 
		    ULONGEST argumentvalue, stack_ptr;

		    /* HandlerAddr = *(dword *)(ARM_SP_REGNUM (13)) at *ExcNotify start */
		    regcache_raw_read_unsigned (get_current_regcache(), 13,
						&stack_ptr); 
		    errcode = target_read_memory (((char*)stack_ptr),
		    				  (char*)&argumentvalue, sizeof(ULONGEST));
		    /* SP must have some valid address always, 
		       but, just in case, who knows for sure ? */
		    if (!errcode)
		      ui_out_field_core_addr (uiout, "handler",get_current_arch(), argumentvalue);
		    else
		      ui_out_field_core_addr (uiout, "handler",get_current_arch(), 0xDEADBEEF);
		  }
		}
...
extern int fCallCatchTriggered;
  ...
  for (bs = bs_head; bs != NULL; bs = bs->next)
	...
	case bp_call_dummy:
	  ...
	  if (fcw_breakpoint_hit (bs->breakpoint_at,
	    ...
	    {
	      this_action = BPSTAT_WHAT_SINGLE_FCALL;
	      /* Push user's function arguments, if FCW is supported by current
		 arch.  */
	      gdbarch_fcw_argument_push (target_gdbarch);
	    }
...
/* EMBARCADERO LOCAL begin:  Disable user breakpoints during varobj evaluation. */
...
/* Find all the user breakpoints that are currently enabled, and
   ...
   won't be hit while in the middle.*/
...
/* EMABARCADERO LOCAL end:  Disable user breakpoints during varobj evaluation. */
			...
			int enabled, int internal)
{
  ...
  if (sals.nelts == 1)
    ...
    /* EMBARCADERO Local: replace multiple bkpts with single bkpts
       with multiple locations if addr string is same, otherwise
       problem with multiple breakpoint removal */
  if (sals.nelts > 1
      && !strcmp (canonical->canonical[0], canonical->canonical[1]))
			     ...
			     struct symtabs_and_lines sals_end)
{
  ...
  if (file_log)
    fprintf_unfiltered (file_log, "Updating bp locs\n");

  /* EMBARCADERO Local: multiple bkpts  */
...
addr_string_to_sals (struct breakpoint *b, char *addr_string, int *found)
{
  ...
  if (e.reason == 0 || e.error != NOT_FOUND_ERROR)
	...
	if (b->condition_not_parsed && s && s[0])
	  {
	    ...
	    /* EMBARCADERO Local: multiple bkpts  */
	    find_condition_and_thread (s, sals.sals[i].pc,
	...
	if (b->type == bp_static_tracepoint && !marker_spec)
	  /* EMBARCADERO Local: multiple bkpts  */
	  sals.sals[i] = update_static_tracepoint (b, sals.sals[i]);
      }
      *found = 1;
    }
  else
...
re_set_breakpoint (struct breakpoint *b)
{
  ...
  if (sals.nelts == 1)
    ...
    /* EMBARCADERO Local: multiple bkpts  */
    update_breakpoint_locations (b, sals, sals);
...
breakpoint_re_set_one (void *bint)
{
  ...
  if (file_log)
    fprintf_unfiltered (file_log, "Bkpt reset one\n");
...
is_syscall_catchpoint_enabled (struct breakpoint *bp)
{
  if (syscall_catchpoint_p (bp)
      ...
      && bp->enable_state != bp_hand_call_disabled
