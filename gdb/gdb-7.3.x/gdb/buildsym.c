...
#if 0 // DAWN FIXME: Is this needed?
      /* EMBARCADERO LOCAL DOS style filenames */
      /* If subfile_name has no path, don't compare with the fullname.  */
      if ((HAS_DIR_SEPARATOR (subfile_name)
	   && FILENAME_CMP (subfile_name, name) == 0)
	  || FILENAME_CMP (subfile_name, basename (name)) == 0)
#else
