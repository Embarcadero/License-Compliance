...
/* EMBARCADERO Local: _NativeMain addr */
extern CORE_ADDR nativemain_addr;

/* EMBARCADERO Local: This variable initialized with RTL 
   dbkFCallWrapperAddr ptr contents */
CORE_ADDR fcallwrapper_addr_ptr = NULL;

/* EMBARCADERO Local: begin DbgExcNotify functionality control */
extern int set_dbgexchook;
extern int debughook_enable;
/* EMBARCADERO Local: end DbgExcNotify functionality control */

/* EMBARCADERO Local: Using host images for .text and other ro 
   sections xfer ops instead of target */
...
solib_read_symbols (struct so_list *so, int flags)
{
  ...
  if (so->symbols_loaded)
      ...
      TRY_CATCH (e, RETURN_MASK_ERROR)
    {
      ...
      /* EMBARCADERO Local: Workaround against main module in solib...
         Adjusting main module sections like svr4_relocate_main_executable()
         does in main EXE case.  */
#if defined (HAVE_REALPATH)
      if (filename_cmp (gdb_realpath(so->objfile->name),
                symfile_objfile->name) == 0)
#else
      /* TODO: Should be replaced with proper realpath replacement for
         Windows host.  */
      if (strstr ((const char *)symfile_objfile->name,
              (const char *)so->objfile->name) != NULL)  
#endif
        {
          ...
          displacement = (so->objfile->section_offsets)->offsets[0]
                 - (symfile_objfile->section_offsets)->offsets[0];
        ...
        new_offsets->offsets[i] = displacement;
          ...
          /* EMBARCADERO Local: for IDE needs - DebugHook set to 1 */
          {
        struct symbol *sym;
        CORE_ADDR dbghook_addr = 0;

        sym = lookup_symbol ("DebugHook",
                     get_selected_block (&dbghook_addr),
                     VAR_DOMAIN, NULL);
        if (sym != NULL)
          {
            dbghook_addr = SYMBOL_VALUE_ADDRESS (sym);
            if (file_log)
              fprintf_unfiltered (file_log,
                      "DebugHook variable at %s\n",
                      paddress(get_current_arch (),
                      dbghook_addr));
            if (dbghook_addr != NULL)
              {
            char dbghookval = 1;
            if (target_write_memory (dbghook_addr, &dbghookval, 1) != 0)
            if (file_log)
              fprintf_unfiltered (file_log,
                          "DebugHook variable set failed\n");
              }
          }   
          }
          /* EMBARCADERO Local: Get entry point address - needed for fCall
        as reliable location in debuggee address space for return address. */
          nativemain_addr = get_nativemain_entry_point();

          /* EMBARCADERO Local: If we got reliable return address point for 
             fCall it means we can execute fCalls on target, let's try to 
             get fCall wrapper function address - it should be initialized 
             and exported by RTL. */
          if (NULL != nativemain_addr)
              fcallwrapper_addr_ptr = NULL;

          /* EMBARCADERO LOCAL begin debug hook */
          if (debughook_enable && !set_dbgexchook)
        {
          /* We can set hook right now */
          set_dbgexcnotify();
          set_dbgexchook = 2; 
        }
          else
        {
          /* Postponed hook bkpt - syms were resolved and ready.  */
          set_dbgexchook = 1;
        }
          /* EMBARCADERO LOCAL end debug hook */

         /* EMBARCADERO Local: main program lib loaded, so we
            can start using direct xfer ops on host debug images */ 
         trust_readonly = 1;
      }
    }
       ...
       struct target_ops *target, int readsyms)
{
     ...
     aren't already loaded, if no pattern was given.  */
  {
      ...
      if (! pattern || re_exec (gdb->so_name))
    {
      ...
      /* EMBARCADERO Local: Notify any observer that the shared object
         has been loaded now that we've added it to GDB's tables. 
         We do this here because info about symbols in loaded solib is
         known only at this point.  */
      if (!gdb->was_reported)
        {
          observer_notify_solib_loaded (gdb);
          gdb->was_reported = 1;
          if (raw_stdout != NULL)
        {
          fputs_unfiltered ("(gdb) \n", raw_stdout);
