...
#define PUSH_SIGS(nsigs,flags,save_flags) \
  do { \
    int signum = (nsigs); \
    while (signum-- > 0) \
	(save_flags)[signum] = (flags)[signum]; \
  } while (0)

#define POP_SIGS(nsigs,flags,save_flags) \
  do { \
    int signum = (nsigs); \
    while (signum-- > 0) \
	(flags)[signum] = (save_flags)[signum]; \
  } while (0)

#define SET_SIG_FLAGS(nsigs,flags,value) \
  do { \
    int signum = (nsigs); \
    while (signum-- > 0) \
	(flags)[signum] = value; \
...
{
  /* inferior may be partially deleted here on debuggee exit */
  if (inf && inf->pid)
...
process_event_stop_test:
  ...
  /* Handle cases caused by hitting a breakpoint.  */
  {
	...
	return;
      ...
      /* EMBARCADERO Local: fCall wrapper internal breakpoint action - 
	 no stop, just log and continue process here.  */
      case BPSTAT_WHAT_SINGLE_FCALL:
	if (using_current_fcw ())
	  {
	    if (debug_infrun)
	      fprintf_unfiltered (gdb_stdlog, "infrun: BPSTAT_WHAT_SINGLE_FCALL\n");
	    break;
	  }
	  /* Else, fall through.  */
...
struct infcall_control_state
{
  ...
  /* EMBARCADERO Local: save/restore signals in fCall */
  unsigned char *fcw_signal_stop_savearea;
  unsigned char *fcw_signal_print_savearea;
  unsigned char *fcw_signal_program_savearea;
};
...
save_infcall_control_state (void)
{
  ...
  /* EMBARCADERO Local begin: save/restore signals in fCall */
  if (using_current_fcw ())
    {
      int numsigs = (int) TARGET_SIGNAL_LAST-1;
      inf_status->fcw_signal_stop_savearea = (unsigned char *) 
	xmalloc (sizeof (inf_status->fcw_signal_stop_savearea[0]) * numsigs);
      inf_status->fcw_signal_print_savearea = (unsigned char *)
	xmalloc (sizeof (inf_status->fcw_signal_print_savearea[0]) * numsigs);
      inf_status->fcw_signal_program_savearea = (unsigned char *)
	xmalloc (sizeof (inf_status->fcw_signal_program_savearea[0]) * numsigs);

      PUSH_SIGS(numsigs,signal_stop,inf_status->fcw_signal_stop_savearea);
      PUSH_SIGS(numsigs,signal_print,inf_status->fcw_signal_print_savearea);
      PUSH_SIGS(numsigs,signal_program,inf_status->fcw_signal_program_savearea);

      SET_SIG_FLAGS(numsigs,signal_stop,0);
      SET_SIG_FLAGS(numsigs,signal_print,0);
      SET_SIG_FLAGS(numsigs,signal_program,1);

      signal_program[TARGET_SIGNAL_TRAP] = 0;
      signal_program[TARGET_SIGNAL_INT] = 0;
    }
  /* EMBARCADERO Local end: save/restore signals in fCall */
...
restore_infcall_control_state (struct infcall_control_state *inf_status)
{
  ...
  if (using_current_fcw ())
    {
      int numsigs = (int) TARGET_SIGNAL_LAST-1;
      POP_SIGS(numsigs,signal_stop,inf_status->fcw_signal_stop_savearea);
      POP_SIGS(numsigs,signal_print,inf_status->fcw_signal_print_savearea);
      POP_SIGS(numsigs,signal_program,inf_status->fcw_signal_program_savearea);

      xfree(inf_status->fcw_signal_stop_savearea);
      xfree(inf_status->fcw_signal_print_savearea);
      xfree(inf_status->fcw_signal_program_savearea);
    }
  /* EMBARCADERO Local end: save/restore signals in fCall */
