...
gnuv3_method_ptr_to_value (struct value **this_p, struct value *method_ptr)
{
  ...
  if (TYPE_NFIELDS (method_type) > 0
      && TYPE_FIELD_METACLASS (method_type, 0))
    {
...
gnuv3_pass_by_reference (struct type *type)
{
  ...
  /*EMBARCADERO Local: Check type length before give up here */
  if (TYPE_CODE (type) == TYPE_CODE_STRUCT
      && (TYPE_LENGTH (type) > 
	TYPE_LENGTH (builtin_type (get_type_arch (type))->builtin_int)))
      return 1;
