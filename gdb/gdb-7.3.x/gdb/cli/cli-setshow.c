...
do_setshow_command (char *arg, int from_tty, struct cmd_list_element *c)
{
  if (c->type == set_cmd)
	...
	case var_filename:
	case var_enum:
	  /* EMBARCADERO Local: current language hack for MI -gdb-show
	     command against show enum */
	  if (*(char**)c->var)
	    {
	      if ((!strcmp (c->name, "language"))
		  && (!strcmp(*(char **) c->var, "auto")))
		fprintf_filtered (stb->stream, "%s; currently %s",
				  *(char **)c->var, current_language->la_name);
	      else
