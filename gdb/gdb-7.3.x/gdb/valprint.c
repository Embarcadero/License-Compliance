...
/* EMBARCADERO LOCAL: begin skip field names */
/* If nonzero, causes field names of structure members to be printed.  */

static void
show_field_name_print (struct ui_file *file, int from_tty,
		       struct cmd_list_element *c, const char *value)
{
  fprintf_filtered (file, _("Printing of member names in structures is %s.\n"), value);
}
/* EMBARCADERO LOCAL: end skip field names */

/* EMBARCADERO LOCAL: begin expand aggregates in MI */
/* If nonzero, causes aggregates to be expanded when MI commands are used.  */

static void
show_expand_aggregates (struct ui_file *file, int from_tty,
			struct cmd_list_element *c, const char *value)
{
  fprintf_filtered (file, _("Printing of aggregate contents is %s.\n"), value);
}
		...
		struct ui_file *stream)
{
  ...
  /* EMBARCADERO Local: begin precision change */
  if (len < sizeof (double))
      fprintf_filtered (stream, "%.7g", (double) doub);
  else if (len == sizeof (double))
      fprintf_filtered (stream, "%.15g", (double) doub);
  else
#ifdef PRINTF_HAS_LONG_DOUBLE
    fprintf_filtered (stream, "%.19Lg", doub);
#else
    /* This at least wins with values that are representable as
       doubles.  */
    fprintf_filtered (stream, "%.15g", (double) doub);
#endif
  /* EMBARCADERO Local: end precision change */
}
			  ...
			  unsigned int i)
{
  ...
  for (; i < len && things_printed < options->print_max; i++)
	...
	{
	  val_print (elttype,
		     (TYPE_CODE (elttype) == TYPE_CODE_STRING) ? NULL : valaddr,
		     embedded_offset + i * eltlen,
		     address + embedded_offset + i * eltlen,
	     ...
	     enum bfd_endian byte_order, gdb_byte **buffer, int *bytes_read)
{
  ...
  /* EMBARCADERO LOCAL limit string length to print_max size */
  ...
  *buffer = NULL;
  ...
  if (len > 0)
    {
      /* EMBARCADERO LOCAL limit printed length*/
      if (len > opts.print_max)
        len = opts.print_max + 1;
	...
	{
	  QUIT;
      /* EMBARCADERO LOCAL avoid allocation of huge amount of memory */
      if (fetchlimit > opts.print_max)
        fetchlimit = opts.print_max + 1;
...
Show printing of addresses."), NULL,
  ...
  /* EMBARCADERO LOCAL: skip field names */
  add_setshow_boolean_cmd ("field-name", class_support,
			   &user_print_options.field_name_print, _("\
Set printing of member names in structures."), _("\
Show printing of member names in structures."), NULL,
			   NULL,
			   show_field_name_print,
			   &setprintlist, &showprintlist);

  /* EMBARCADERO LOCAL: expand aggregates in MI */
  add_setshow_boolean_cmd ("expand-aggregates", class_support,
			   &user_print_options.expand_aggregates, _("\
Set printing of aggregate members when MI commands are used."), _("\
Show printing of aggregate members when MI commands are used."), NULL,
			   NULL,
			   show_expand_aggregates,
			   &setprintlist, &showprintlist);
