...
/* EMBARCADERO LOCAL Delphi multi-byte chars */
enum c_string_type
c_classify_type (struct type *elttype, struct gdbarch *gdbarch,
		 const char **encoding)
{
  ...
  while (elttype)
      ...
      if (!strcmp (name, "wchar_t"))
	{
      ...
      /* EMBARCADERO LOCAL: Delphi UTF support  */
      /* Treat Delphi chars as char16_t.  */
      if (width == 2
	  && (!strcmp (name, "char16_t")
	      || !strcmp (name, "N6System8WideCharE")))
	{
	  result = C_CHAR_16;
	  goto done;
	}
...
c_printchar (int c, struct type *type, struct ui_file *stream)
{
  ...
  str_type = c_classify_type (type, get_type_arch (type), NULL);
  switch (str_type)
    {
    case C_CHAR:
      break;
...
/* Function returns start index of array depending on the type. */
int
c_string_lower_bound (struct gdbarch *gdbarch, struct type *stype)
{
 if (gdbarch_osabi(gdbarch) == GDB_OSABI_LINUX 
     && !is_target_linux_android()
     && c_delphi_string_type(stype))
    return 1;
  else
    return 0;
}
