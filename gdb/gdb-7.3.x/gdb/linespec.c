...

{
  ...
  *argptr = q;
  ...
  values.sals[0] = val;
  values.nelts = 1;
#endif //ifdef USE_NEW_LINESPEC_r22189

#ifdef USE_NEW_LINESPEC_r22189
  /* Scan the file_symtab and see if we how many 
...

  {
    ...
    /* EMBARCADERO LOCAL: begin check for NULL line table */
    if (!l)
      {
	error (_("no line table found for file \"%s\"."),
	       file_symtab->filename);
        return values;
      }
    /* EMBARCADERO LOCAL: end check for NULL line table */
  ...
  do_with_best:
            ...
            if (item->line == val.line)
              {
                ...
                if (new_blockvector == NULL)
                ...
                else
                  {
                    ...
                    sal->pspace = SYMTAB_PSPACE (val.symtab);
                  }
              }

            if (!exact && item->line > val.line && (best == 0 || item->line < best))
              {
                best = item->line;
              }
          }
    ...
    /* TODO: Find first always for now */
