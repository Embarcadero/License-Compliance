	     ...
	     const struct value_print_options *options)
{
  ...
  unsigned len;
  /* EMBARCADERO LOCAL Force ellipsis to be printed if nonzero */
  ...
  switch (TYPE_CODE (type))
	      ...
	      if (options->stop_print_at_null)
		{
		  ...
		  for (temp_len = 0;
			...
			&& extract_unsigned_integer (valaddr + embedded_offset
		       ...
		       ++temp_len)
		    ;
		  /* EMBARCADERO LOCAL begin print only print_max characters */
		  if (temp_len > options->print_max)
		    {
		      temp_len = options->print_max;
		      force_ellipsis = 1;
		    }  
		  /* EMBARCADERO LOCAL end print only print_max characters */
		  len = temp_len;
		}
	  ...
	  if (c_textual_element_type (elttype, options->format))
	      ...
	      if (0 != stringaddr)
		{
		  ...
		  if (errcode == 0)
			...
			if (errcode == 0)
			  {
			    gdb_assert (stringcharwidth == eltlen);
			    LA_PRINT_STRING (stream, elttype, buffer,
					     stringlength, NULL,
			...
			else
			  {
				...
				fputs_filtered (paddress (gdbarch, stringaddr),
						stream);
	...
	if (TYPE_DELPHI_RETURN (type)
	    && TYPE_NFIELDS (type) == 1)
	  {
	    ...
	    return c_val_print (type, valaddr, embedded_offset, address,
