...
error_unknown_struct_elt (struct type *type, char *name)
{
  ...
  typename = type_to_string (type);
  make_cleanup (xfree, typename);
  error (_("Type %s has unknown component named %s."), typename, name);
}
	   ...
	   char *name, struct objfile *objfile)
{
  ...
  /* EMBARCADERO Local: Delphi specific flags */
  if (flags & TYPE_FLAG_DELPHI_CLASS)
    TYPE_DELPHI_CLASS (type) = 1;
  if (flags & TYPE_FLAG_DELPHI_RECORD)
    TYPE_DELPHI_RECORD (type) = 1;
  if (flags & TYPE_FLAG_DELPHI_RETURN)
