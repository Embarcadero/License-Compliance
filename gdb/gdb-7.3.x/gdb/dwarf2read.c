...
#include "p-lang.h" /* FIXME: kludge for pascal_create_string_type() */
#include "valprint.h"
#include <ctype.h>
...
struct mapped_index
{
  /* EMBARCADERO LOCAL Pascal case insensitive globals */
...
/* EMBARCADERO LOCAL dwarf extensions */
static char *dwarf2_linkage_name (struct die_info *die, struct dwarf2_cu *cu);
			  ...
			  offset_type **vec_out)
{
  ...
  /* EMBARCADERO LOCAL Pascal case insensitive globals */
  hash = mapped_index_string_hash (index->version, name);
  ...
  if (attr && DW_STRING (attr))
	 ...
	 */
      ...
      if (cu->language == language_pascal)
        /* EMBARCADERO Local: Delphi support */
        demangle_options = DMGL_PARAMS | DMGL_DELPHI | DMGL_RET_DROP;
      else if (cu->language == language_java)
        demangle_options = DMGL_PARAMS | DMGL_ANSI | DMGL_JAVA | DMGL_RET_POSTFIX;
      else
        demangle_options = DMGL_PARAMS | DMGL_ANSI | DMGL_RET_DROP;

      demangled = cplus_demangle (mangled, demangle_options);
      if (demangled)
	{
...
read_import_statement (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (imported_die->tag != DW_TAG_namespace
    ...
    {
      /* EMBARCADERO LOCAL Delphi namespaces/units */
...
read_tag_Delphi_dynamic_array_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  type = alloc_type_copy (target_type);
  ...
  TYPE_LENGTH (type)
    = gdbarch_ptr_bit (get_type_arch (target_type)) / TARGET_CHAR_BIT;
...
read_tag_string_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (cu->language == language_pascal)
    {
      /* FIXME: This should be going through the vector, but
	 DELPHI_PRIMITIVE_TYPES had to be disabled in p-lang.c due to
	 regressions, so this is a hack for now.
         Note: On iOS we use dwarf2_fundamental_type() but we don't have
	 that on Android.  */
      type = pascal_create_string_type (gdbarch);
    }
  else
...
/* Embarcadero Local dccaarm - ignore compiler set default is_stmt value = 0 */
/* This define is a work-around for the lack of CFI instructions in the
   .debug_frame section on ARM targets by ignoring lines in prolog, there-by
   disallowing stepping through prolog code.  This has the unfortunate
   consequence of not recognising the function declaration as a viable
   statement for setting a breakpoint at.  */
#define WORKAROUND_FOR_ISSTMT_STEPINTO_BUG 1
...
#if defined(WORKAROUND_FOR_ISSTMT_STEPINTO_BUG)
		  if (!decode_for_pst_p)
#else
...
/* EMBARCADERO Local: DELETEME Compiler Clarify KIRILL DAWN: 
   Workaround against broken directories table disabled now 
   - issue should be fixed in compiler. */
#if 1
...
read_type_die_1 (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  switch (die->tag)
    ...
    case DW_TAG_BORLAND_Delphi_dynamic_array:
      ...
      gdb_assert (TYPE_DELPHI_DYNARRAY (this_type) == 1);
      break;
    /* EMBARCADERO LOCAL Delphi sets */
    case DW_TAG_BORLAND_Delphi_set:
