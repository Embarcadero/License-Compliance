...
got_symtab:
  ...
  ALL_OBJFILES (objfile)
  {
	...
	&& objfile->sf->qf->lookup_symtab (objfile, name, full_path, real_path,
					   &s))
      {
	/* EMBARCADERO Local: Breakpoint doesn't set first time */
	objfile->sf->qf->expand_symtabs_with_filename (objfile, name);
	found = 1;
	break;
      }
  }
                     ...
                     enum language language)
{
  ...
  if (gsymbol->language == language_d
      ...
      || gsymbol->language == language_pascal
			    ...
			    const char *mangled)
{
  ...
  if ((gsymbol->language == language_pascal
      ...
      || gsymbol->language == language_auto)
      /* EMBARCADERO LOCAL: Avoid changing symbol's language. */
      /* If current language is language_cplus and try demagle symbol as
         C++ symbol first. */
      && current_language->la_language != language_cplus)
    {
      demangled =
        cplus_demangle (mangled, DMGL_PARAMS | DMGL_DELPHI);
      if (demangled != NULL)
	{
	  gsymbol->language = language_pascal;
	  return demangled;
  ...
  if (gsymbol->language == language_cplus
      ...
      if (demangled != NULL)
	{
      ...
      /* EMBARCADERO LOCAL: begin - Avoid changing symbol's language. */
      /* If we failed to demagle the symbol as C++ and the current language
	 is C++, also try to demangle the symbol as Delphi. */
      else if (current_language->la_language == language_cplus)
	{
	  demangled = cplus_demangle (mangled, DMGL_PARAMS | DMGL_DELPHI);
	  if (demangled != NULL)
	    {
	      gsymbol->language = language_pascal;
	      return demangled;
      ...
      /* EMBARCADERO LOCAL: end - Avoid changing symbol's language. */
    }
			   ...
			   int *is_a_field_of_this)
{
  ...
  else if (lang == language_pascal)
    {
      demangled_name = cplus_demangle (name,
		      		       DMGL_PARAMS | DMGL_DELPHI);
      if (demangled_name)
	{
	  modified_name = demangled_name;
	  make_cleanup (xfree, demangled_name);
		   ...
		   int *is_a_field_of_this)
{
  ...
  /* Make sure we do something sensible with is_a_field_of_this, since
     ...
     certainly use it later and expect it to be either 0 (ifk_none), ifk_this,
     ifk_frame or ifk_frameptr.  If we don't set it, the contents of
  ...
  if (langdef->la_name_of_this != NULL && is_a_field_of_this != NULL
      ...
      if (sym)
	{
	  ...
	  if (check_field (t, name))
	    {
	      /* EMBARCADERO LOCAL: nested functions */
	      *is_a_field_of_this = ifk_this;
	      return NULL;
	    }
	}
    }
  /* EMBARCADERO LOCAL: begin nested functions */
  /* DAWN FIXME: move to function and add
     langdef->la_name_of_implicit_frame_struct and
     langdef->la_name_of_implicit_frameptr_param.  
     For now, we just define them here to keep changes to a minimum.  */
#define LA_NAME_OF_IMPLICIT_FRAME_STRUCT "__FRAME__"
#define LA_NAME_OF_IMPLICIT_FRAMEPTR_PARAM "__FRAMEPTR__"
  /* We didn't find a 'this' pointer.  Do we have a 'frame' structure?  */
  if (LA_NAME_OF_IMPLICIT_FRAME_STRUCT != NULL && is_a_field_of_this != NULL
      && block != NULL)
    {
      struct symbol *sym = NULL;
      const struct block *function_block = block;
	 ...
	 enclosing function block.  */
      ...
      if (function_block && !dict_empty (BLOCK_DICT (function_block)))
	sym = lookup_block_symbol (function_block, LA_NAME_OF_IMPLICIT_FRAME_STRUCT,
				   VAR_DOMAIN);
      if (sym)
	{
	  struct type *t = sym->type;
	  ...
	  if (check_field (t, name))
	    {
	      /* EMBARCADERO LOCAL: nested functions */
	      *is_a_field_of_this = ifk_frame;
	      return NULL;
	    }
	}
    }
  /* We didn't find a 'this' pointer or a 'frame' structure.
     Do we have a 'frameptr' parameter?  */
  if (LA_NAME_OF_IMPLICIT_FRAMEPTR_PARAM != NULL && is_a_field_of_this != NULL
      && block != NULL)
    {
      struct symbol *sym = NULL;
      const struct block *function_block = block;
	 ...
	 enclosing function block.  */
      ...
      if (function_block && !dict_empty (BLOCK_DICT (function_block)))
	sym = lookup_block_symbol (function_block, LA_NAME_OF_IMPLICIT_FRAMEPTR_PARAM,
				   VAR_DOMAIN);
      if (sym)
	{
	  struct type *t = sym->type;
	  ...
	  if (check_field (t, name))
	    {
	      /* EMBARCADERO LOCAL: nested functions */
	      *is_a_field_of_this = ifk_frameptr;
	      return NULL;
	    }
	}
    }
		       ...
		       domain_enum domain)
{
  ...
  if (symbol_language == language_cplus
      || symbol_language == language_pascal
...
/* EMBARCADERO Local: Only exact matches allowed in line-to-pc calculations */
#define MATCHEXACTLINEONLY 1
...
#ifdef MATCHEXACTLINEONLY
  symtab = find_line_symtab (symtab, line, &ind, &exact);
#else
  symtab = find_line_symtab (symtab, line, &ind, NULL);
#endif

#ifdef MATCHEXACTLINEONLY
  if ((symtab != NULL) && exact)
#else
  if (symtab != NULL)
#endif
    {
...
skip_prologue_sal (struct symtab_and_line *sal)
{
  ...
  /* EMBARCADERO Local: We never adjust SAL for prologue skip; 
     just let user to set bkpt at exact PC */
  /* if (sal->explicit_pc) */
    return;
